You can find the changelogs for the individual packages in their respective `CHANGELOG.md` files:

### Main AI SDK package

- [ai](./packages/ai/CHANGELOG.md)

### Providers

- [@ai-sdk/alibaba](./packages/alibaba/CHANGELOG.md)
- [@ai-sdk/amazon-bedrock](./packages/amazon-bedrock/CHANGELOG.md)
- [@ai-sdk/anthropic](./packages/anthropic/CHANGELOG.md)
- [@ai-sdk/anthropic-aws](./packages/anthropic-aws/CHANGELOG.md)
- [@ai-sdk/assemblyai](./packages/assemblyai/CHANGELOG.md)
- [@ai-sdk/azure](./packages/azure/CHANGELOG.md)
- [@ai-sdk/baseten](./packages/baseten/CHANGELOG.md)
- [@ai-sdk/black-forest-labs](./packages/black-forest-labs/CHANGELOG.md)
- [@ai-sdk/bytedance](./packages/bytedance/CHANGELOG.md)
- [@ai-sdk/cartesia](./packages/cartesia/CHANGELOG.md)
- [@ai-sdk/cerebras](./packages/cerebras/CHANGELOG.md)
- [@ai-sdk/cohere](./packages/cohere/CHANGELOG.md)
- [@ai-sdk/deepgram](./packages/deepgram/CHANGELOG.md)
- [@ai-sdk/deepinfra](./packages/deepinfra/CHANGELOG.md)
- [@ai-sdk/deepseek](./packages/deepseek/CHANGELOG.md)
- [@ai-sdk/elevenlabs](./packages/elevenlabs/CHANGELOG.md)
- [@ai-sdk/fal](./packages/fal/CHANGELOG.md)
- [@ai-sdk/fireworks](./packages/fireworks/CHANGELOG.md)
- [@ai-sdk/fish-audio](./packages/fish-audio/CHANGELOG.md)
- [@ai-sdk/gateway](./packages/gateway/CHANGELOG.md)
- [@ai-sdk/gladia](./packages/gladia/CHANGELOG.md)
- [@ai-sdk/google](./packages/google/CHANGELOG.md)
- [@ai-sdk/google-vertex](./packages/google-vertex/CHANGELOG.md)
- [@ai-sdk/groq](./packages/groq/CHANGELOG.md)
- [@ai-sdk/huggingface](./packages/huggingface/CHANGELOG.md)
- [@ai-sdk/hume](./packages/hume/CHANGELOG.md)
- [@ai-sdk/klingai](./packages/klingai/CHANGELOG.md)
- [@ai-sdk/lmnt](./packages/lmnt/CHANGELOG.md)
- [@ai-sdk/luma](./packages/luma/CHANGELOG.md)
- [@ai-sdk/minimax](./packages/minimax/CHANGELOG.md)
- [@ai-sdk/mistral](./packages/mistral/CHANGELOG.md)
- [@ai-sdk/moonshotai](./packages/moonshotai/CHANGELOG.md)
- [@ai-sdk/open-responses](./packages/open-responses/CHANGELOG.md)
- [@ai-sdk/openai](./packages/openai/CHANGELOG.md)
- [@ai-sdk/openai-compatible](./packages/openai-compatible/CHANGELOG.md)
- [@ai-sdk/perplexity](./packages/perplexity/CHANGELOG.md)
- [@ai-sdk/prodia](./packages/prodia/CHANGELOG.md)
- [@ai-sdk/quiverai](./packages/quiverai/CHANGELOG.md)
- [@ai-sdk/replicate](./packages/replicate/CHANGELOG.md)
- [@ai-sdk/revai](./packages/revai/CHANGELOG.md)
- [@ai-sdk/togetherai](./packages/togetherai/CHANGELOG.md)
- [@ai-sdk/voyage](./packages/voyage/CHANGELOG.md)
- [@ai-sdk/spacexai](./packages/spacexai/CHANGELOG.md)

### Framework Integrations

- [@ai-sdk/langchain](./packages/langchain/CHANGELOG.md)
- [@ai-sdk/llamaindex](./packages/llamaindex/CHANGELOG.md)

### UI integrations

- [@ai-sdk/angular](./packages/angular/CHANGELOG.md)
- [@ai-sdk/react](./packages/react/CHANGELOG.md)
- [@ai-sdk/rsc](./packages/rsc/CHANGELOG.md)
- [@ai-sdk/svelte](./packages/svelte/CHANGELOG.md)
- [@ai-sdk/vue](./packages/vue/CHANGELOG.md)

### Harnesses and Sandboxes

- [@ai-sdk/harness](./packages/harness/CHANGELOG.md)
- [@ai-sdk/harness-claude-code](./packages/harness-claude-code/CHANGELOG.md)
- [@ai-sdk/harness-codex](./packages/harness-codex/CHANGELOG.md)
- [@ai-sdk/harness-deepagents](./packages/harness-deepagents/CHANGELOG.md)
- [@ai-sdk/harness-opencode](./packages/harness-opencode/CHANGELOG.md)
- [@ai-sdk/harness-pi](./packages/harness-pi/CHANGELOG.md)
- [@ai-sdk/sandbox-just-bash](./packages/sandbox-just-bash/CHANGELOG.md)
- [@ai-sdk/sandbox-vercel](./packages/sandbox-vercel/CHANGELOG.md)
- [@ai-sdk/workflow-harness](./packages/workflow-harness/CHANGELOG.md)

### Other

- [@ai-sdk/code-mode](./packages/code-mode/CHANGELOG.md)
- [@ai-sdk/codemod](./packages/codemod/CHANGELOG.md)
- [@ai-sdk/devtools](./packages/devtools/CHANGELOG.md)
- [@ai-sdk/mcp](./packages/mcp/CHANGELOG.md)
- [@ai-sdk/otel](./packages/otel/CHANGELOG.md)
- [@ai-sdk/policy-opa](./packages/policy-opa/CHANGELOG.md)
- [@ai-sdk/provider](./packages/provider/CHANGELOG.md)
- [@ai-sdk/provider-utils](./packages/provider-utils/CHANGELOG.md)
- [@ai-sdk/test-server](./packages/test-server/CHANGELOG.md)
- [@ai-sdk/tui](./packages/tui/CHANGELOG.md)
- [@ai-sdk/valibot](./packages/valibot/CHANGELOG.md)
- [@ai-sdk/workflow](./packages/workflow/CHANGELOG.md)
