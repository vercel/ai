...
The `render` function is a helper function to create a streamable UI from an LLM response. It currently only supports
...
`render` provides a generator-based API for streaming updating UIs from the LLM.
