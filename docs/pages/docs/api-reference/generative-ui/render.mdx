...
By default, `render` will stream the text content of the LLM response wrapped with a React Fragment `<>` tag. This behavior can be modified with a custom component using the `text` key.
...
<Tabs items={['Next.js (App Router)']}>
    ...
    async function submitUserMessage(content: string) {
      ...
      return {
        id: Date.now(),
        // You can render ui on the client with something like `{message.ui}`
        display: ui
      };
    }
    ```
...
### Customizing the Streaming Text

You can also customize the React component by using the `text` key.
...
<Tabs items={['Next.js (App Router)']}>
    ...
    async function submitUserMessage(content: string) {
      ...
      return {
        id: Date.now(),
        // You can render ui on the client with something like `{message.ui}` and the
