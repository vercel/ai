...
The AWS Bedrock stream functions are utilties that transform the outputs from the AWS Bedrock API into a `ReadableStream`. They uses `AIStream` under the hood and applying a specific parser for the Bedrock's response data structure and the model.

There are currently 3 AWS Bedrock stream functions:
