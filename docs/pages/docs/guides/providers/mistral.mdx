...
<Callout>
  Mistral's REST APIs [are compatible with OpenAI's so we will use OpenAI's
  JavaScript SDK to make the requests. This makes it very easy to migrate and
  try out Mistral's models.
</Callout>
...
Mistral's REST APIs are compatible with OpenAI's so we will use OpenAI's JavaScript SDK to make the requests.
...
Create a Next.js Route Handler that uses the Edge Runtime that we'll use to generate a chat completion via Mistral that we'll then stream back to our Next.js.
...
// but configure it to point to Mistral
