...
<Callout>
Before the release of `openai@4`, we recommended using the `openai-edge` library because of it's compatibility with Vercel Edge Runtime. 
The OpenAI SDk now supports Edge Runtime out of the box, so we recommend using the official `openai` library instead.
</Callout>
