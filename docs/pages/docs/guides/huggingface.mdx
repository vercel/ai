...
Inference Endpoints offers a secure solution to deploy models from the Hub on dedicated and autoscaling infrastructured. It supports popular libraries such as `transformers` and `diffusers` out-of-the-box, but users can also implement custom handlers for their own solutions. Read more about Inference Endpoints in [their documentation](https://huggingface.co/docs/inference-endpoints/index).
 With Inference Endpoints, you can even use state of the art open-source LLMs requiring more compute such as [Falcon 40B](https://huggingface.co/blog/inference-endpoints-llm).
