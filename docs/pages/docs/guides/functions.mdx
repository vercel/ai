...
On the server, you can pass an `experimental_onFunctionCall` callback to the `OpenAIStream`, which will be called when the stream processes a complete "function_call" message.
The stream will block and wait for a function to be resolved or ignored before continuing streaming.
You can use the `createFunctionCallMessages` callback provided to construct the relevant "assistant" and "function" messages to send in your request.
...
const stream = OpenAIStream(response, {
  experimental_onFunctionCall: async (
    ...
    // if you do nothing with the function call, it's passed to the client
    if (name === 'get_current_weather') {
      ...
      return openai.createChatCompletion({
        // We attach the new function call messages so the model for the function call has up-to-date context
        messages: [...messages, ...newMessages],
        stream: true,
        model: 'gpt-3.5-turbo-0613',
        // If you want the function call to be able to call other functions, you need to pass a functions object again.
        // It does NOT need to be the same functions as passed to the stream.
        functions
      })
    }
  }
