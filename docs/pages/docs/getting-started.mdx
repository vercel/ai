...
We've written some code to get you started — follow the instructions below to download the code and run the app.
  ...
  <Tabs items={['Next.js', 'SvelteKit']}>
    ...
    ```sh
    pnpm create svelte@next my-ai-app
    ```
    </Tab>
    ...
    Next, we'll install `ai` and `openai-edge`. The latter is preferred over the official OpenAI SDK due to its compatiblity with Vercel Edge Functions.
