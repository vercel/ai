...
We've written some code to get you started — follow the instructions below to download the code and run the app.
  ...
  <Tabs items={['Next.js', 'SvelteKit']}>
    ...
    const config = new Configurationai
    ...
    In the above code, the `openai.createCompletion` method gets a response stream from the OpenAI API. This stream is then passed into the `OpenAIStream` function provided by the `ai` library.
    ...
    <Tabs items={['Next.js', 'SvelteKit']}>
      ...
      export default function SloganGenerator() {
        const { completion, input, handleInaitChange, handleSubmit } = useCompletion();
