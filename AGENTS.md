...
- Every `getFromApi` call in this repository must set `validateUrl` explicitly
  ...
  omitting it skips validation — never rely on that). Use `true` when the URL
  comes from a provider response body (image/audio/video download or a polling
  URL); use `false` only for URLs built from a configured `baseURL`.
