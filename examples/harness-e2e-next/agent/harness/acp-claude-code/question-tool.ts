...
export const claudeCodeACPAskUserQuestions = {
  ...
  isNativeToolCall: ({ nativeToolCall }) =>
    nativeToolCall._meta?.claudeCode != null &&
    (nativeToolCall._meta.claudeCode as Record<string, unknown>).toolName ===
      'AskUserQuestion',
  ...
  toNativeResponse: ({ nativeRequest, toolResult }) => {
    ...
    const nativeInput = claudeQuestionInputSchema.parse(
      toolResult.providerOptions?.['acp-claude-code']?.nativeToolInput,
    );
