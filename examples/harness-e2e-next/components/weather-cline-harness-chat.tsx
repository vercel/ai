...
import type { WeatherClineHarnessAgentMessage } from '@/agent/harness/cline/weather-agent';
...
export default function WeatherClineHarnessChat({
  apiRoute,
  exampleLabel,
}: {
  apiRoute: string;
  ...
  const {
    ...
    addToolOutput,
  } = useChat<WeatherClineHarnessAgentMessage>({
    id: chatId,
  ...
  return (
    <div className="flex flex-col pt-12 pb-24 mx-auto w-full max-w-5xl stretch">
      <h1 className="mb-2 text-xl font-bold">Cline — {exampleLabel}</h1>
        ...
        <div key={message.id} className="mb-3">
          ...
          {message.parts.map((part, index) => {
            switch (part.type) {
              ...
              case 'dynamic-tool': {
                // Cline already emits Write tool calls, so we don't need to surface these dynamic fileChange events, which effectively show the same thing.
