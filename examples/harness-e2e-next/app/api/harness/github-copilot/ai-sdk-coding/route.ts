import { githubCopilotAiSdkCodingHarnessAgent } from '@/agent/harness/github-copilot/ai-sdk-coding-agent';
    ...
    stream: createUIMessageStream({
      execute: async ({ writer }) => {
        const { session } = await resumeOrCreateSession({
          agent: githubCopilotAiSdkCodingHarnessAgent,
          chatId,
          ports: [4000],
        });
        const result = await githubCopilotAiSdkCodingHarnessAgent.stream({
          session,
          messages,
        });
        writer.merge(
