import { weatherGitHubCopilotHarnessAgent } from '@/agent/harness/github-copilot/weather-only-agent';
    ...
    stream: createUIMessageStream({
      execute: async ({ writer }) => {
        const { session } = await resumeOrCreateSession({
          agent: weatherGitHubCopilotHarnessAgent,
          chatId,
          ports: [4000],
        });
        const result = await weatherGitHubCopilotHarnessAgent.stream({
          session,
          messages,
        });
        writer.merge(
