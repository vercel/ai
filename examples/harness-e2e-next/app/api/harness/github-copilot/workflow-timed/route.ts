...
import { githubCopilotTimedWorkflow } from './workflow';

/*
 * Durable, multi-turn GitHub Copilot ACP chat via the Vercel Workflow DevKit. The
  ...
  return createUIMessageStreamResponse({
    stream: createUIMessageStream({
      execute: async ({ writer }) => {
        const run = await start(githubCopilotTimedWorkflow, [
