import { claudeCodeACPAiSdkCodingHarnessAgent } from '@/agent/harness/acp-claude-code/ai-sdk-coding-agent';
    ...
    stream: createUIMessageStream({
      execute: async ({ writer }) => {
        const { session } = await resumeOrCreateSession({
          agent: claudeCodeACPAiSdkCodingHarnessAgent,
          chatId,
          ports: [4000],
        });
        const result = await claudeCodeACPAiSdkCodingHarnessAgent.stream({
          session,
          messages,
        });
        writer.merge(
