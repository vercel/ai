...
import { claudeCodeACPTimedWorkflow } from './workflow';

/*
 * Durable, multi-turn ClaudeCode ACP chat via the Vercel Workflow DevKit. The
  ...
  return createUIMessageStreamResponse({
    stream: createUIMessageStream({
      execute: async ({ writer }) => {
        const run = await start(claudeCodeACPTimedWorkflow, [
