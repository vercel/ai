import { fxAiSdkCodingHarnessAgent } from '@/agent/harness/fx/ai-sdk-coding-agent';
    ...
    stream: createUIMessageStream({
      execute: async ({ writer }) => {
        const { session } = await resumeOrCreateSession({
          agent: fxAiSdkCodingHarnessAgent,
          chatId,
          ports: [4000],
        });
        const result = await fxAiSdkCodingHarnessAgent.stream({
          session,
          messages,
        });
        writer.merge(
