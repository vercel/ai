import { codexACPAiSdkCodingHarnessAgent } from '@/agent/harness/acp-codex/ai-sdk-coding-agent';
    ...
    stream: createUIMessageStream({
      execute: async ({ writer }) => {
        const { session } = await resumeOrCreateSession({
          agent: codexACPAiSdkCodingHarnessAgent,
          chatId,
          ports: [4000],
        });
        const result = await codexACPAiSdkCodingHarnessAgent.stream({
          session,
          messages,
        });
        writer.merge(
