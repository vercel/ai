import { weatherClineHarnessAgent } from '@/agent/harness/cline/weather-only-agent';
    ...
    stream: createUIMessageStream({
      execute: async ({ writer }) => {
        const { session } = await resumeOrCreateSession({
          agent: weatherClineHarnessAgent,
          chatId,
        });

        const result = await weatherClineHarnessAgent.stream({
          session,
          messages,
        });
