import { grokBuildAiSdkCodingHarnessAgent } from '@/agent/harness/grok-build/ai-sdk-coding-agent';
    ...
    stream: createUIMessageStream({
      execute: async ({ writer }) => {
        const { session } = await resumeOrCreateSession({
          agent: grokBuildAiSdkCodingHarnessAgent,
          chatId,
          ports: [4000],
        });
        const result = await grokBuildAiSdkCodingHarnessAgent.stream({
          session,
          messages,
        });
        writer.merge(
