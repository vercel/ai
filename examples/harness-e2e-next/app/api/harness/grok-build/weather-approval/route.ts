import { weatherApprovalGrokBuildHarnessAgent } from '@/agent/harness/grok-build/weather-approval-agent';
    ...
    stream: createUIMessageStream({
      execute: async ({ writer }) => {
        const session = await resumeOrCreateSession(
          weatherApprovalGrokBuildHarnessAgent,
          chatId,
        );
        const result = await weatherApprovalGrokBuildHarnessAgent.stream({
          session,
          messages,
        });
        writer.merge(
