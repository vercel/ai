import { weatherGrokBuildACPHarnessAgent } from '@/agent/harness/acp-grok-build/weather-only-agent';
    ...
    stream: createUIMessageStream({
      execute: async ({ writer }) => {
        const { session } = await resumeOrCreateSession({
          agent: weatherGrokBuildACPHarnessAgent,
          chatId,
          ports: [4000],
        });
        const result = await weatherGrokBuildACPHarnessAgent.stream({
          session,
          messages,
        });
        writer.merge(
