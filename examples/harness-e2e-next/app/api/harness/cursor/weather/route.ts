import { weatherCursorHarnessAgent } from '@/agent/harness/cursor/weather-agent';
    ...
    stream: createUIMessageStream({
      execute: async ({ writer }) => {
        const { session } = await resumeOrCreateSession({
          agent: weatherCursorHarnessAgent,
          chatId,
          ports: [4000],
        });
        const result = await weatherCursorHarnessAgent.stream({
          session,
          messages,
        });
        writer.merge(
