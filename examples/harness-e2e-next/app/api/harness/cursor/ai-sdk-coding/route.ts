import { cursorAiSdkCodingHarnessAgent } from '@/agent/harness/cursor/ai-sdk-coding-agent';
    ...
    stream: createUIMessageStream({
      execute: async ({ writer }) => {
        const { session } = await resumeOrCreateSession({
          agent: cursorAiSdkCodingHarnessAgent,
          chatId,
          ports: [4000],
        });
        const result = await cursorAiSdkCodingHarnessAgent.stream({
          session,
          messages,
        });
        writer.merge(
