...
import { cursorTimedWorkflow } from './workflow';

/*
 * Durable, multi-turn Cursor ACP chat via the Vercel Workflow DevKit. The
  ...
  return createUIMessageStreamResponse({
    stream: createUIMessageStream({
      execute: async ({ writer }) => {
        const run = await start(cursorTimedWorkflow, [
