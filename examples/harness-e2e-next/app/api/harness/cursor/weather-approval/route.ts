import { weatherApprovalCursorHarnessAgent } from '@/agent/harness/cursor/weather-approval-agent';
    ...
    stream: createUIMessageStream({
      execute: async ({ writer }) => {
        const { session } = await resumeOrCreateSession({
          agent: weatherApprovalCursorHarnessAgent,
          chatId,
          ports: [4000],
        });
        const result = await weatherApprovalCursorHarnessAgent.stream({
          session,
          messages,
        });
        writer.merge(
