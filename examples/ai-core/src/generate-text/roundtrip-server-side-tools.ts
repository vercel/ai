...
async function runOpenAIExample() {
  ...
  const result = await generateText({
    ...
    tools: {
      ...
      findNearbyCity: tool({
        ...
        inputSchema: z.object({
          searchTerm: z.string().describe('Search term or place to find nearby city for'),
        }),
        execute: async ({ searchTerm }) => {
          console.log(`Client finding nearby city for: ${searchTerm}`);
          
          const cities = ['Paris', 'London', 'New York', 'Tokyo', 'Sydney', 'Berlin'];
          return {
    ...
    prompt: 'I need help planning a trip. First search for recent travel news in Europe, then find nearby weather for any cities mentioned. Show me a summary with weather details.',
    ...
    onStepFinish: (step) => {
      ...
      step.toolCalls.forEach(call => {
        const wasProviderExecuted = call.providerExecuted ? ' (provider-executed)' : ' (client-executed)';
  ...
  console.log(`Total tool calls: ${result.steps.reduce((sum, step) => sum + step.toolCalls.length, 0)}`);
  console.log(`Total tool results: ${result.steps.reduce((sum, step) => sum + step.toolResults.length, 0)}`);
}
  ...
  const result = await generateText({
    ...
    tools: {
      ...
      getRestaurantRecommendations: tool({
        ...
        inputSchema: z.object({
          city: z.string().describe('The city to get restaurant recommendations for'),
          cuisine: z.string().optional().describe('Preferred cuisine type'),
        }),
        execute: async ({ city, cuisine }) => {
          console.log(`Client finding restaurants in ${city}${cuisine ? ` for ${cuisine} cuisine` : ''}`);
          ...
          return {
            ...
            recommendations: restaurants.slice(0, Math.floor(Math.random() * 3) + 2),
          };
        },
      }),
    },
    stopWhen: stepCountIs(3),
    prompt: 'Search for current food trends in major cities, then give me restaurant recommendations for one of the cities mentioned.',
...
async function main() {
  try {
    ...
    console.log('   • Server-side tools (web search) are executed by the provider');
    ...
    console.log('   • Provider-executed tools are excluded from client execution');
