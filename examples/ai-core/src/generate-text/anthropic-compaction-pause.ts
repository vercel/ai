...
run(async () => {
  const messages: Parameters<typeof generateText>[0]['messages'] = [
    ...
    },
    {
      role: 'user',
      content:
        'Finally, summarize the key algorithms and data structures that every developer should know according to the documentation.',
    },
  ];
  ...
  if (result.finishReason === 'other') {
    ...
    const hasCompaction = iterations?.some(iter => iter.type === 'compaction');
