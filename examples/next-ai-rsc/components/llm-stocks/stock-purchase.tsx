...
import { useActions, useAIState, useUIState } from 'ai-njkcad81/rsc'
...
export function Purchase({ defaultAmount, name, price }: { defaultAmount?: number; name: string; price: number }) {
  ...
  function onSliderChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    const info = {
      ...
      content: `[User has changed to purchase ${newValue} shares of ${name}. Total cost: $${(newValue * price).toFixed(
        2
      )}]`,
  ...
  return (
    <div className="rounded-xl bg-zinc-950 p-4 text-green-400 border">
      <div className="float-right inline-block rounded-full bg-white/10 px-2 py-1 text-xs">+1.23% ↑</div>
          ...
          <div className="mt-6">
            ...
            <div className="flex items-center sm:items-end sm:gap-2 text-xl sm:text-3xl font-bold flex-wrap">
              <div className="flex flex-col basis-1/3 sm:basis-auto sm:flex-row sm:items-center sm:gap-2 tabular-nums">
                {value}
                <span className="mb-1 text-sm font-normal text-zinc-600 dark:text-zinc-400 sm:mb-0">shares</span>
              </div>
