...
export function parseDiffForVisualization(diff: string): {
  ...
  addedLines: Array<{ line: string; lineNumber: number }>;
  removedLines: Array<{ line: string; lineNumber: number }>;
  contextLines: Array<{ line: string; lineNumber: number }>;
} {
  const diffLines = normalizeDiffLines(diff);
  const addedLines: Array<{ line: string; lineNumber: number }> = [];
  const removedLines: Array<{ line: string; lineNumber: number }> = [];
  const contextLines: Array<{ line: string; lineNumber: number }> = [];
  ...
  for (const rawLine of diffLines) {
    ...
    if (rawLine[0] === '+') {
      ...
      orderedLines.push({ type: 'added', line, lineNumber });
      lineNumber++;
    } else if (rawLine[0] === '-') {
      const line = rawLine.slice(1);
      removedLines.push({ line, lineNumber });
      orderedLines.push({ type: 'removed', line, lineNumber });
      lineNumber++;
    } else if (rawLine[0] === ' ') {
      const line = rawLine.slice(1);
      contextLines.push({ line, lineNumber });
      orderedLines.push({ type: 'context', line, lineNumber });
      lineNumber++;
    }
  }
