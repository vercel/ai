...
>(
  options: ContextParameter<TOOLS, NoInfer<USER_CONTEXT>> & {
    userContextFunction?: (
      context: USER_CONTEXT & InferToolSetContext<TOOLS>,
    ) => void;
  },
): void;
...
// mixed tools with context defined externally, user context per function, empty context
inferA({
  tools: mixedTools,
  userContextFunction: ({ x }: { x: string }) => ({
    x,
  }),
  // @ts-expect-error - hoisted tools preserve the required weatherApiKey
  context: {},
...
// mixed tools with context defined inline, user context per function, empty context
inferA({
  tools: {
    weather: tool({
      contextSchema: schema<{ weatherApiKey: string }>(),
