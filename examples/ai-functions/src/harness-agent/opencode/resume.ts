/*
 * Cross-process resume smoke test for the OpenCode harness.
 *
 ...
 * the persisted `HarnessAgentResumeSessionState`. The resume payload carries
 * the OpenCode session id so the bridge can reconnect the second turn to the
 * existing OpenCode session and the model remembers the name from turn 1.
 */
import {
  HarnessAgent,
...
run(async () => {
  ...
  let resumeState: HarnessAgentResumeSessionState;
  {
    ...
    console.log('\n[stopped] resume state:', JSON.stringify(resumeState));
  }
