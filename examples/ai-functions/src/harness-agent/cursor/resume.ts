/*
 * Cross-process resume smoke test for the Cursor harness.
 *
 ...
 * answers from the workdir state the Cursor ACP implementation preserved across the snapshot.
 */
import {
  HarnessAgent,
...
run(async () => {
  ...
  {
    const agent = new HarnessAgent({ harness: cursor, sandbox });
