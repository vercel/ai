...
/*
 * Context compaction (Cline).
 *
 * Cline performs context compaction natively (and automatically, when the context
 ...
 * RESULT"): the result carries `{ trigger, summary, tokensBefore? }` (Cline does
 * not report `tokensAfter`).
 *
 * Triggering: `session.compact(customInstructions?)` calls Cline's native
 * `session.compact()`. Cline aborts the current turn before it summarizes, so a
 ...
 *
 * NOTE on the summary: Cline keeps the most recent ~20k tokens (its
 ...
 * the conversation exceeds ~20k tokens. This is unrelated to Cline's AUTOMATIC
