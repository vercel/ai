...
const personalizedContext = Array.from({ length: 44 }, (_, index) => {
...
const taxonomy = Array.from({ length: 120 }, (_, index) => {
  const id = String(index + 1).padStart(3, '0');
...
Relevant taxonomy IDs: T-004, T-019, T-088, T-114.
...
function getMessages(scenario: Scenario) {
    ...
    return [
      ...
      },
      {
        ...
        content: [
          ...
          {
            type: 'image' as const,
            image: readFileSync('../ai-core/data/comic-cat.png'),
          },
        ],
      },
    ];
  ...
  return [
    ...
    },
    {
      ...
      content:
        'I loaded the account context and taxonomy and will use the requested schema.',
    },
    {
      role: 'user' as const,
      content:
...
function getRequestShape(body: Record<string, unknown>) {
...
async function runAttempt({
  ...
  activeLabel = `${scenario}/${attempt}`;

  try {
    const result = await generateObject({
      model: provider(MODEL),
      schema: analysisSchema,
      schemaName: 'personalized_analysis',
      schemaDescription:
        'A personalized analysis returned directly without an outer wrapper.',
...
async function main() {
  if (visionPrompt.length < 5_183) {
    throw new Error(
      `Harness error: vision prompt is only ${visionPrompt.length} characters`,
    );
  }
  ...
  for (let attempt = 1; attempt <= 3; attempt++) {
    await runAttempt({ scenario: 'vision', attempt });
  }

  for (let attempt = 1; attempt <= 5; attempt++) {
    await runAttempt({ scenario: 'cached-text', attempt });
  }
  ...
  for (const request of requestRecords) {
    console.log(
      JSON.stringify({
        label: request.label,
        requestShape: getRequestShape(request.body),
      }),
    );
  }

  console.log(
    'ISSUE_10845_NOT_REPRODUCED: all 8 live jsonTool calls returned schema-valid direct objects',
  );
}
