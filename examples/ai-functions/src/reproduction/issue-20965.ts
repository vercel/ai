...
async function main() {
  // Current main uses a DNS-pinned Node fetch for default downloads. Marking
  // this synthetic run as an Edge-style runtime keeps the default downloader
  // on the replaceable global fetch without changing the code path under test.
  const runtimeGlobal = globalThis as typeof globalThis & {
    EdgeRuntime?: string;
  };
  const previousEdgeRuntime = runtimeGlobal.EdgeRuntime;
  runtimeGlobal.EdgeRuntime = 'issue-20965-synthetic-fetch';
  ...
  try {
    ...
    if (previousEdgeRuntime === undefined) {
      ...
      runtimeGlobal.EdgeRuntime = previousEdgeRuntime;
    }
  }
}
