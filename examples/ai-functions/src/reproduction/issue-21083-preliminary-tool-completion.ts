...
async function main() {
  ...
  for (const kind of ['static', 'dynamic'] as const) {
    ...
    if (lastAssistantMessageIsCompleteWithToolCalls({ messages: [progress] })) {
      observations.push(
        `${kind} preliminary output was treated as tool-complete`,
      );
    }
  ...
  if (prematureSends.length > 0) {
    throw new Error(
      `${issueSignal}: ${prematureSends.join('; ')}. Helper observations: ${observations.join('; ')}`,
    );
  }
