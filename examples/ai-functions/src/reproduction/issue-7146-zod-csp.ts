...
async function runCurrentSupportedSetup(): Promise<ScenarioResult> {
  const zodV4Url = new URL(
    '../../../../tools/konsistent-provider/node_modules/zod/v4/index.js',
    import.meta.url,
  );
  const z = await import(zodV4Url.href);

  // Zod 4.4.2+ documents/fixes this opt-in behavior. It must be configured
  // before importing code that initializes Zod object schemas.
  z.config({ jitless: true });
  ...
  try {
    ...
    return {
      ...
      zodVersion: await readPackageVersion(
        new URL(
          '../../../../tools/konsistent-provider/node_modules/zod/package.json',
          import.meta.url,
        ),
      ),
    };
