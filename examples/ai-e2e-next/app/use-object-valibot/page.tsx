...
export default function Page() {
  const { submit, isLoading, object, stop, error, clear } = useObject({
    api: '/api/use-object-valibot',
    schema: notificationSchema,
  });

  return (
    <div className="flex flex-col items-center p-4 m-4 min-h-screen">
      <button
      ...
      <div className="flex flex-col gap-4 mt-4">
        {object?.notifications?.map((notification, index) => (
          <div
            className="flex gap-4 items-start p-4 bg-gray-100 rounded-md dark:bg-gray-800"
            key={index}
          >
