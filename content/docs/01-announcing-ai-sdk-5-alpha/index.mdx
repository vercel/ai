...
## For Community Provider Maintainers

AI SDK 5 introduces a [new LanguageModel specification](#languagemodelv2) that represents a breaking change from the previous version. Community provider packages will need to be updated to work with this new specification.

For detailed information about implementing the new specification in your provider package, please refer to our comprehensive documentation: [v5.ai-sdk.dev/providers/](https://v5.ai-sdk.dev/providers/community-providers/custom-providers)
...
To update an existing project dependencies, run the following command:

```bash
npm update ai@alpha ‘@ai-sdk/*'@alpha
```
...
const result = generateText({
  // ...
  // stop loop when weather tool called or at 5 steps
  stopWhen: [hasToolCall('weather'), stepCountIs(5)],
});
