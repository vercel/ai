  ...
  experimental_download: async (requestedDownloads) => {
    return Promise.all(
      requestedDownloads.map(async ({ url, isUrlSupportedByModel }) => {
        // Pass through if model supports this URL
        if (isUrlSupportedByModel) return url;

        try {
          const response = await fetch(url);

          // Remove broken images/files
          if (response.status === 404) return null;

          // Replace with text for auth errors
...
The download function can return:

- `{ data: Uint8Array, mediaType?: string }` - downloaded content
- `URL` - pass URL through to model as-is
- `string` - replace image/file part with text
