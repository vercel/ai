...
The default download implementation automatically downloads files when they are not supported by the model. However, it does not support retries, custom authentication, and more.

You can provide a custom download function:

```ts
const result = await generateText({
  model: openai('gpt-4o'),
  experimental_download: async ({ url, isUrlSupportedByModel }) => {
    // Let model handle supported URLs
    if (isUrlSupportedByModel) {
      return null;
    }

    // Custom download with auth
