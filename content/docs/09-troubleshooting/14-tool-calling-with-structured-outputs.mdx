...
When a generation finishes with tool calls (finish reason: `tool-calls`), the structured output parsing is automatically skipped.
This means:

- experimental_output will not enforce structured output when the LLM returns with tool calls as a last message.
...
For more information about using structured outputs with `generateText` and `streamText`
see [Generating Structured Data](/docs/ai-sdk-core/generating-structured-data#structured-outputs-with-generatetext-and-streamtext).
