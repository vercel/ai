...
By default, the AI SDK retains the full request and response bodies in the step results. When processing images, the request body contains the base64-encoded image data, which can be very large (a single image can be 1MB+ when base64 encoded). If you process many images and keep references to the results, this data accumulates in memory.

For example, processing 100 images of 500KB each would retain ~50MB+ of request body data in memory.

## Solution

Use the `experimental_retention` option to disable retention of request and/or response bodies:
...
const result = await generateText({
  ...
  // Disable retention of request body to reduce memory usage
...
The `experimental_retention` option accepts:

- `requestBody`: Set to `false` to exclude the request body from step results. This is where base64-encoded images are stored. Default: `true`.
- `responseBody`: Set to `false` to exclude the response body from step results. Default: `true`.
...
When you disable body retention:
