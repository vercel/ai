...
`embed` and `embedMany` throw
[`AI_NoEmbeddingGeneratedError`](/docs/reference/ai-sdk-errors/ai-no-embedding-generated-error)
when an embedding model does not return exactly one embedding for every
requested value. An empty `embedMany` input remains valid.

The error includes the requested values, any returned embeddings, expected and
actual counts, raw responses, usage, and provider metadata when available.
...
try {
  ...
  if (NoEmbeddingGeneratedError.isInstance(error)) {
    console.log(error.expectedCount, error.actualCount);
