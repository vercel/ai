...
Text batches require a provider that implements batch processing. Support is
provider- and model-specific. The first-party providers with support are:
...
Text batches support both client-defined tools and provider-defined tools. Tool
definitions are sent with each request that includes them, so the model can
call them independently for each request.
