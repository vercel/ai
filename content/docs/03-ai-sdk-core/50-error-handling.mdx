...
tool calls and tool results, are not replayed.

You can also decide dynamically in `onError`:
...
The default for `streamRetries` is `0`. An `onError` return value other than
`{ retry: true }` keeps its previous behavior and does not request recovery.
Provider error events that are recovered are not emitted as final `error`
parts.

<Note type="warning">
  Output emitted before a provider error cannot be retracted. A retried model
  step may therefore append repeated or divergent partial output to consumer
  streams. Retries also add latency and may incur additional provider usage and
  cost.
</Note>
