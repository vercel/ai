...
When the model calls a tool that doesn't exist or provides input that doesn't match the tool's schema, the AI SDK captures these as `InvalidToolCall` objects instead of throwing errors. In multi-step scenarios, error feedback is automatically sent back to the model.

You can inspect invalid tool calls on the result or on individual steps:
...
for (const invalidCall of result.invalidToolCalls) {
  console.log('Tool:', invalidCall.toolName);
  console.log('Error:', invalidCall.error);
}
```

See [Tool Calling — Handling Errors](/docs/ai-sdk-core/tools-and-tool-calling#handling-errors) for more details.
