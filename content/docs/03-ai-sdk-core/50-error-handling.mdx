...
const { textStream } = streamText({
  ...
  onAbort: ({ steps, usage, totalUsage }) => {
    ...
    console.log('Last step usage', usage);
    console.log('Total usage before abort', totalUsage);
    console.log('Reasoning tokens so far', usage.reasoningTokens ?? 'n/a');
    console.log('Cached input tokens so far', totalUsage.cachedInputTokens ?? 'n/a');
  },
...
- `usage`: Token usage of the last completed step (or `undefined` values when no step finished)
- `totalUsage`: Combined token usage across all completed steps

Both usage objects include `reasoningTokens` and `cachedInputTokens` when provided by the model, even if the stream aborts before completion.
