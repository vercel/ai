...
const { embeddings } = await embedMany({
  ...
  onStart({ callId, operationId, modelId, value, runtimeContext }) {
...
const { ranking } = await rerank({
  ...
  onEnd({ callId, ranking, runtimeContext }) {
...
Lifecycle callbacks receive the full `runtimeContext` values that flow through
`generateText`, `streamText`, `embed`, `embedMany`, `rerank`, and
`ToolLoopAgent` calls. Text generation and agent callbacks also receive
`toolsContext`.
