...
meaning-preserving audio-to-audio transforms such as translation and voice
conversion — analogous to how `LanguageModelV4` underlies both `generateText`
and `generateObject`. The translation function constrains the specification
to the translation task, e.g. by requiring `targetLanguage`.

<Note>
  Provider implementations of the speech-to-speech model specification ship
  separately. Pass any model instance that implements
  `Experimental_SpeechToSpeechModelV4` — see your provider's documentation for
...
- `responses`: Metadata about the speech-to-speech model responses, including
