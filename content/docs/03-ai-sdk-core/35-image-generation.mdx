...
Providers that report token counts for image generation expose them through the `usage` property, combined across all underlying provider calls.
...
Not every provider reports token usage — many image APIs bill per image rather than per token. Each field is `undefined` when the provider does not report it, and this is not an error condition. Providers also differ in which fields they populate: some report output and total tokens but no input token count. Check the provider page for specifics.
