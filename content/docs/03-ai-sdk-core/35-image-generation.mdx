...
### URL-Based Image Responses

Some image-generation endpoints return images as URLs instead of base64-encoded strings. The `generateImage` function automatically detects URLs in the response and downloads them, converting them to `GeneratedFile` format.
...
// Works seamlessly whether the provider returns base64 or URLs
console.log(image.base64); // Always available
console.log(image.uint8Array); // Always available
```

#### Custom Download Function (Experimental)

You can provide a custom download function via the `experimental_download` parameter to control how URLs are fetched. This is useful for implementing authentication, retries, caching, or other custom download logic.
...
const { image } = await generateImage({
  ...
  > => {
    // Custom download logic with authentication, retries, etc.
