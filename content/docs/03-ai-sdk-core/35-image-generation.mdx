...
Empty image responses are retried according to `maxRetries` before this error is thrown.
Image model providers can set `isRetryable: false` on an empty response when it
represents a terminal result such as a content policy block.
...
- `warnings`: Warnings from completed image model calls.
- `providerMetadata`: Provider-specific metadata from completed image model calls.
- `usage`: Combined token usage across completed image model calls.
