...
- `calls`: Per-call images, response metadata, provider metadata, warnings, and usage.
- `usage`: Combined token usage across calls.
- `providerMetadata`: Combined provider-specific metadata.
