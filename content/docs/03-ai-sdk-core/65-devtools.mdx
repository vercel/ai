...
video data is previewed directly, while the original JSON and metadata remain
...
preview** in the viewer to fetch it in your browser. Unsupported media,
provider references, malformed values, and unsafe URL schemes retain their JSON
and metadata fallback without an embedded preview.
