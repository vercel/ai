...
### Grouping steps with custom run IDs

By default, each `devToolsMiddleware()` instance creates a new run. To group steps across different model instances, provide a custom `runId`:
...
const model1 = wrapLanguageModel({
  middleware: devToolsMiddleware({ runId }),
  model: openai('gpt-4'),
});
await generateText({ model: model1, prompt: 'Analyze...' });

// Step 2 - appears under the same run
const model2 = wrapLanguageModel({
  middleware: devToolsMiddleware({ runId }),
  model: openai('gpt-4'),
});
await streamText({ model: model2, prompt: 'Summarize...' });
```

This is useful for:

- Grouping LLM calls per API request or user session
- Tracking multi-step workflows across different execution contexts
- Organizing calls in agent systems with multiple tool invocations
