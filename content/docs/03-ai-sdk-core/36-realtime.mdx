...
1. Your server creates a short-lived provider token with `realtime.getToken()`.
...
Use the `useRealtime` hook to connect to the provider, capture microphone audio,
...
const openaiModel = openai.realtime('gpt-4o-realtime-preview');
const googleModel = google.realtime('gemini-3.1-flash-live-preview');
const xaiModel = xai.realtime('grok-3');
const elevenlabsModel = elevenLabs.realtime('your-agent-id');
```
