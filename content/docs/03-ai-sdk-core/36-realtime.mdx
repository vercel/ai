...
The endpoint returns an `Experimental_RealtimeSetupResponse` with `token`,
`url`, and `tools`, plus optional `expiresAt` and `warnings` fields. Include
provider warnings from server-side session preparation in `warnings`. The
browser session deduplicates them with warnings produced while preparing the
client-side session configuration, logs them through the AI SDK warning logger,
and passes each warning to the optional `onWarning` callback.
