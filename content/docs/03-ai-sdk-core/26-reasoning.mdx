...
| `'none'`             | Disable reasoning where supported; known models may use a minimum fallback |
...
`'none'` is not a guarantee of zero reasoning tokens. For known models that
cannot disable thinking, some providers map it to the lowest supported effort
or budget. For example, Gemini 2.5 Pro uses a thinking budget of 128, and
OpenAI GPT-6 Astra uses `low` effort. Fallback support varies by provider and
model; custom model IDs keep their provider's existing behavior. Use explicit
`providerOptions` when you need a specific native setting.
