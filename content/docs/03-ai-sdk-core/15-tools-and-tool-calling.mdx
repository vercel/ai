...
call in one place. The callback receives `toolCall`, `tools`, `toolsContext`, and
`messages`:
...
const result = await generateText({
  ...
  toolApproval: ({ toolCall, tools, toolsContext, messages }) => {
    if (toolCall.toolName === 'runCommand' && !toolCall.dynamic) {
      // Inspect toolCall.input, cross-tool state, or messages for policy.
...
that receives the tool input and options `toolCallId`, `messages`, and
`context` (the same `context` shape as tool execution, without `abortSignal`).
