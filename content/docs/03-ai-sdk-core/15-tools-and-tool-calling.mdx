...
In order to have tool results from previous messages converted before sending them to the model, remember that you must pass the tools to `convertToModelMessages`.
See the [convertToModelMessages documentation](/docs/reference/ai-sdk-ui/convert-to-model-messages#multi-modal-tool-responses) for more details.
