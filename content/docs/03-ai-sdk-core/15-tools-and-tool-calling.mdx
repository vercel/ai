...
`data:` URLs in URL-style parts. HTTP(S) tool-result files are downloaded and
sent as inline data. With Google Vertex and Gemini 3 or later, `file-url` parts
can also reference `gs://` URLs directly when a supported full `mediaType` is
provided. See [Google Vertex tool result files](/providers/ai-sdk-providers/google-vertex#tool-result-files)
for supported MIME types and an example.
