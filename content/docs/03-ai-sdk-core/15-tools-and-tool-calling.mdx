...
- `experimental_context`: The runtime context passed via the `experimental_context` setting (experimental).
...
### Runtime Context (experimental)

You can pass in arbitrary runtime context from `generateText` or `streamText` via the `experimental_context` setting.
This runtime context is available in `prepareStep` and in the `experimental_context` tool execution option.
...
- Pass the full runtime context object through `experimental_context`
  ...
  tools: {
    weather: tool({
      ...
      execute: async (
        ...
        const { weatherApiKey } = experimental_context;
