...
- Set `toolApproval` on `generateText` or `streamText` to configure approval for specific tools at call time using approval statuses
...
`toolApproval` accepts one of four statuses per tool:
...
`toolApproval` can also be a function per tool, which lets you return one of these statuses dynamically based on the tool input and runtime options such as `toolCallId`, `messages`, and `toolContext`.
...
Manual approval can come from either tool-level `needsApproval` or call-level `toolApproval` returning `'user-approval'`. If the tool should just execute normally, use `'not-applicable'` or omit the setting entirely. If `toolApproval` returns `'approved'` or `'denied'`, the SDK records that decision automatically in the same generation by emitting approval request/response parts.
...
When the tool should execute without any approval metadata in the output, use `'not-applicable'` or omit `toolApproval`. Use `'approved'` or `'denied'` only when you want the result to include an automatic `tool-approval-request` with `isAutomatic: true` followed by a `tool-approval-response`, so you can inspect or render the decision without prompting the user again.
...
You can use a similar function in `toolApproval` when you want to return `'not-applicable'`, `'approved'`, `'denied'`, or `'user-approval'` at call time instead of defining the default on the tool itself. Return `'not-applicable'` for the normal execution path.
