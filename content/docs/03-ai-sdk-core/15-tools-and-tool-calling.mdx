...
- Pass shared step-level state through `context`
...
The `weather` tool then receives only its own typed `context` based on its `contextSchema`. In this case, `execute` can access `weatherApiKey`, while the shared step-level `context` remains separate.
