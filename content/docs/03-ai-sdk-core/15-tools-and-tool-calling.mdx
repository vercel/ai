...
Use `sensitiveContext` on a tool definition to exclude top-level context properties from telemetry integrations:

```ts highlight="10-12,25-29"
...
Properties set to `false` or omitted are still sent.
