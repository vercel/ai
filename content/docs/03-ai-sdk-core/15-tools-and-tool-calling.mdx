...
- Set `toolApproval` on `generateText`, `streamText`, or `ToolLoopAgent` to configure approval at call or agent level: a `GenericToolApprovalFunction` for all tool calls, or a per-tool map of statuses and/or `SingleToolApprovalFunction` callbacks (either function may return `undefined` for the same effect as `'not-applicable'`)
...
`messages`. It may return `undefined` for the same effect as `'not-applicable'`.
...
`toolContext` (the same shape as tool execution `context`, without `abortSignal`).
Return `undefined` for the same effect as `'not-applicable'`.
...
normally, use `'not-applicable'`, return `undefined` from a `GenericToolApprovalFunction` or
`SingleToolApprovalFunction`, or omit the setting entirely. If
