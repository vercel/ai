...
You can enable multi-step calls by setting the `maxSteps` setting to a number greater than 1. When `maxSteps` is set to a number greater than 1, the AI SDK will send any tool results back to the model alongside the initial prompt to trigger a new generation until there
...
<Note>
  When deciding what value to set for `maxSteps`, consider the most complex task
  the call might handle and the number of sequential steps required for
  completion, rather than just the number of available tools.
</Note>
