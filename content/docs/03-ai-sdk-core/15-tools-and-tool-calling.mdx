...
- `'not-applicable'`: no approval is required
- `'approved'`: approve automatically and continue in the same generation
- `'denied'`: deny automatically and surface a denied tool output
...
Manual approval can come from either tool-level `needsApproval` or call-level `toolApproval` returning `'user-approval'`. If `toolApproval` returns `'approved'` or `'denied'`, the SDK records the decision automatically in the same generation.
...
When approval is handled automatically with `'approved'` or `'denied'`, the result includes a `tool-approval-request` with `isAutomatic: true` followed by a `tool-approval-response`, so you can inspect or render the decision without prompting the user again.
...
You can use a similar function in `toolApproval` when you want to return `'not-applicable'`, `'approved'`, `'denied'`, or `'user-approval'` at call time instead of defining the default on the tool itself.
