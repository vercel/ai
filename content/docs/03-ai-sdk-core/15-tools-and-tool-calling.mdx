...
tool needs to interact with an execution environment. The sandbox is
...
the sandbox, such as `sandbox.executeCommand(...)`, run in the sandbox
environment.

```ts highlight="7-25,31"
import { generateText, tool } from 'ai';
import { z } from 'zod';
...
const result = await generateText({
  ...
  sandbox,
  prompt: 'List the files in the project and read package.json.',
});
```

The `Sandbox` interface includes `executeCommand({ command })` for running shell
commands and `readFile({ path })` for reading file contents from the sandbox.
...
The `Sandbox` interface is only a contract. Its isolation guarantees depend on
your implementation or sandbox provider. A local implementation that uses
...
available commands, set command timeouts, and combine sandbox usage with
[tool approval](#tool-execution-approval) for sensitive actions.
