...
- Set `toolApproval` on `generateText`, `streamText`, or `ToolLoopAgent` to configure approval at call or agent level: a `GenericToolApprovalFunction` for all tool calls, or a per-tool map of statuses and/or `SingleToolApprovalFunction` callbacks
...
`toolApproval` can be a `GenericToolApprovalFunction`: a single function that runs
for every tool call and receives `toolCall`, `tools`, `toolsContext`, and
`messages`, so you can apply one policy across all tools.

Alternatively, use a per-tool object. Each key can be a
`SingleToolApprovalFunction` that receives the tool input and options
`toolCallId`, `messages`, and `context` (the same `context` shape as tool
execution, without `abortSignal`).
...
Manual approval can come from either tool-level `needsApproval` or call-level
`toolApproval` returning `'user-approval'`. If the tool should just execute
normally, use `'not-applicable'` or omit the setting entirely. If
`toolApproval` returns `'approved'` or `'denied'` or their object forms, the
SDK records that decision automatically in the same generation by emitting
approval request/response parts. When you provide a `reason` on an automatic
approval or denial, that reason is included in the emitted approval response
...
`'not-applicable'` or omit `toolApproval`. Use `'approved'`, `'denied'`, or
...
`'not-applicable'`, `'approved'`, `'denied'`, or `'user-approval'` at call time
...
`{ type: 'denied', reason: 'blocked by policy' }`. Return `'not-applicable'`
for the normal execution path. For decisions that need the full `toolCall` or
