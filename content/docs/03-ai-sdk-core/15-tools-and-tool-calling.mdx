...
- `context`: The runtime context passed via the `context` setting.
...
When using both static and dynamic tools, use the `dynamic` and `invalid` flags for type narrowing:
  ...
  onStepFinish: ({ toolCalls, toolResults }) => {
    ...
    for (const toolCall of toolCalls) {
      if (toolCall.invalid) {
        ...
        console.log('Invalid:', toolCall.toolName, toolCall.error);
        continue;
      }
...
You can pass in arbitrary runtime context from `generateText` or `streamText` via the `context` setting.
This runtime context is available in `prepareStep` and in the `context` tool execution option.

To avoid confusion with prompt context or retrieved context, the docs refer to this feature as runtime context.

This is useful for values like API keys, tenant information, feature flags, session data, or other server-side state that should influence tool execution without being embedded into the prompt.

At a high level:

- Pass the full runtime context object through `context`
- Read or update it in `prepareStep`
- Declare each tool's expected runtime context with `contextSchema`
- Access the tool's typed runtime context in `execute` together with other execution metadata such as `toolCallId`, `messages`, and `abortSignal`
...
const result = streamText({
  ...
  prepareStep: async ({ context }) => {
    ...
    return {
      // You can keep the context unchanged or return a new one
...
The `weather` tool then receives a typed view of the runtime context based on its `contextSchema`. In this case, `execute` can access `weatherApiKey`, while `prepareStep` still sees the full runtime context object.
...
When the model calls a tool that doesn't exist ([`NoSuchToolError`](/docs/reference/ai-sdk-errors/ai-no-such-tool-error)) or provides input that doesn't match the tool's schema ([`InvalidToolInputError`](/docs/reference/ai-sdk-errors/ai-invalid-tool-input-error)), the AI SDK captures these as `InvalidToolCall` objects instead of throwing errors. In multi-step scenarios, error feedback is automatically sent back to the model, giving it a chance to self-correct.

You can access invalid tool calls via the `invalidToolCalls` property on the result or on individual steps:
...
for (const invalidCall of result.invalidToolCalls) {
  console.log('Invalid tool call:', invalidCall.toolName);
  console.log('Error:', invalidCall.error);
  console.log('Raw input:', invalidCall.rawInput);
}

// or check across all steps
const allInvalidCalls = result.steps.flatMap(step => step.invalidToolCalls);
```

If a [`ToolCallRepairError`](/docs/reference/ai-sdk-errors/ai-tool-call-repair-error) occurs during an attempt to repair an invalid tool call, it is also captured as an `InvalidToolCall`.
