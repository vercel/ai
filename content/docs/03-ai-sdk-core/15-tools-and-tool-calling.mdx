...
The tools in `streamText` and `generateText` are defined as a `Record<string, Tool>`.
The type inference helpers `ToolCallUnion<TOOLS extends Record<string, Tool>>`
and `ToolResultUnion<TOOLS extends Record<string, Tool>>` can be used to
