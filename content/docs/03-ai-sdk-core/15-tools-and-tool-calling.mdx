...
`toolApproval` accepts one of four statuses per tool, either as a string or as
...
`toolApproval` can also be a function per tool, which lets you return one of
these string or object statuses dynamically based on the tool input and runtime
options such as `toolCallId`, `messages`, and `toolContext`.
...
You can use a similar function in `toolApproval` when you want to return
