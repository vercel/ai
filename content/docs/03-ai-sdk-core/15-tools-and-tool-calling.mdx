...
When a tool requires approval, `generateText` and `streamText` don't pause execution. Instead, they complete and return `tool-approval-request` parts in the result content. Your application collects user decisions, adds approval responses to the messages array, and calls the model again.
