...
<Note>When a model uses a tool, it is called a "tool call" and the output of the tool is called a "tool result".</Note>
...
<Note>You can use the `tool` helper function to infer the types of the `execute` parameters.</Note>
