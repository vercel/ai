...
When an approved tool executes, its `execute` function receives the decision as
`options.approval`, including the approval ID and optional reason. This lets the
tool act on context collected by the approval UI, such as a corrected value.
Denied tool calls do not execute; their reason is included in the
...
const runCommand = tool({
  ...
  execute: async ({ command }, { approval }) => {
    return await run(command, { approvalNote: approval?.reason });
  },
});
```
