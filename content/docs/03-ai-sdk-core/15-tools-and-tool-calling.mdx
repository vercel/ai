...
Create an MCP client using either `stdio` (for local servers) or `SSE` (for web servers):
...
```typescript
// Use schemas provided by the server
...
Below is a complete example of using MCP tools with the AI SDK.
