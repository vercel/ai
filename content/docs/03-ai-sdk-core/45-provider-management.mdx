...
You can target an OpenAI-compatible proxy or regional gateway by setting the
`OPENAI_BASE_URL` environment variable (or by passing the `baseURL` option when
calling `createOpenAI`). Requests default to `https://api.openai.com/v1` when
neither override is provided.
