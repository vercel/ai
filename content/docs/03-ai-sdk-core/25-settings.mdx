...
When omitted, the provider's default behavior is used. Each provider maps these values to its native reasoning mechanism. Providers that do not support reasoning will emit a warning and ignore the parameter.
