...
Model call options (`ModelCallOptions`) are settings that influence how the model generates its response — token limits, sampling behavior, penalties, stop sequences, seed, and reasoning. They are forwarded to the underlying model.
