...
- `chunkMs`: The timeout between content-bearing output chunks after output has started (streaming only). Non-content chunks do not reset it, and the timeout stops when the model response ends before client-side tools execute. This is useful for detecting streams that stall after generation begins.
