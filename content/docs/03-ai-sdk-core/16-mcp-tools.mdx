...
MCP tool annotations are preserved in `tool.metadata.annotations`. Tools with
`destructiveHint: true` or `readOnlyHint: false` require user approval by
default. MCP annotations are server-provided hints rather than a security
boundary, so configure `toolApproval` explicitly when connecting to an
untrusted server or when every discovered tool must be covered by a policy.
