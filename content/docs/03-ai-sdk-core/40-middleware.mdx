...
The [aiact-audit-log](https://github.com/systima-ai/aiact-audit-log) middleware provides structured, tamper-evident audit logging for AI systems targeting EU AI Act Article 12 compliance. It automatically captures every `generateText`, `streamText`, and `generateObject` call with schema fields mapped to specific Article 12 paragraphs, SHA-256 hash chains for tamper evidence, and configurable storage (S3-compatible or local filesystem).

The `@systima/aiact-audit-log` package wraps any AI SDK model:
...
const result = await generateText({
  model,
  prompt: 'Assess the risk for application #12345',
});
```

The middleware logs all AI SDK calls automatically. Human interventions, session lifecycle events, and custom business logic events require manual `logger.log()` calls. See the [aiact-audit-log documentation](https://github.com/systima-ai/aiact-audit-log) for the full API reference and CLI tooling.
