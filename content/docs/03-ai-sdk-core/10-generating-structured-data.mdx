...
<Note>
  When using the OpenAI provider, strict JSON schema enforcement now defaults to
  on when a schema is provided or tools are present. You can opt out per-request
  with `providerOptions.openai.strictJsonSchema = false`.
</Note>
