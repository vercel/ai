...
For discovery to remain incremental across separate generation calls, retain
the request messages containing the generated catalog along with the response:

```ts
const messages = [
  ...result.finalStep.request.messages!,
  ...result.responseMessages,
];
```

Pass `messages` plus the next user message to the next generation call. If the
previous catalog is not present, the SDK safely starts with a complete catalog.
