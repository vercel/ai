...
their definitions change later, the SDK adds only the added, updated, and
removed tools. It adds no message when the catalog is unchanged.

For discovery to remain incremental across separate generation calls, append
the continuation messages to your input history:

```ts
messages.push(...result.experimental_continuationMessages);
```

The continuation contains any catalog messages added by the SDK as well as the
assistant and tool response messages. Pass `messages` plus the next user message
to the next generation call. If the previous catalog is not present, the SDK
safely starts with a complete catalog.
