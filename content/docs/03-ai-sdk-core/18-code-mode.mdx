...
## Creating a Code Mode Tool

Define the AI SDK tools that generated code should be able to call, then pass
them to `experimental_createCodeModeTool`:
...
`createCodeModeTool` returns a regular AI SDK tool. You can use it with
`generateText`, `streamText`, or a `ToolLoopAgent`.

The code mode tool description includes TypeScript signatures generated from
the input and output schemas of the tools you provide. Descriptions,
