...
| --------------------------------- | -------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------- |
| `runtimeContext`                  | `generateText`, `streamText`, `embed`, `embedMany`, `rerank`, or `ToolLoopAgent` calls | `prepareStep` and step results for text generation; lifecycle callbacks and telemetry for all supported calls | Shared application and operation state                        |
...
| `telemetry.includeRuntimeContext` | Any supported call with `runtimeContext`                                               | Telemetry filtering                                                                                           | Top-level `runtimeContext` properties to include in telemetry |
...
```txt
generateText / streamText / embed / embedMany / rerank / ToolLoopAgent
  -> runtimeContext
       -> lifecycle callbacks
       -> prepareStep and step results for text generation and agents
...
agent loop, or when embedding and reranking operations need request-local
attribution. It is not added to model inputs automatically. Use it to configure
step preparation, track server-side state, or correlate lifecycle and telemetry
events.
...
For `embed`, `embedMany`, and `rerank`, the context is immutable for the
operation and is available in the `onStart` and `onEnd` lifecycle callbacks.
...
<Note>
  ...
  integrations. Lifecycle callbacks still receive the full `runtimeContext`.
  Text generation and agent execution also retain the full context in their
  existing step results and tool flows.
</Note>
...
| --------------------------------- | ---------------------------------------------- | ------------------------------------------- | -------------------------------------------------------- |
| `prepareStep`                     | Read and update for text generation and agents | Read and update                             | Not directly                                             |
...
| Embedding and reranking callbacks | Read the operation context                     | Not available                               | Not available                                            |
