...
with `telemetry.includeRuntimeContext` to include them in telemetry, and mark top-level
...
| Telemetry | Filtered by `telemetry.includeRuntimeContext` | Filtered per tool | Filtered by each tool's `sensitiveContext` |
...
- Use prompt messages for information the model should reason about or mention
  in its answer.
- Use `telemetry.includeRuntimeContext` and `sensitiveContext` to reduce telemetry
