...
| `includeRuntimeContext` | The generation or agent call | Telemetry filtering | Top-level `runtimeContext` properties to include in telemetry |
...
generateText / streamText / ToolLoopAgent
  -> runtimeContext
       ...
       -> telemetry, filtered by includeRuntimeContext
  -> toolsContext
       -> prepareStep
...
not be sent to telemetry providers. Mark top-level runtime context properties
with `includeRuntimeContext` to include them in telemetry, and mark top-level
tool context properties with `sensitiveContext` to omit them from telemetry.
...
Context telemetry filtering is shallow. For `includeRuntimeContext`, only
top-level properties marked as `true` are sent when it is configured; if it is
omitted, all runtime context properties are sent. For `sensitiveContext`,
top-level properties marked as `true` are omitted.
...
| Telemetry | Filtered by `includeRuntimeContext` | Filtered per tool | Filtered by each tool's `sensitiveContext` |
...
- Use prompt messages for information the model should reason about or mention
  in its answer.
- Use `includeRuntimeContext` and `sensitiveContext` to reduce telemetry
