...
Telemetry integrations let you hook into the generation lifecycle to build custom observability — logging, analytics, DevTools, or any other monitoring system. Instead of wiring up individual callbacks on every call, you implement a `TelemetryIntegration` once and register it globally or pass it via `experimental_telemetry.integrations`.
...
- **`invoke_agent {modelId}`** (root span, `INTERNAL`): covers the full operation including all steps and tool calls.
  ...
  - `gen_ai.input.messages`: the full input (system, prompt, messages) as stringified JSON (when `recordInputs` is enabled)
  ...
  - `gen_ai.ai_sdk.telemetry.function_id`: the telemetry function ID
  - `gen_ai.ai_sdk.telemetry.metadata.*`: custom metadata key-value pairs
  - `gen_ai.ai_sdk.settings.*`: additional model settings (e.g. `maxRetries`)
  ...
  - `gen_ai.ai_sdk.usage.reasoning_tokens`: reasoning tokens used
  - `gen_ai.ai_sdk.usage.total_tokens`: total tokens used
  - `gen_ai.ai_sdk.usage.input_token_details.no_cache_tokens`: non-cached input tokens
  - `gen_ai.ai_sdk.usage.output_token_details.text_tokens`: text output tokens
  - `gen_ai.ai_sdk.usage.output_token_details.reasoning_tokens`: reasoning output tokens
  - `gen_ai.ai_sdk.response.provider_metadata`: provider-specific metadata (stringified JSON)
...
- **`chat {modelId}`** (step span, `CLIENT`): one span per LLM provider call, nested under the root span.
  ...
  - `gen_ai.usage.cache_read.input_tokens`: cached input tokens read
  - `gen_ai.usage.cache_creation.input_tokens`: cached input tokens created
  - `gen_ai.ai_sdk.usage.reasoning_tokens`: reasoning tokens
  - `gen_ai.ai_sdk.usage.total_tokens`: total tokens
  - `gen_ai.ai_sdk.usage.input_token_details.no_cache_tokens`: non-cached input tokens
  - `gen_ai.ai_sdk.usage.output_token_details.text_tokens`: text output tokens
  - `gen_ai.ai_sdk.usage.output_token_details.reasoning_tokens`: reasoning output tokens
  - `gen_ai.ai_sdk.response.timestamp`: the response timestamp (ISO 8601)
  - `gen_ai.ai_sdk.response.provider_metadata`: provider-specific metadata (stringified JSON)
  - `gen_ai.output.messages`: the output in [GenAI SemConv message format](#genai-message-format) (when `recordOutputs` is enabled)

  For `streamText`, the step span also records events:
  - `gen_ai.ai_sdk.stream.first_chunk` (event): emitted when the first chunk is received, with `gen_ai.ai_sdk.stream.ms_to_first_chunk`
  - `gen_ai.ai_sdk.stream.finish` (event): emitted when the stream finishes
...
- `gen_ai.ai_sdk.schema`: the output schema (stringified JSON, when `recordInputs` is enabled)
- `gen_ai.ai_sdk.schema.name`: the schema name
- `gen_ai.ai_sdk.schema.description`: the schema description
- `gen_ai.ai_sdk.settings.output`: the output mode
...
- **`embeddings {modelId}`** (root span): covers the full embedding operation.
  ...
  - `gen_ai.request.model`: the requested model ID
  - `gen_ai.ai_sdk.telemetry.function_id`: the telemetry function ID
  - `gen_ai.ai_sdk.telemetry.metadata.*`: custom metadata
  - `gen_ai.ai_sdk.settings.*`: additional settings
  - `gen_ai.ai_sdk.value`: the input value (for `embed`, when `recordInputs` is enabled)
  - `gen_ai.ai_sdk.values`: the input values array (for `embedMany`, when `recordInputs` is enabled)

  Attributes set on finish:
  - `gen_ai.usage.input_tokens`: the number of tokens used
  - `gen_ai.ai_sdk.embedding`: the embedding result (for `embed`, when `recordOutputs` is enabled)
  - `gen_ai.ai_sdk.embeddings`: the embeddings array (for `embedMany`, when `recordOutputs` is enabled)
...
- **`embeddings {modelId}`** (inner span, `embedMany` only): one span per provider batch call, nested under the root span.
  ...
  - `gen_ai.ai_sdk.values`: the batch values (when `recordInputs` is enabled)

  Attributes set on finish:
  - `gen_ai.ai_sdk.embeddings`: the batch embeddings (when `recordOutputs` is enabled)
  - `gen_ai.usage.input_tokens`: the number of tokens used
...
- **`rerank {modelId}`** (root span): covers the full rerank operation.
  ...
  - `gen_ai.ai_sdk.telemetry.metadata.*`: custom metadata
  - `gen_ai.ai_sdk.settings.*`: additional settings
  - `gen_ai.ai_sdk.documents`: the documents to rerank (when `recordInputs` is enabled)
...
- **`rerank {modelId}`** (inner span): one span per provider rerank call, nested under the root span.
  ...
  - `gen_ai.provider.name`: the provider
  - `gen_ai.request.model`: the model ID
  - `gen_ai.ai_sdk.documents`: the documents (when `recordInputs` is enabled)

  Attributes set on finish:
  - `gen_ai.ai_sdk.ranking.type`: the document type
  - `gen_ai.ai_sdk.ranking`: the ranking results (when `recordOutputs` is enabled)
...
- `gen_ai.tool.call.arguments`: the input arguments (stringified JSON, when `recordOutputs` is enabled)
