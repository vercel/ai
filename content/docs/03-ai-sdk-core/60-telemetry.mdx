...
- `ai.usage.inputTokens`: the number of input (prompt) tokens that were used
- `ai.usage.outputTokens`: the number of output (completion) tokens that were used
- `ai.usage.totalTokens`: the total number of tokens (input + output)
- `ai.usage.reasoningTokens`: the number of reasoning/thinking tokens (when the model reports them, e.g. extended thinking)
- `ai.usage.cachedInputTokens`: the number of input tokens served from cache (when the provider reports cache reads)
- `ai.usage.promptTokens`: same as `ai.usage.inputTokens`
- `ai.usage.completionTokens`: same as `ai.usage.outputTokens`
