...
OpenTelemetry span collection requires the `@ai-sdk/otel` package. Install it and register the integration at the top level of any file where you use the AI SDK:
...
For Next.js applications, place this at the top of your API route files (not in `instrumentation.ts`). Your `instrumentation.ts` should only contain the OpenTelemetry provider setup (e.g. `registerOTel` or `NodeTracerProvider`). See the [Next.js OpenTelemetry guide](https://nextjs.org/docs/app/building-your-application/optimizing/open-telemetry) for more details on setting up the provider.
...
The `OpenTelemetryIntegration` from `@ai-sdk/otel` is the primary integration for collecting OpenTelemetry spans (see [Enabling telemetry](#enabling-telemetry) above).
