...
OpenTelemetry span collection requires the `@ai-sdk/otel` package. Install it and register the integration once at application startup, before any AI SDK calls:
...
For Next.js applications, place this in your `instrumentation.ts` file alongside your OpenTelemetry provider setup. See the [Next.js OpenTelemetry guide](https://nextjs.org/docs/app/building-your-application/optimizing/open-telemetry) for more details.
