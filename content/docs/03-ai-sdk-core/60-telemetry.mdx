...
When you use a `LanguageModelV2Middleware`, additional spans may be recorded.

- `ai.languageModelMiddleware.transformParams` (span): This span is recorded when a middleware's `transformParams` function is executed. It wraps the transformation, showing how the parameters are modified before being sent to the model.
  It contains the [basic LLM span information](#basic-llm-span-information) and the following attributes:

  - `operation.name`: `ai.languageModelMiddleware.transformParams` and the functionId that was set through `telemetry.functionId`
  - `ai.operationId`: `"ai.languageModelMiddleware.transformParams"`
  - `ai.prompt.messages`: The messages that were passed into the middleware.
  - `ai.prompt.messages.transformed`: The messages after they have been transformed by the middleware.
...
`ai.generateObject`, `ai.generateObject.doGenerate`, `ai.streamObject`, `ai.streamObject.doStream`, `ai.languageModelMiddleware.transformParams`) contain the following attributes:
