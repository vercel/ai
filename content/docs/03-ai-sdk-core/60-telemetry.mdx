...
<PropertiesTable
  content={[
    ...
    {
      name: 'onEvaluateStart',
      type: '(event: Experimental_EvaluationModelCallStartEvent) => void | PromiseLike<void>',
      description: 'Called when an individual evaluation model call begins.',
    },
    {
      name: 'onEvaluateEnd',
      type: '(event: Experimental_EvaluationModelCallEndEvent) => void | PromiseLike<void>',
      description: 'Called when an individual evaluation model call completes.',
    },
    {
      name: 'onEnd',
...
usage is recorded on the child span.

Evaluation state, questions, and answers do not have GenAI semantic-convention
attributes. Enable the `evaluation` supplemental option to record them under
AI SDK-specific `ai.evaluation.*` attributes.
...
- `spanType`: the type of span being created (`operation`, `step`, `languageModel`, `tool`, `embedding`, `reranking`, or `evaluation`).
...
- `evaluation`: evaluation state, questions, and answers.
