...
In this example, telemetry integrations receive `runtimeContext` as `{ requestId: 'req_abc' }`. Properties set to `false` or omitted are excluded. If `includeRuntimeContext` is omitted, no runtime context properties are included. `includeRuntimeContext` is supported by `generateText`, `streamText`, and `ToolLoopAgent`.
