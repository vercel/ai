...
that you can use to add custom headers to the video generation request. The same
headers are also forwarded when the SDK downloads URL-type videos, which is
required for authenticated provider media URLs (for example OpenRouter
`/videos/{id}/content`).
