...
const { video } = await generateVideo({
  ...
  poll: {
    ...
    backoff: 'exponential', // Double interval after each poll, capped at 60s
...
<Note>
  For production use, consider using `backoff: 'exponential'` to reduce
  unnecessary API calls while waiting for longer generations.
</Note>
