...
- `providerMetadata`: Combined provider-specific metadata from generation and status calls.
- `cause`: The cause of the error. You can use this for more detailed error handling

The same typed error is used when asynchronous video generation times out, a
provider reports a terminal failure, or a webhook notification is followed by
