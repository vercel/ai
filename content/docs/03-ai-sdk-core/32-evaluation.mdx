...
Language-model adapters request `reasoning: 'none'` by default. OpenAI, Anthropic,
and Google disable thinking where the model supports it, otherwise they request
the lowest supported reasoning effort or budget. Evaluations can therefore still
consume reasoning tokens. See [reasoning](/docs/ai-sdk-core/reasoning) for model-family
fallbacks and custom model limitations. To enable reasoning for more demanding
