...
`typeSafeAi.evaluationModel('jev-latest')` or
`openai.evaluationModel('gpt-5.6-luna')`.

TypeSafe AI supplies native Choice, Score, and Boolean evaluations. OpenAI adapts
structured language-model output for Choice, Score, and Boolean. Boolean answers
contain prompted estimates of P(true), validated to be finite and in `[0, 1]`.
These estimates are not guaranteed to be calibrated. Choice and Score answers
do not include probability distributions. Select a model that supports the provider's structured-output API; judge quality against your
