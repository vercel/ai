...
description: Subscribe to lifecycle events in generateText, streamText, and rerank calls
---

# Event Callbacks

The AI SDK provides per-call event callbacks that you can pass to `generateText`, `streamText`, and `rerank` to observe lifecycle events. This is useful for building observability tools, logging systems, analytics, and debugging utilities.

## Basic Usage

Pass callbacks directly to `generateText`, `streamText`, or `rerank`:
...
Errors thrown inside callbacks are caught and do not break the generation or reranking flow. This ensures that monitoring code cannot disrupt your application:
