...
<PropertiesTable
  content={[
    ...
    },
    {
      ...
      type: '(event: ModelCallStartEvent) => void | Promise<void>',
      description:
    ...
    },
    {
      ...
      type: '(event: ModelCallEndEvent) => void | Promise<void>',
      description:
...
`experimental_onStepStart` -> `experimental_onModelCallStart` ->
`experimental_onModelCallEnd` -> tool execution callbacks -> `onStepFinish`.
...
const result = await generateText({
  model: openai('gpt-4o'),
  prompt: 'Hello!',
  experimental_onModelCallStart: event => {
...
const result = await generateText({
  ...
  experimental_onModelCallEnd: event => {
...
<PropertiesTable
  content={[
    ...
    {
      name: 'toolCalls',
      type: 'ReadonlyArray<TypedToolCall>',
