...
- `outputTokensPerSecond`: For streaming steps, output tokens per second after the first output token, calculated as `outputTokens / outputStreamSeconds`. For `generateText`, this is `undefined`.
- `inputTokensPerSecond`: For streaming steps, input tokens per second before the first output token, calculated as `inputTokens / ttftSeconds`. For `generateText`, this is `undefined`.
...
- `timeToFirstOutputTokenMs`: For streaming steps, the time until the first text, reasoning, or tool input delta was received in milliseconds. For `generateText`, this is `undefined`.
- `timeBetweenOutputTokensMs`: For streaming steps with at least two output-token chunks, timing statistics for the gaps between streamed output tokens in milliseconds.
...
For `streamText`, `timeToFirstOutputTokenMs` is set when the first text, reasoning, or tool input delta is received for a step. `timeBetweenOutputTokensMs` includes `min`, `p10`, `median`, `avg`, `p90`, and `max` when at least two output-token chunks are received.
