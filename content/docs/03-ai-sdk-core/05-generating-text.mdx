...
- `tokensPerSecond`: Average output tokens generated per second, calculated from `responseTimeMs`.
...
- `maxToolExecutionTimeMs`: Maximum time spent executing a single client-side tool in the step in milliseconds.
- `timeToFirstTokenMs`: For streaming steps, the time until the first text, reasoning, or tool input delta was received in milliseconds. For `generateText`, this is `undefined`.
...
For `streamText`, `timeToFirstTokenMs` is set when the first text, reasoning, or tool input delta is received for a step.
