...
- `timeBetweenOutputTokens`: For streaming steps with at least two output-token chunks, timing statistics for the gaps between streamed output tokens in milliseconds.
...
For `streamText`, `timeToFirstOutputTokenMs` is set when the first text, reasoning, or tool input delta is received for a step. `timeBetweenOutputTokens` includes `min`, `p10`, `median`, `avg`, `p90`, and `max` when at least two output-token chunks are received.
