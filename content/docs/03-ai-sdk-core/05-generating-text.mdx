...
- `result.content`: The content that was generated in the final step.
- `result.text`: The generated text from the final step.
- `result.files`: The files that were generated in the final step.
- `result.sources`: Sources that have been used as references in the last step (only available for some models).
- `result.toolCalls`: The tool calls that were made in the final step.
- `result.toolResults`: The results of the tool calls from the final step.
...
- `result.finalStep`: Details for the final step.
...
- `result.text`: The generated text from the final step.
- `result.finalStep`: Details for the final step.
- `result.files`: Files that have been generated by the model in the final step.
- `result.sources`: Sources that have been used as references in the last step (only available for some models).
- `result.toolCalls`: The tool calls that have been executed in the final step.
- `result.toolResults`: The tool results that have been generated in the final step.
- `result.finishReason`: The reason the model finished generating text.
- `result.rawFinishReason`: The raw reason why the generation finished (from the provider).
- `result.usage`: The total usage across all steps (for multi-step generations).
