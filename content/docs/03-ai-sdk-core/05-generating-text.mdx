...
const result = await generateText({
  ...
  experimental_onStart({ model }) {
    console.log('Generation started', { model });
  },
  ...
  experimental_onModelCallStart({ modelId, messages }) {
    console.log('Model call starting', { modelId, messageCount: messages.length });
  },

  experimental_onModelCallEnd({ modelId, finishReason, toolCalls }) {
...
- **`experimental_onModelCallStart`**: Called immediately before the provider model call begins. Useful when you want to observe the model invocation separately from later tool execution.
- **`experimental_onModelCallEnd`**: Called after the model response has been normalized and parsed, but before any client-side tool execution begins. Receives the model-call text, reasoning, files, usage, and parsed tool calls.
