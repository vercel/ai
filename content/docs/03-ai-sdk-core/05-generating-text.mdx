...
const result = await generateText({
  ...
  experimental_onStepStart({ steps, modelId, messages }) {
    console.log(`Step ${steps.length} starting`, { modelId });
  },
...
- **`experimental_onStepStart`**: Called before each step (LLM call). Receives the model, messages being sent, tools, and prior steps (use `steps.length` for the step index).
