...
<Note>
  Streaming is currently supported in a simulated capacity.
</Note>
...
<Note>
  Due to o1's current limitations, real-time streaming is not supported. With the
  `simulateStreaming` option, the response will be sent once the model completes
  its reasoning and generates the full output. The latest version of `o1-mini`
  supports streaming.
</Note>
