...
2. [**o1-mini**](https://platform.openai.com/docs/models#o1): A faster and cheaper version of o1, particularly adept at coding, math, and science tasks where extensive general knowledge isn't required.
...
<Note>Streaming is currently supported in a simulated capacity.</Note>
...
export async function POST(req: Request) {
  ...
  const result = streamText({
    model: openai('o1', { simulateStreaming: true }),
    messages,
  });
...
<Note>
  Due to o1's current limitations, real-time streaming is not supported. With
  the `simulateStreaming` option, the response will be sent once the model
  completes its reasoning and generates the full output. The latest version of
  `o1-mini` supports streaming.
</Note>
