...
The release of Meta's Llama 3.1 marks a watershed moment in the AI landscape. As the first state-of-the-art open weight AI model, Llama 3.1 is set to reshape how developers build and deploy AI-powered applications. Available in 8B, 70B, and 405B sizes, these instruction-tuned models excel at a wide range of tasks, including dialogue generation, translation, reasoning, and code generation.
...
With the release of Llama 3.1, Meta introduced a new model with 405B parameters, becoming the largest open-source model available today. This model is designed to handle the most complex and demanding tasks, such as large-scale data processing, code generation, and multi-step reasoning.
...
At the center of the Vercel AI SDK is [AI SDK Core](/docs/ai-sdk-core/overview), which provides a unified API to call any LLM. The code snippet below is all you need to call Llama 3.1 (using [Groq](https://groq.com)) with the Vercel AI SDK:
...
<Note>
  Llama 3.1 is available to use via a number of AI SDK providers including
...
While text generation can be useful, your usecase will likely call for generating structured data. For example, you might want to extract information from text, classify data, or generate synthetic data. AI SDK Core provides two functions ([`generateObject`](/docs/reference/ai-sdk-core/generate-object) and [`streamObject`](/docs/reference/ai-sdk-core/stream-object)) to generate structured data, allowing you to constrain model outputs to a specific schema.
...
This code snippet will generate a type-safe recipe that conforms to the specified schema!

### Tools

While LLMs have incredible generation capabilities, they struggle with discrete tasks (e.g. mathematics) and interacting with the outside world (e.g. getting the weather). The solution: tools, which are like programs that you provide to the model, which it can choose to call as and when it deems necessary.

### Using Tools with the Vercel AI SDK

The Vercel AI SDK supports the tool usage across several of its functions, including [`generateText`](/docs/reference/ai-sdk-core/generate-text) and [`streamUI`](/docs/reference/ai-sdk-rsc/stream-ui). By passing one or more tools to the `tools` parameter, you can extend the capabilities of LLMs, allowing them to perform discrete tasks and interact with external systems.
