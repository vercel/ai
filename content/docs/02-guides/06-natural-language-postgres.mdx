...
description: Learn how to build a Next.js app that uses AI to interact with a PostgreSQL database using natural language.
---
...
The application will use AI to:
...
To focus on the AI-powered functionality, rather than project setup and configuration, we've prepared a starter repository. This includes a database schema and a few components to help you concentrate on building the AI features.

Clone the starter repository:

<Snippet
  text={[
    'git clone https://github.com/vercel/ai-sql-starter',
    'cd ai-sql-starter',
...
Let's set up the project and load the dataset:
...
You should see a message indicating that the database has been seeded successfully.
...
The starter repository already includes all the components that you will need, including:
...
The application contains a single page in `app/page.tsx` that serves as the main interface. Here are the key components you'll work with:
...
All the necessary components are included in the starter repository. Let's implement the AI-powered functionality to bring it all together.
...
const handleSubmit = async (suggestion?: string) => {
  ...
  try {
    const query = generateQuery(question);
    ...
    // TODO: Generate chart config & update chart config state with result
  } catch (e) {
    toast.error('An error occurred. Please try again.');
    setLoading(false);
  }
...
<Note>
  Don't forget to import the `explanationsSchema` from the `types.ts` file.
</Note>

This action uses the `generateObject` function again. However, you haven't defined the schema yet. Let's define it in another file so it can also be used by your components to render the generation.
...
Next, update the `query-viewer.tsx` component to display these explanations. The `handleExplainQuery` function is called every time the user clicks the question icon button on the right side of the query. Let's update this function to the new `explainQuery` action and updates the component state with the explanations:
...
3. The model will generation an array of explanations
4. The explanations will be set in the component state and rendered in the UI

Try submitting a query and clicking the explanation button. Now try hovering over different elements of the query. You should see the explanations for each section!

### Generate Chart Visualizations

Finally, let's try and render the query results in a chart. There are two approaches you could take:

1. Send both the query and data to the model and ask it to return the data in a visualization-ready format. While this provides complete control over the visualization, it requires the model to send back all of the data, which increases latency and costs.

2. Send the query and data to the model and ask it to generate a chart configuration (fixed-size and not many tokens) that maps your data appropriately. This configuration specifies how to visualize the information while delivering the insights from your natural language query, without requiring the model to process and return the full dataset.
...
Create the chart generation action in `app/actions.ts`:
...
With the chart generation action in place, you'll want to trigger it automatically after receiving query results. This ensures the visualization appears almost immediately after data loads
...
You've built an AI-powered SQL analysis tool that can:
...
Consider extending the application to use your own data sources, or adding more advanced features like:

- Customizing the chart configuration schema to support more chart types and options
- Adding more complex SQL query generation capabilities
