...
- Slack utility functions for request validation and message formatting (`lib/slack-utils.ts`)
- API endpoints to handle Slack events (`api/events.ts`)
- Event handlers for different message types (`lib/handle-messages.ts` and `lib/handle-app-mention.ts`)
