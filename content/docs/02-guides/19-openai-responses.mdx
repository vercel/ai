...
The Responses API includes a built-in image generation tool that allows you to create images using OpenAI's image models. Here's how to use it:
...
const result = await generateText({
  model: openai.responses('gpt-4o'),
  prompt: 'Generate an image of a sunset over a calm ocean',
  tools: {
...
// The generated image will be available in the response
console.log(result.files);
```

The image generation tool supports various configuration options:

- `model`: The image generation model to use (e.g., 'gpt-image-1')
- `size`: The dimensions of the generated image (e.g., '1024x1024', '1024x1536', '1536x1024', 'auto')
- `quality`: The quality of the generated image ('low', 'medium', 'high', 'auto')
- `output_format`: The format of the output image ('png', 'webp', 'jpeg')
- `background`: The background type ('transparent', 'opaque', 'auto')

You can also specify the tool as a required choice for the model:
...
The generated images will be returned in the response's `files` array, containing the image data and metadata.
