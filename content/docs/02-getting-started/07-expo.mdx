...
<div className="my-4">
  <Tabs items={['pnpm', 'npm', 'yarn']}>
    <Tab>
      <Snippet text="pnpm add ai@alpha @ai-sdk/openai@alpha @ai-sdk/react@alpha zod" dark />
    </Tab>
    <Tab>
      <Snippet text="npm install ai@alpha @ai-sdk/openai@alpha @ai-sdk/react@alpha zod" dark />
    </Tab>
    <Tab>
      <Snippet text="yarn add ai@alpha @ai-sdk/openai@alpha @ai-sdk/react@alpha zod" dark />
    </Tab>
  </Tabs>
</div>
...
To solve this, you can enable multi-step tool calls using `stopWhen` in your API route. This feature will automatically send tool results back to the model to trigger an additional generation until the stopping condition you define is met. In this case, you want the model to answer your question using the results from the weather tool.

### Update Your API Route

Modify your `app/api/chat+api.ts` file to include `stopWhen`:

```tsx filename="app/api/chat+api.ts" highlight="2,11"
import { openai } from '@ai-sdk/openai';
