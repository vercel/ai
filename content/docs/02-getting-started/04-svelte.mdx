...
<Note type="secondary" fill>Make sure you are using `ai` version 3.1 or higher.</Note>
...
1. First, you create an OpenAI provider instance with the `createOpenAI` function from the `@ai-sdk/openai` package.
2. Next, you define an `POST` request and extract `messages` from the body of the request. The `messages` variable contains a history of the conversation with you and the chatbot and will provide the chatbot with the necessary context to make the next generation.
