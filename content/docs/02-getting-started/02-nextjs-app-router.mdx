...
For example, if a user asks about the current weather, without tools, the AI would only be able to provide general information based on its training data. But with a weather tool, it can fetch and provide up-to-date, location-specific weather information.
...
2. You define a `tools` object with a `weather` tool. This tool:
   ...
   - Has a description that helps the AI understand when to use it.
   ...
   Now your chatbot can "fetch" weather information for any location the user asks about. When the AI determines it needs to use the weather tool, it will generate a tool call with the necessary parameters. The `execute` function will then be automatically run, and you can access the results via the `toolInvocations` key that is available on the message object.

Try asking something like "What's the weather in New York?" and see how the AI uses the new tool.
...
You may have noticed that while we can now see the tool results in our chat interface, the AI isn't necessarily using this information to answer our original query. This is because once the model generates a tool call, it has techincally completed its generation.

To solve this, we can enable multi-step tool calls using the `maxSteps` option in our `useChat` hook. This feature will automatically send tool results back to the model to trigger an additional generation. In this case, we want the AI to answer the user's question using the results from the weather tool.
...
By setting `maxSteps` to 5, you're allowing the model to use up to 5 "steps" for any given generation. This enables more complex interactions and allows the AI to gather and process information over several steps if needed. We can see this in action by adding another tool to convert the temperature from Fahrenheit to Celsius.
...
This simple example demonstrates how tools can expand your model's capabilities. You can create more complex tools to integrate with real APIs, databases, or any other external systems, allowing the model to access and process real-world data in real-time. Tools bridge the gap between the model's knowledge cutoff and current information, making your chatbot more versatile and informative.

## Where to Next?

You've built an AI chatbot using the AI SDK! From here, you have several paths to explore:

- If you're interested in building an AI-powered knowledge base, check out our [RAG (Retrieval-Augmented Generation) guide](/docs/guides/rag).
- For quick starts on various AI applications, explore our [templates](https://vercel.com/templates?type=ai) to jumpstart your next project.
- To learn more about the AI SDK, check out our [documentation](/docs).
