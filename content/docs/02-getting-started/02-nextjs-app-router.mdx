...
To solve this, you can enable multi-step tool calls using the `maxSteps` option in your `useChat` hook. This feature will automatically send tool results back to the model to trigger an additional generation. In this case, you want the model to answer the your question using the results from the weather tool.
...
By setting `maxSteps` to 5, you're allowing the model to use up to 5 "steps" for any given generation. This enables more complex interactions and allows the AI to gather and process information over several steps if needed. You can see this in action by adding another tool to convert the temperature from Fahrenheit to Celsius.
...
- If you're interested in building an AI-powered knowledge base, check out the [RAG (Retrieval-Augmented Generation) guide](/docs/guides/rag).
- For quick starts on various AI applications, explore [templates](https://vercel.com/templates?type=ai) to jumpstart your next project.
- To learn more about the AI SDK, check out the [documentation](/docs).
