...
- **Re-mounting Issue During Streaming**: when using `createStreamableUI`, there is a noticeable flash that occurs when components re-mount on `.done()`, causing undesirable behavior.
- **Human-in-the-Loop Roundtrips**: currently there is no support for human-in-the-loop chatbot roundtrips with AI SDK RSC.
