...
<Note type="warning">
  ...
  bug reports on GitHub. Your reports directly contribute to making the final
  release more stable and reliable.
</Note>
...
AI SDK 5 represents a complete redesign of the AI SDK's protocol and architecture. Drawing on lessons from two years of real-world usage, we've rebuilt the foundation to accommodate the rapidly evolving LLM landscape. The new architecture prepares for future multi-modal capabilities, supporting both input and output beyond just text. We've also modernized the UI and protocols that have remained largely unchanged since AI SDK 2/3, creating a more robust platform that enables quick and reliable support for emerging AI provider features.
...
We've introduced a new stream writer for `createUIMessageStream` (previously known as `createDataStream`) that makes it simple to stream parts from the server to the client. This enables many use-cases like streaming custom sources form your RAG pipeline or sending custom reasoning summaries.
...
const stream = createUIMessageStream({
  execute: writer => {
    ...
    // stream custom reasoning
    writer.write({
      type: 'reasoning',
      value: { text: 'This is a summary of the reasoning...' },
    });
  },
});
```
...
[PLACEHOLDER ADD LARS XEET DIAGRAM]
...
const result = await generateText({
  // ...
  prepareStep: async ({ model, stepNumber, maxSteps, steps }) => {
    if (stepNumber === 0) {
      return {
...
- Hitting a token usage threshold
...
const result = streamText({
  // ...
  // stop loop at 20000 tokens or your own custom condition
  continueUntil: [maxTotalTokens(20000), yourOwnCondition],
});
```
