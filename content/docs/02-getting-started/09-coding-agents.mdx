...
This page explains how to get the most out of the AI SDK when working inside a coding agent (such as Claude Code, Cursor, Windsurf, or any other AI-assisted development environment).
...
This adds the skill to your project's `.agents/skills` directory. From there, any agent that supports the [Agent Skills](https://agentskills.io) format will automatically discover it and load it when working on AI SDK tasks.
...
After installation, your agent can reference the bundled documentation and type definitions at paths like:

```
node_modules/ai/README.md         # Overview and quick-start examples
node_modules/ai/dist/index.d.ts   # Full TypeScript types and JSDoc
```

This means your agent can look up accurate API signatures, types, and usage examples directly from the installed package — ensuring it always uses the version of the SDK that's actually installed in your project.
