...
### `generateText` and `streamText` Tool Calls, Tool Results, Files, and Sources Now Include All Steps

The `toolCalls`, `staticToolCalls`, `dynamicToolCalls`, `toolResults`, `staticToolResults`, `dynamicToolResults`, `files`, and `sources` properties on `generateText` and `streamText` results now return values accumulated across every step.
