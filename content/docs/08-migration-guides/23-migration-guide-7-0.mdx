...
### `result.content` Contains Content from All Steps

For `generateText` and `streamText`, `result.content` now contains content
parts from all steps, including files generated before the final step. In AI SDK
6, it only contained content from the final step.

If you need the previous final-step-only behavior, use `result.finalStep.content`.
If you only need the files from the final step, use `result.finalStep.files`.

```tsx filename="AI SDK 6"
const result = await generateText({
  model: yourModel,
  tools,
  prompt: 'Create an artifact, then summarize it.',
});

const finalStepContent = result.content;
const finalStepFiles = result.files;
```
...
const finalStepFiles = result.finalStep.files;
```
...
### `generateText` and `streamText` Tool Calls, Tool Results, Files, Sources, and Warnings Now Include All Steps

The `toolCalls`, `staticToolCalls`, `dynamicToolCalls`, `toolResults`, `staticToolResults`, `dynamicToolResults`, `files`, `sources`, and `warnings` properties on `generateText` and `streamText` results now return values accumulated across every step.

Use `finalStep` to read the previous behavior, which only returned values from the final step:
