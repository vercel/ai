...
OpenTelemetry span collection is no longer built into the `ai` package. To continue receiving OpenTelemetry traces, you must install the new `@ai-sdk/otel` package and register the `OpenTelemetryIntegration` globally before any AI SDK calls.
...
```tsx filename="instrumentation.ts"
...
This applies to all AI SDK functions that accept `experimental_telemetry`, including `generateText`, `streamText`, `embed`, `embedMany`, and `rerank`.
