...
### `generateText` and `streamText` `files` Now Includes All Steps

The `files` property on `generateText` and `streamText` results now returns all files generated across every step.

Use `finalStep.files` to read the previous `files` behavior, which only returned files from the final step:
...
For `streamText`, await `finalStep` first when you need final-step-only files:
