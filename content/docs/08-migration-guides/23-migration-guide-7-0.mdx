...
### `generateText` and `streamText` `files` and `sources` Now Include All Steps

The `files` and `sources` properties on `generateText` and `streamText` results now return all files and sources generated across every step.

Use `finalStep.files` and `finalStep.sources` to read the previous behavior, which only returned files and sources from the final step:

```tsx filename="AI SDK 6"
const result = await generateText({
  model: yourModel,
  prompt: 'Generate an image, then revise it.',
  stopWhen: stepCountIs(2),
});
...
For `streamText`, await `finalStep` first when you need final-step-only files or sources:
