...
### Tools: `context` Is Typed via Generic `CONTEXT` and `contextSchema`

Tool callbacks are now generic over a `CONTEXT` type instead of treating `context` as an untyped value. In practice, this means `tool()` can infer the type of `context` from a tool's `contextSchema`.
...
In AI SDK 7, declare the runtime context the tool expects with `contextSchema`. The `execute` callback, approval callback, and tool input lifecycle hooks then receive a typed `context` automatically:
