...
### Streaming: Move `includeRawChunks` to `experimental_include.rawChunks`

The top-level `includeRawChunks` option on `streamText` is deprecated in AI SDK
7. Move it into the `experimental_include` options object as `rawChunks`.
...
The deprecated top-level option continues to work in AI SDK 7, so you can
