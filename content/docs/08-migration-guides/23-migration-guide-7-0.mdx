...
### `generateText` and `streamText` `onFinish` Result Properties Changed

The `onFinish` callback for `generateText`, `streamText`, and agents follows
...
The deprecated top-level aliases on the `onFinish` event continue to work in AI
