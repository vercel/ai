...
```tsx filename="AI SDK 7"
import { toUIMessageChunkStreamResponse } from 'ai';

return toUIMessageChunkStreamResponse({
  stream: result.fullStream,
  generateMessageId,
  originalMessages,
  onFinish,
...
// note to self: use existing `createUiMessageStreamResponse` method
...
```tsx filename="AI SDK 7"
import { pipeTextStreamToUIMessageStreamResponse } from 'ai';
...
const textStream = toTextStream({
  stream: result.stream, // TODO: create separate PR to rename result.fullStream to result.stream
  generateMessageId,
  originalMessages,
  onFinish,
});
