...
Follow this three-phase approach for a safe, zero-downtime migration:
...
While Phase 1 gets you working immediately, migrate your schema within weeks of completing Phase 1. This phase uses a side-by-side migration approach with an equivalent v5 schema:
...
### Phase 3: Optimize Your Schema (Optional)

**Goal:** Further optimize your schema by normalizing message parts into a separate table.

Phase 2 gets you to a v5-compatible schema, but you may want to optimize further for better query performance and flexibility. This phase is optional but recommended for production applications with complex message structures.

**Timeline:** Can be done months after Phase 2, or not at all if your current schema works well.
...
<Note type="warning">
**Adapt Phase 2 and 3 Examples to Your Setup**
...
Phase 2 gets you to a v5-compatible schema that works well for most applications. However, storing message parts as JSONB has some limitations:

- Difficult to query specific part types (e.g., "find all tool calls")
- No database-level type safety for part fields
- Less efficient indexing for part-specific searches
- Harder to analyze message patterns at scale

**Phase 3 normalizes your schema** by moving parts into a separate table with proper columns for each part type. This is optional but recommended for:

- Production applications with complex queries
- Apps that need to analyze message patterns
- Systems requiring efficient part-level indexing
- Teams wanting stronger type safety

### Overview: Normalized Schema Design

Instead of storing parts as JSONB, create a `parts` table with dedicated columns for different part types:

```typescript
export const messages = pgTable('messages', {
  id: varchar().primaryKey(),
  chatId: varchar().references(() => chats.id, { onDelete: 'cascade' }),
  role: varchar().$type<'user' | 'assistant' | 'system'>(),
  createdAt: timestamp().defaultNow(),
});

export const parts = pgTable('parts', {
  id: varchar().primaryKey(),
  messageId: varchar().references(() => messages.id, { onDelete: 'cascade' }),
  type: varchar().$type<MyUIMessage['parts'][number]['type']>(),
...
```sql
-- Find all messages with tool calls
SELECT DISTINCT messages.*
FROM messages
JOIN parts ON parts.messageId = messages.id
WHERE parts.type LIKE 'tool-%';

-- Find messages with specific tool
SELECT messages.*
FROM messages
JOIN parts ON parts.messageId = messages.id
WHERE parts.tool_toolName = 'searchWeb';
```

**Type Safety:**

- Database enforces part structure
...
CREATE INDEX idx_parts_tool_name ON parts(tool_toolName);
CREATE INDEX idx_parts_type ON parts(type);
```

### Migration Strategy

Use the same side-by-side approach from Phase 2:

1. Create normalized `messages_optimized` and `parts_optimized` tables
2. Dual-write to both schemas
3. Migrate existing data with a background job
4. Switch reads to optimized schema
5. Remove dual-write
6. Drop old tables

This phase follows the same pattern as Phase 2, just with a different target schema. Refer to Phase 2 steps for the detailed migration process.
