...
`strictJsonSchema` mode is now disabled by default for OpenAI Responses and Chat language models. The provider options for strict JSON schema have been updated:
...
```tsx filename="AI SDK 4.0"
// strictJsonSchema on by default
...
// Must explicitly enable strictJsonSchema via provider options
const result = await generateObject({
  model: openai('gpt-4.1-2024-08-06'),
  schema: z.object({ name: z.string() }),
  providerOptions: {
