...
<Note type="warning">
  `structuredOutputs` and `strictJsonSchema` control different parts of the
  request. `structuredOutputs` enables the Chat Completions structured-output
  response format, while `strictJsonSchema` controls the `strict` flag for JSON
  Schema validation. If you relied on AI SDK 4's strict schema behavior, set
  `strictJsonSchema: true` in addition to `structuredOutputs: true`.
</Note>
...
const result = await generateObject({
  ...
  providerOptions: {
    openai: {
      ...
      strictJsonSchema: true, // opt back in to the AI SDK 4 strict schema behavior
