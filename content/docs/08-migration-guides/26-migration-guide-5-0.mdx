...
The `structuredOutputs` parameter has been removed from the model instance and is now disabled by default. It can be enabled via provider options.
...
const result = await generateText({
  ...
  providerOptions: {
    openai: {
      structuredOutputs: true, // opt-in via provider options
    } satisfies OpenAIResponsesProviderOptions,
  },
});
```

#### Strict Mode Changes & Compatibility Option Removal

The `compatibility` option has been removed.

Strict mode is now disabled by default for OpenAI Responses and Chat language models. The provider options for strict JSON schema have been updated:

- For chat language models, use the new `strictJsonSchema` provider option
- For response language models, the `strictSchemas` provider option has been renamed to `strictJsonSchema`
...
// Must explicitly enable strict mode via provider options
const result = await generateObject({
  model: openai('gpt-4.1-2024-08-06'),
  schema: z.object({ name: z.string() }),
  providerOptions: {
    openai: {
      strictJsonSchema: true, // opt-in via provider options
    } satisfies OpenAIResponsesProviderOptions,
  },
});
```
