...
In v4, you typically rendered tool invocations using a catch-all `tool-invocation` type. In v5, the **recommended approach is to handle each tool specifically using its typed part name (e.g., `tool-getWeather`)**. However, if you need a catch-all pattern for rendering all tool invocations the same way, you can use the `isStaticToolUIPart` and `getToolName` helper functions as a fallback.
...
```tsx filename="AI SDK 5.0"
import { isStaticToolUIPart, getToolName } from 'ai';
