...
<Note>
  **Important:** Most AI providers only support specific file types as file parts:
  - **OpenAI/Azure OpenAI**: Only `image/*`, `application/pdf`, and `audio/*` are supported
  - **Text files** (`text/plain`, `text/markdown`, etc.) are **not supported** as file parts

For text files, read the content and send as a text part instead:
