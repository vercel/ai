...
The `structuredOutputs` parameter has been removed from the provider instance configuration and renamed to `strictJsonSchema`. It is now disabled by default and can be enabled via provider options.
