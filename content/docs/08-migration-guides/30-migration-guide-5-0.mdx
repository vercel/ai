...
<Note>
  AI SDK 5.0 is a major release that introduces significant improvements in type safety,
  consistency, and developer experience. This guide will help you migrate from AI SDK 4.0 to 5.0.
</Note>
        ...
        case 'tool-getWeatherInformation': {
          switch (part.state) {
            ...
            case 'input-available':
              return <div key={index}>Getting weather for {part.input.city}...</div>;
...
#### Stream Text Response Structure

The generate text result now returns structured text objects instead of plain strings.
...
console.log(result.text.type); // "text"
```

**Codemod:** `replace-generatetext-text-property`

#### Stream Text Delta Changes

Stream text deltas have been restructured.
...
for await (const delta of result.fullStream) {
  switch (delta.type) {
    case 'text': { // Changed from 'text-delta'
      process.stdout.write(delta.text); // Changed from 'textDelta'
      break;
    }
  }
}
...
**Codemod:** `migrate-to-data-stream-protocol-v2`
...
const { messages } = useChat({ streamProtocol: 'data' });
```
...
const { messages } = useChat(); // streamProtocol is now handled automatically
```
...
The deprecated `isLoading` helper has been removed in favor of `state`.
...
The `getUIText` helper has been removed.
...
#### Experimental Wrappers Removal

Experimental prefixes have been removed from stable APIs.
...
#### Active Tools No Longer Experimental

The `experimental_activeTools` option has been promoted and no longer requires the experimental prefix.
...
#### Prepare Step No Longer Experimental
...
await generateText({
  ...
  // Implicitly temperature: 0 (deterministic)
});
```
...
await generateText({
  model: openai('gpt-4'),
  prompt: 'Write a creative story',
  temperature: 0, // Must explicitly set for deterministic results
});
```
...
The `onCompletion` callback has been renamed to `onFinal`.
...
**Codemod:** `replace-oncompletion-with-onfinal`
...
**Key benefits of custom typing:**
- **Auto-completion** for `tool-${toolName}` cases
- **Type safety** for `part.input` and `part.output` properties
- **IntelliSense** shows available tool states and properties
- **Compile-time errors** if you reference non-existent tools or properties
...
**Key advantages of `stopWhen`:**
- **Flexible conditions**: Stop based on step count, tool results, content, etc.
- **Multiple criteria**: Combine different stopping conditions with custom logic
- **Access to all steps**: Make decisions based on complete step history
- **Better control**: More precise than simple numeric limits
- **Built-in helpers**: Use `stepCountIs()`, `hasToolCall()`, or write custom conditions
...
stopWhen: ({ steps }) =>
  steps.length >= 10 ||
  steps.some(step => step.toolCalls?.some(call => call.toolName === 'finalizeTask')) ||
...
| Stream Text Delta Changes | `replace-textdelta-with-text` |
...
<Note>
  Codemods marked as "messages only" provide warnings about complex changes that need manual intervention.
  Changes marked as "N/A" either don't require codemods (handled by TypeScript) or are too complex for automated migration.
</Note>