...
// NOTE: change to 4.3?
// NOTE: need to review titles for clarity and accuracy
...
// note: do we need this for message content parts too?
...
// NOTE: move to core changes (done)
// another idea that ui message changes should go in core
// only things directly related to useChat should be in useChat section
...
// note: section for all message changes?
...
// NOTE: move to core changes
// probably want to show the useChat element here too
// maybe message annotations too?
...
// note: move to UIMessage section?
...
// NOTE:
// 1) think about suggested pattern for maxSteps vs stopWhen
// answer here is main stopping conditions on the server with stopWhen, then have maxSteps > 1 in useChat hook for client side tool-calling
// 2) do a better job at describing stopWhen are the stopping conditions when there are tool results

For core functions like `generateText` and `streamText`, the `maxSteps` parameter has been replaced with `stopWhen`, which provides more flexible control over multi-step execution. Note that `useChat` still supports the `maxSteps` parameter. The `stopWhen` parameter is a condition for stopping the generation when there are tool results in the last step. When the condition is an array, any of the conditions can be met to stop the generation. Note that `useChat` still supports the `maxSteps` parameter
...
const result = await generateText({
  model: openai('gpt-4'),
  messages,
  maxSteps: 5, // Stop after a maxmimum of 5 steps
});
...
const result = await generateText({
  model: openai('gpt-4'),
  messages,
  stopWhen: stepCountIs(5), // Stop at step 5
});
...
// useChat still uses maxSteps
const { messages } = useChat({
  maxSteps: 3, // Still available in useChat
});
```
...
// NOTE: maybe remove allowEmptySubmit?
...
// NOTE: NOT ACCURATE (we're using extract ts type in onChunk - awaiting confirmation)
...
**Codemod:** `migrate-to-ui-message-stream`
...
// note: probably makes more sense to have this in the announcement post -> point there
...
| Stream Protocol v2                                              | `migrate-to-ui-message-stream`                    |
