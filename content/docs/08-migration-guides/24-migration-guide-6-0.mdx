...
#### Responses API `providerOptions` Key Changed from `openai` to `azure`

In AI SDK 5, the Azure Responses integration reused the OpenAI Responses provider options, so you had to pass options under the `openai` key.
In AI SDK 6, when you call the **Responses API** via `azure()` or `azure.responses()`, you must instead pass those options under the `azure` key.

This change only applies to the **Responses API**. Other Azure model types that use the OpenAI-compatible surface (for example, `azure.chat()` or `azure.embedding()`) continue to use the `openai` key for `providerOptions`.
