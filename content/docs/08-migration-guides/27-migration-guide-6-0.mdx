...
The tool strict mode is now controlled by setting `strict` on each tool.

This enables fine-grained control over strict tool calls, which is important since it depends on the tool input schema.
...
The `textEmbeddingModel` and `textEmbedding` methods on providers have been renamed to `embeddingModel` and `embedding` respectively. Additionally, generics have been removed from `EmbeddingModel`, `embed`, and `embedMany`. See [PR #10592](https://github.com/vercel/ai/pull/10592) for more details.
...
#### Unknown Models Treated as Reasoning Models

The `@ai-sdk/openai` provider now treats unknown or custom model IDs as reasoning models by default ([PR #9976](https://github.com/vercel/ai/pull/9976)). Previously, unknown models were treated as non-reasoning models.

This change reflects the trend that most new models from OpenAI support reasoning capabilities. If you're using custom model IDs that don't support reasoning, you may need to adjust your configuration.
...
Anthropic has introduced native structured outputs for Claude Sonnet 4.5 and later models. The `@ai-sdk/anthropic` provider now includes a `structuredOutputMode` option to control how structured outputs are generated ([PR #10502](https://github.com/vercel/ai/pull/10502)).
...
Model-specific warning types have been consolidated into a single generic `Warning` type exported from the `ai` package ([PR #10631](https://github.com/vercel/ai/pull/10631)). This simplifies warning handling across different model types.
