...
Similar to [ AI SDK Core ](/docs/ai-sdk-core/overview) APIs (like [ `streamText` ](/docs/reference/ai-sdk-core/stream-text) and [ `streamObject` ](/docs/reference/ai-sdk-core/stream-text), `streamUI` provides a single function to call a model and allow it to respond with React Server Components.
...
## Advanced Usage
In some cases, you might need to stream text or components within a tool call of a streamUI function. This can be achieved by nesting another streamUI call inside the tool. Here's an example of how you might structure this:
typescriptCopyconst result = await streamUI({
  model: openai('gpt-4'),
  prompt: 'Create a data visualization dashboard',
  text: ({ content }) => <div>{content}</div>,
  tools: {
    generateDashboard: {
      description: 'Generate a data visualization dashboard based on user specifications',
      parameters: z.object({
...
The outer streamUI call handles the main conversation and determines when to generate a dashboard.
When the generateDashboard tool is called, it yields a loading component initially.
Inside the tool, another streamUI call is made to handle the dashboard generation process.
This inner streamUI can stream text updates about the dashboard generation progress.
Once the inner streamUI is complete, the final dashboard component is returned.

This nested structure allows for more complex interactions and real-time updates during tool execution.
