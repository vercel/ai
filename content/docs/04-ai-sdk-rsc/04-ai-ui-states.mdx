...
- the **AI state** is usually the conversation history (messages) between the user and the assistant. The LLM reads this history so it can generate the next message. This value is also the source of truth for the current application state. 
- On the client side, the **UI state** is the list of actual UI elements that are displayed to the user.
