...
You can also send URLs as attachments along with a message. This can be useful for sending links to external resources or media content. The following example demonstrates uploading a file using the `@vercel/blob` client and sending the URL as an attachment to the AI provider.
