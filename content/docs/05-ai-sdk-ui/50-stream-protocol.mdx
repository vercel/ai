...
You can use this information develop custom backends and frontends for your use case, e.g.,
to provide compatible API endpoints that are implemented in a different language such as Python.

## Text Stream Protocol

Text streams are simple text chunks that are streamed to the frontend.
Each chunk is a string that is appended to the response text.
...
Data streams are a special protocol that the Vercel AI SDK provides to stream data to the frontend.
