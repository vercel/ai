...
### Removed `embedding` and `textEmbedding` methods

The `embedding` and `textEmbedding` methods have been removed from all providers.
Please use the `textEmbeddingModel` method instead.
...
#### Removed `chat` and `messages` methods

The `chat` and `messages` methods have been removed from the Anthropic provider.
Please use the `languageModel` method instead.
...
#### Removed `generativeAI` method

The `generativeAI` method has been removed from the Google Generative AI provider.
Please use the `languageModel` method instead.
