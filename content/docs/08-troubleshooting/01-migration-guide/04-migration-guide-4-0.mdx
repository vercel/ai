...
#### Removed `embedding` and `textEmbedding` exports

The `embedding` and `textEmbedding` exports have been removed from the Mistral provider.
Please use the `textEmbeddingModel` export instead.
