...
We introduced support for using [React Server Components](https://react.dev/reference/rsc/server-components) (RSC) within the AI SDK to simplify building generative user interfaces for frameworks that supported RSC.

However, since we're pushing the boundaries of this technology, AI SDK RSC currently faces significant limitations that make it unsuitable for stable production use.
...
- Large amount of suspense can lead to crashes [(2843)](https://github.com/vercel/ai/issues/2843).
...
By building [v0](https://v0.dev) over the past year, we spent a lot of time figuring out ways to deliver the best chat experience for the web. The AI SDK UI packs some of these best practices and commonly used patterns like middleware, multi-step tool calls, attachments, telemetry, provider registry, and many more into a neat abstraction that you can use to reliably integrate AI into your applications.
...
The chat interface calls the server action, whose response is then stored in the `useUIState` hook.
...
The `streamText` function executes as part of a route handler and streams the response to the client. The `useChat` hook on the client decodes this stream and renders the chat interface.
...
This setup gets your chat-based application up and running!

### Generative User Interfaces

The `streamUI` function uses `tools` as a wat to execute functions based on user input and renders React components based on the function output to go beyond text in the chat interface.
...
With AI SDK RSC, components streamed to the client become interactive by using event handlers to call server actions. This is possible as long as the component is a descendant of the `<AI/>` context provider.
...
After switching to AI SDK UI, these messages are by initializing the `useChat` hook in the component with the same `id` as the parent component.
...
Before implementing `streamUI` as a server action, you should create an `<AI/>` provider and wrap your application at the root layout to sync between AI and UI states. During its initialization, you typically use the `onSetAIState` callback function to track updates of a mutable AI state and save it to the database when `done(...)` is called.
...
When you switch over to AI SDK UI, you will save chats using the `onFinish` callback function of `streamText` present in the route handler.
...
On the other hand, the AI SDK UI uses the `messages` field of `useChat` to store messages. To load messages when `useChat` is mounted, you should use `initialMessages`.

Since messages are typically loaded from the database, we can use a server actions inside a Page component to fetch an older chat from the database during static generation and pass the messages as props to the `<Chat/>` component.
...
The `createStreamableValue` function streams any serializable data from the server to the client. As a result, this function streams object generations when used along with `streamObject`.
...
To migrate to AI SDK UI, you should use the `useObject` hook and implement `streamObject` as part of a route handler.
