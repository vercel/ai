...
To check if the model has called a tool, you can use the `toolInvocations` property of the UIMessage object. This property contains information about any tools that were invoked in that generation including `toolCallId`, `toolName`, `args`, `toolState`, and `result`.
...
1. Check if the UIMessage has `toolInvocations`.
