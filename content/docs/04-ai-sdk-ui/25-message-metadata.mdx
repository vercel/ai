...
description: Learn how to attach and use metadata with messages and message parts in AI SDK UI
---
...
When you need metadata on individual parts such as text, reasoning, tool calls, sources, or files, you can use part metadata. Part metadata is attached to `message.parts[i].metadata` and streams alongside the corresponding part.
...
Use part metadata when the information belongs to an individual part instead of the whole message.
...
export const partMetadataSchema = z.object({
  planId: z.string(),
  phase: z.enum(['input', 'approval', 'output']).optional(),
});

type MessageMetadata = z.infer<typeof messageMetadataSchema>;
type PartMetadata = z.infer<typeof partMetadataSchema>;
...
```ts filename="app/api/chat/route.ts" highlight="10-19"
return result.toUIMessageStreamResponse({
...
On the client, access part metadata through `part.metadata`:
...
{message.parts.map((part, index) => {
  ...
  if (part.type.startsWith('tool-')) {
    return (
      <div key={index}>
        {part.toolCallId} ({part.metadata?.planId})
      </div>
    );
  }
...
<Note>
  Use message metadata for information about the whole message, and part metadata
  for information that belongs to a specific content part. The wire protocol uses
  `messageMetadata` for message-level chunks and `metadata` for individual part
  chunks.
</Note>
