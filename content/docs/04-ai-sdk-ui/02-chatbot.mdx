...
By default, `useChat` publishes message snapshots to React at most once every
50ms while a response streams. The first snapshot is published immediately,
and the final messages are published together with the terminal `ready` or
`error` status.

Stream processing, tool handling, and callbacks remain immediate. Only the
message snapshots exposed to React are paced.

You can set a custom publication cadence with the `throttle` option:
...
Set `throttle: 0` if your application intentionally needs every message
snapshot to be published without throttling:

```tsx filename="page.tsx" highlight="2-3"
const { messages, ... } = useChat({
  // Disable the default 50ms publication cadence:
  throttle: 0,
})
```
