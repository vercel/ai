...
The optional `inputFormat` field describes how subsequent input deltas are
encoded. It is `"json"` by default. Providers that stream unencoded freeform
text set it to `"text"`.
...
```
data: {"type":"tool-input-start","toolCallId":"call_fJdQDqnXeGxTmr4E3YPSR7Ar","toolName":"getWeatherInformation","inputFormat":"json"}
