...
<Note>
  The UI message stream parser is strict. Custom backends should only send the
  documented chunk types and fields. Rolling out new optional fields, such as
  part-level `metadata`, requires updated client bundles as well. Older clients
  can reject chunks that contain fields they do not know about.
</Note>
...
Text, reasoning, tool, source, file, and custom chunks can also include an
optional `metadata` field for application-defined part metadata.
