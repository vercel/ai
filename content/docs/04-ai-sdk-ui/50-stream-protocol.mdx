...
Indicates that a tool call failed validation — either the tool does not exist or the model produced input that does not match the tool's schema. When `invalid` is `true`, this represents an invalid tool call rather than a tool execution error.
...
```
data: {"type":"tool-input-error","toolCallId":"call_abc123","toolName":"unknownTool","input":{},"errorText":"Model tried to call unavailable tool 'unknownTool'.","invalid":true}
