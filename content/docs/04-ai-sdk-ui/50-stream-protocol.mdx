...
Indicates that a tool call requires approval.
...
```
data: {"type":"tool-approval-request","toolCallId":"call_fJdQDqnXeGxTmr4E3YPSR7Ar","approvalId":"approval_123","approvalDescriptor":{"scope":"account:delete"}}

```

`approvalDescriptor` is optional opaque metadata for the approval. When the
stream is processed into UI messages, it is available as
`part.approval.descriptor` and is retained when the client records an approval
response.
