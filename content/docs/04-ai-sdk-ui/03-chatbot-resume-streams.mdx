...
Stream resumption requires storing active streams outside your application. The AI SDK provides tools to connect to storage, but you need to set up the storage yourself.
...
- Redis to store the stream data
...
<Note>
  You must send the chat ID with each request (see `prepareSendMessageRequest`).
</Note>
...
After implementing the `GET` handler, create your `POST` handler to create resumable streams.

### 3. Create the POST handler
...
By default, the `useChat` hook makes a GET request to `/api/chat/[id]/stream` when resuming. Customize this endpoint using the `prepareReconnectToStreamRequest` option in `DefaultChatTransport`:
