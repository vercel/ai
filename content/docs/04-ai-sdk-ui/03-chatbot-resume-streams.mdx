...
<Note type="warning">
  Resumable streams are not compatible with request-bound abort handling.
  Closing a tab, refreshing the page, or calling `useChat().stop()` aborts the
  active HTTP request. If your server forwards that abort signal to
  `streamText`, the generation stops and there is nothing left to resume.

  If you need both resumable streams and a manual "Stop" action, implement
  stopping on the server: keep a server-owned `AbortController` (or shared
  cancellation registry), expose a separate stop endpoint, persist the partial
  assistant message, and clear the `activeStreamId` when stopping. In
  serverless or multi-instance deployments, do not rely on in-memory controller
  state in a single function instance. See
  [Stopping Streams](/docs/advanced/stopping-streams) and
...
- Two required API endpoints: POST to create streams and GET to resume them
- An optional POST stop endpoint if you need manual stopping with resumable streams
...
## Add a custom stop endpoint

The built-in `stop()` helper aborts the current HTTP request. That works for
non-resumable chats, but it conflicts with resumable streams when the server
forwards `req.signal` to `streamText`.

To support both resumable streams and manual stopping:

1. Create a server-owned `AbortController` (or cancellation token) for each active generation
2. Store a cancellation lookup key alongside the `activeStreamId` in shared storage (for example Redis)
3. Accumulate the assistant response on the server while streaming
4. Add a `POST /api/chat/[id]/stop` endpoint that resolves the key and aborts generation
5. When stopping, persist the partial assistant message and clear the `activeStreamId`
...
<Note>
  ...
  source of truth for persistence. In serverless or multi-instance deployments,
  resolve cancellation through shared infrastructure instead of in-memory state
  on a single process.
</Note>
...
- **Request-bound aborts**: Stream resumption is not compatible with aborting the active chat request by forwarding `req.signal` to the model generation. Closing a tab, refreshing the page, or calling `useChat().stop()` will abort the HTTP request and break resumption in that setup
- **Custom stopping**: If you need resumable streams and a manual stop action, implement cancellation on the server with a separate stop endpoint and shared cancellation infrastructure (not only in-memory state), then persist the partial assistant response before clearing the `activeStreamId`
...
- **Security**: Ensure proper authentication and authorization for creating, resuming, and optionally stopping streams
