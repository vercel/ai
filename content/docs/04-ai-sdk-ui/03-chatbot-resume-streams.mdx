...
<Note type="warning">
  ...
  stopping on the server: keep a server-owned `AbortController`, expose a
  separate stop endpoint, persist the partial assistant message, and clear the
  `activeStreamId` when stopping. See
...
- **Custom stopping**: If you need resumable streams and a manual stop action, implement cancellation on the server with a separate stop endpoint and a server-owned `AbortController`, then persist the partial assistant response before clearing the `activeStreamId`
