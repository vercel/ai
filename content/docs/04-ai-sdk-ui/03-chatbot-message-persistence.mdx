...
However, **for persistence, you should use IDs that are stable before messages are stored** to ensure consistency across sessions and prevent ID conflicts when messages are restored.
...
When implementing persistence, you have two options for generating server-side IDs for assistant response messages:
