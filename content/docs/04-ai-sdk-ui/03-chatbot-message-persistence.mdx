...
However, **for persistence, you typically want a server-generated ID for the assistant response** so the ID is determined where the message originates and persisted consistently across sessions.

### Setting Up Server-side ID Generation

When implementing persistence, you have two options for controlling the assistant response message ID on the server:
...
<Note>
  Both options control the **assistant** message ID only. The user message ID
  is still generated on the client by `useChat` (via its `generateId` option)
  before the request is sent. If you need a server-issued ID for the user
  message, either configure the client `generateId` to match your server's
  format, pass an explicit `id` when calling `sendMessage`, or assign an ID to
  the user message server-side before saving it.
</Note>
