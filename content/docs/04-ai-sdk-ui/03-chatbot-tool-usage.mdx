...
Sometimes an error may occur during tool use. Use the `addToolResult` method with a `state` of `output-error` and `errorText` value instead of `output` record the error.
