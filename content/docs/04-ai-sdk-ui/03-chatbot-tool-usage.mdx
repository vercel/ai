...
1. The [`onToolCall`](/docs/reference/ai-sdk-ui/use-chat#on-tool-call) callback is used to handle client-side tools that should be automatically executed.
   ...
   <Note>
     Always check `if (toolCall.dynamic || toolCall.invalid)` first in your
     `onToolCall` handler. Without this check, TypeScript will throw an error
     like: `Type 'string' is not assignable to type '"toolName1" |
     "toolName2"'` when you try to use `toolCall.toolName` in
     `addToolOutput`.
   </Note>
...
export default function Chat() {
  const { messages, sendMessage, addToolOutput } = useChat({
    ...
    async onToolCall({ toolCall }) {
      // Check for dynamic or invalid tools first for proper type narrowing
...
```tsx filename='app/page.tsx'
{
  message.parts.map((part, index) => {
    switch (part.type) {
      ...
      // Invalid tool calls (tool not found or bad input)
      case 'invalid-tool':
        return (
          <div key={index} className="text-red-500">
            Invalid tool call: {part.toolName} — {part.errorText}
          </div>
        );
    }
  });
