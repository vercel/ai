...
const stream = createUIMessageStream({
  ...
  onEnd: ({
    ...
    // Called on success, abort, and mid-stream error with the messages so far.
    console.log('Stream ended with messages:', messages);
    console.log('Stream outcome:', outcome.status, { isAborted });
  },
});
```
...
<PropertiesTable
  content={[
    ...
    },
    {
      ...
      description:
        'A callback that is called when the stream ends, including after abort or a mid-stream error. It receives the messages collected so far and an `outcome` of `completed`, `aborted`, `failed`, or `unknown`.',
      properties: [
        {
          type: 'EndOptions',
          parameters: [
