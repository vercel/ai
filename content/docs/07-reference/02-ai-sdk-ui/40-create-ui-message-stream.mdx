...
const stream = createUIMessageStream({
  ...
  onFinish: ({ messages, isContinuation, outcome, responseMessage }) => {
...
the final `onFinish` outcome `failed`. Individual `error` chunks do not change
the outcome by themselves. When merging multiple child streams, aggregate
their outcomes and call `setOutcome` once.
