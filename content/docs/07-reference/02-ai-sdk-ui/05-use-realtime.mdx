...
The hook supports token-based provider WebSockets and application-owned WebSocket
relays. It provides
...
including Responses delegation, live under `providerOptions.openai` and use
camelCase fields; the provider converts them to wire names.
...
<PropertiesTable
  content={[
    ...
    {
      name: 'api',
      type: '{ token: string } | { websocket: string; protocols?: string[] }',
      description:
        'Choose one supported connection mechanism: token setup or a WebSocket relay. Conversation semantics and connection capabilities are independent.',
      properties: [
        {
          type: 'Object',
          parameters: [
