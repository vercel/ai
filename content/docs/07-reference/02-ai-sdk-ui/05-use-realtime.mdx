...
not by itself confirm usage. `disconnect()` releases resources immediately; unmount
releases them in passive-effect cleanup.
...
An urgent Suspense fallback can hide an already-committed conversation without
unmounting it. Its connection, microphone, playback, and session state remain active
through hide/reveal, and retained controls still target that committed session.
Call `disconnect()` explicitly if the application should stop capture while hidden.
