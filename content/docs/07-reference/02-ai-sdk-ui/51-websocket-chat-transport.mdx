...
React `useChat` and Vue `useChat` close replaced transports and close the
current transport during hook cleanup. If you manage a `Chat` instance
directly, call `dispose()`:
