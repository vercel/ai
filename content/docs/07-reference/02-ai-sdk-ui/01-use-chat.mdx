...
<PropertiesTable
  content={[
    ...
    {
      name: 'ignoreIncompleteToolCalls',
      type: 'boolean',
      isOptional: true,
      description:
        'When true, tool calls with state "input-streaming" or "input-available" are filtered out before sending to the model. This allows users to send new messages even when frontend tools are awaiting input.',
    },
    {
      name: 'onFinish',
