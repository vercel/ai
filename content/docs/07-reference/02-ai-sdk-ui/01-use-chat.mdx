...
<PropertiesTable
  content={[
    ...
    },
    {
      ...
      isOptional: true,
      description:
        'Optional callback for automatic client-side tool execution. It is invoked once per tool call, when the complete tool input is available (the tool-input-available stream part), and only for tool calls that are not provider-executed. You must call addToolOutput to provide the tool result. It does not fire on tool-input-start, and there is no counterpart callback for tool results. To react to tools that execute on the server, derive the status from the tool parts of the assistant message or stream custom data parts and handle them in onData (see the Chatbot Tool Usage and Streaming Custom Data guides).',
    },
    {
