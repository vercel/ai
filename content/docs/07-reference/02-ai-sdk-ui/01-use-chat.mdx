...
<PropertiesTable
  content={[
    ...
    },
    {
      ...
      description:
        'Function to add a tool output to the chat. This will update the chat messages with the tool output. If sendAutomaticallyWhen is configured, it may trigger an automatic submission.',
    },
    {
      name: 'addToolResult',
      type: '(options: { tool: string; toolCallId: string; output: unknown } | { tool: string; toolCallId: string; state: "output-error", errorText: string }) => void',
      description:
        '**@deprecated** Use addToolOutput instead. Function to add a tool result to the chat. This will update the chat messages with the tool result. If sendAutomaticallyWhen is configured, it may trigger an automatic submission.',
    },
    {
      name: 'setMessages',
