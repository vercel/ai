...
When both fields are present, `input` takes precedence, including when its value
is `null`. `rawInput` is used only when `input` is `undefined`. Migrate stored
messages to `input` before the next major version, when `rawInput` will be
removed.
