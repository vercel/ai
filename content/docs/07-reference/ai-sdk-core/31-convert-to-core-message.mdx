...
The `convertToCoreMessages` function is used to transform an array of messages from the `useChat` hook into an array of `CoreMessage` objects. These `CoreMessage` objects are compatible with AI core functions like `streamText`.
  ...
  content={[
    {
      name: 'messages',
      type: 'Message[]',
      description: `An array of messages from the useChat hook to be converted`
    }
  ]}
/>
