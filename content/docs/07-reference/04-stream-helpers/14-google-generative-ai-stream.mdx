...
The Google Gemini multi-modal API is now integrated into the AI SDK. This API allows for real-time multi-modal interactions, including text, image, and audio inputs.

### Example Usage

Here is an example of how to use the Google Gemini multi-modal API with the AI SDK:

```javascript
import { createGoogleGenerativeAI } from 'ai-sdk/google';

const provider = createGoogleGenerativeAI({ apiKey: 'your-api-key' });

const model = provider.languageModel('gemini-2.0-flash-exp');
