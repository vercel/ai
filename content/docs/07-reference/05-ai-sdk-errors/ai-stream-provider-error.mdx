...
- `type`: The provider-defined error type or code discriminator (optional)
...
provider adapter rather than inferred from arbitrary provider error-code
substrings. Errors that are already `Error` instances and malformed or unknown
provider values are preserved unchanged.
