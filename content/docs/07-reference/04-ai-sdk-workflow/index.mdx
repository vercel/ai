...
[`@ai-sdk/workflow`](/docs/agents/workflow-agent) provides the `WorkflowAgent` class for building durable, resumable AI agents that run on the [Workflow SDK](https://workflow-sdk.dev/). It handles tool schema serialization, workflow step boundaries, and built-in tool approval flows.
