...
Runs the durable agent loop without forwarding streaming chunks. Pass either
`prompt` or `messages`; the remaining options match `stream()` except for the
stream-only `writable`, `sendFinish`, `preventClose`, `includeRawChunks`, and
`experimental_transform` options. Returns a promise resolving to a
`WorkflowAgentGenerateResult`.
...
The original no-argument placeholder signature remains as a deprecated no-op
for patch-release compatibility. New code should always pass `prompt` or
`messages`.
