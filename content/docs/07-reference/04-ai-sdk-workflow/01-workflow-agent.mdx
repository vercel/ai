...
Pass a non-negative `uiStartIndex` to omit that many transformed UI chunks.
This lets a reconnect endpoint replay a raw workflow stream from index `0`
while resuming at the UI chunk cursor reported by `WorkflowChatTransport`.
