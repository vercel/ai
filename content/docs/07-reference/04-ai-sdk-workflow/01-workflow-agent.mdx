...
Runs the durable agent loop and returns a non-streaming result that follows the
[`GenerateTextResult`](/docs/reference/ai-sdk-core/generate-text#returns)
experience. The result includes text, content, reasoning, files, sources,
tool calls and results from all steps, usage, warnings, request and response
metadata, response messages, structured output, and the final step.
`WorkflowAgent` also adds `messages`, containing the accumulated durable
conversation.
...
Pass exactly one of `prompt` or `messages`. `generate()` accepts the same
non-streaming call options as `stream()`, but does not accept streaming-only
options such as `writable`, `includeRawChunks`, `experimental_transform`,
`onError`, `onAbort`, `sendFinish`, or `preventClose`.

Although durable model calls may consume provider streams internally,
`generate()` uses non-streaming semantics: provider errors and aborts reject
the returned promise, and accessing a required structured `output` throws
`NoOutputGeneratedError` when no output was generated.
