...
<PropertiesTable
  content={[
    ...
    },
    {
      ...
      isOptional: true,
      description:
        'Default stop condition for the agent loop. Default: isStepCount(20). Per-stream values override this default. Use `isLoopFinished()` to remove the step limit and let the agent run until the model stops calling tools, but beware of potential runaway loops. See https://ai-sdk.dev/v7/docs/reference/ai-sdk-core/loop-finished#isloopfinished.',
    },
    {
      name: 'activeTools',
...
<PropertiesTable
  content={[
    ...
    },
    {
      ...
      type: 'StopCondition | StopCondition[]',
      isOptional: true,
      description: 'Condition(s) for ending the agent loop. Default: isStepCount(20). Use `isLoopFinished()` to remove the step limit and let the agent run until the model stops calling tools, but beware of potential runaway loops. See https://ai-sdk.dev/v7/docs/reference/ai-sdk-core/loop-finished#isloopfinished.',
    },
