...
<PropertiesTable
  content={[
    ...
    },
    {
      ...
      isOptional: true,
      description:
        'Callback that is called when an error occurs during streaming. The callback receives the error and a retry function.',
      properties: [
        {
          type: 'OnErrorResult',
          parameters: [
            ...
            {
              name: 'retry',
              type: '(newOptions?: Partial<StreamObjectOptions>) => StreamObjectResult',
              description:
                'Function to retry the streaming with modified options. The new options are merged with the original options.',
            },
          ],
        },
      ],
