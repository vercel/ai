...
<PropertiesTable
  content={[
    ...
    },
    {
      ...
      description:
        'The files that make up the skill. Each file must have a relative `path` and `data`. The `data` can be a string, a `Uint8Array`, or an object with `type: \'text\'` and a string `text` value, or `type: \'data\'` and a string `data` value.',
    },
    {
      name: 'displayTitle',
      type: 'string',
