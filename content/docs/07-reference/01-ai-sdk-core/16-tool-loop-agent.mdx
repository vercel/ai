...
<PropertiesTable
  content={[
    ...
    },
    {
      ...
      isOptional: true,
      description:
        'Whether messages with `role: "system"` are allowed in the `prompt` or `messages` fields. When false (default), system content must be provided through the `instructions` option. Set to true for trusted, server-generated `role: "system"` messages in conversation history.',
    },
    {
      name: 'tools',
