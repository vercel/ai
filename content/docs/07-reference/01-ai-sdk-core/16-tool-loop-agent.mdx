...
<PropertiesTable
  content={[
    ...
    },
    {
      ...
      isOptional: true,
      description:
        "Per-tool approval configuration for the agent. Each tool can be set to `'not-applicable'`, `'approved'`, `'denied'`, or `'user-approval'`, or to a function that returns one of those statuses based on the tool input and options such as `toolCallId`, `messages`, and `toolContext`. This setting takes precedence over a tool's `needsApproval` default.",
    },
    {
      name: 'output',
      type: 'Output',
