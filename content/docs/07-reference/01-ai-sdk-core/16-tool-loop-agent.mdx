...
<PropertiesTable
  content={[
    ...
    },
    {
      ...
      isOptional: true,
      description:
        'Experimental: Custom download function for fetching files/URLs. Return values: `{ data, mediaType }` for downloaded content, `URL` to pass through, `string` to replace with text, or `null` to remove the part.',
    },
    {
