...
<Note>
  ...
  `experimental_onToolCallStart`, and `experimental_onToolCallFinish` are
  accepted in `stream()` for API compatibility but are not currently forwarded
  to the underlying `streamText` call. Use `generate()` for full lifecycle
  callback support.
</Note>
