...
<PropertiesTable
  content={[
    ...
    },
    {
      ...
      isOptional: true,
      description:
        'Callback that is called when all agent steps are finished and the response is complete. Receives step results, total usage, shared `context`, `toolsContext`, and functionId. If also specified in `generate()` or `stream()`, both callbacks are called (constructor first).',
    },
    {
      name: 'context',
      type: 'CONTEXT',
