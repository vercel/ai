...
export async function POST(request: Request) {
  ...
  return createAgentUIStreamResponse({
    ...
    // sandbox, // optional: passed through to tool execution
...
- 3. **Streaming Agent Output:** Invokes the agent’s `.stream({ prompt, ... })` to get a stream of chunks (steps/UI messages), passing through options such as `sandbox`.
