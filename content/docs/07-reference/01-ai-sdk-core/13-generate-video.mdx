      ...
      properties: [
        {
          ...
          type: 'object',
          description:
            'A prompt object for video generation with optional input files',
          parameters: [
            {
              name: 'files',
              type: 'Array<DataContent>',
              description:
                'Input files (images) for image-to-video generation. Each item can be a URL string, base64-encoded string, a `Uint8Array`, an `ArrayBuffer`, or a `Buffer`.',
            },
            {
              name: 'text',
              type: 'string',
