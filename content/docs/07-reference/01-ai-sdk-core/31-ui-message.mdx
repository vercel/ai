...
`UIMessage` is designed to be type-safe and accepts four generic parameters to ensure proper typing throughout your application:
...
4. **`PART_METADATA`** - Custom metadata type for content-bearing message parts
...
Most content-bearing parts expose `metadata?: PART_METADATA`. Data parts and
`step-start` parts do not carry part metadata.
