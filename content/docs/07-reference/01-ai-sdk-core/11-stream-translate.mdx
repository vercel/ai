...
`experimental_streamTranslate` is built on the general speech-to-speech model
specification (`Experimental_SpeechToSpeechModelV4`), which covers
meaning-preserving audio-to-audio transforms (translation, voice conversion,
...). The translation function constrains the specification to the translation
task, e.g. by requiring `targetLanguage`. Provider implementations of the
specification ship separately — see your provider's documentation for
...
const result = streamTranslate({
  ...
  // speech-to-speech model specification (Experimental_SpeechToSpeechModelV4):
...
<PropertiesTable
  content={[
    {
      name: 'model',
      type: 'Experimental_SpeechToSpeechModelV4',
      description:
        'The speech-to-speech model to use for translation. Translation is a streaming-only modality. Pass a provider model instance that implements the experimental speech-to-speech model specification. String model IDs resolve through the global provider (AI Gateway by default) when it supports speech-to-speech models.',
    },
    {
      name: 'audio',
...
<PropertiesTable
  content={[
    ...
    {
      name: 'usage',
      type: 'Promise<Experimental_SpeechToSpeechModelV4Usage | undefined>',
      description:
    ...
    {
      name: 'responses',
      type: 'Promise<Array<SpeechToSpeechModelResponseMetadata>>',
