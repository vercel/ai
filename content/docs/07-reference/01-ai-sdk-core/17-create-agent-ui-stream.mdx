...
export async function* streamAgent(
  ...
  const stream = await createAgentUIStream({
    ...
    uiMessages,
    abortSignal,
    // sandbox, // optional: pass a sandbox through to tool execution
...
3. **Agent Streaming:** The agent's `.stream({ prompt, ... })` method is invoked with the converted model messages, optional call options, abort signal, sandbox, and any experimental transforms.
