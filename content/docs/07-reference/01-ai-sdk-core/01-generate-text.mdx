...
<PropertiesTable
  content={[
    ...
    {
      name: 'sensitiveRuntimeContext',
      type: 'SensitiveContext<CONTEXT>',
      isOptional: true,
      description:
        'Top-level runtime context properties that contain sensitive data and should be excluded from telemetry. Properties marked as `true` are omitted from telemetry integrations. Lifecycle callbacks and returned results still receive the full `runtimeContext`.',
    },
    {
      name: 'toolsContext',
      type: 'InferToolSetContext<TOOLS>',
...
Marks top-level `runtimeContext` properties that should be excluded from telemetry. Set a property to `true` to omit it from telemetry integrations; omitted or `false` properties are still sent.
