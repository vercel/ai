...
<PropertiesTable
  content={[
    ...
    },
    {
      ...
      type: 'Array<Source>',
      description:
        'Sources that have been used as input to generate the response. For multi-step generation, the sources are accumulated from all steps. When using a LanguageModelV1 provider, each source conforms to the `LanguageModelV1Source` interface.',
      properties: [
        {
          type: 'Source',
          parameters: [
