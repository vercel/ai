...
<PropertiesTable
  content={[
    ...
    },
    {
      ...
      isOptional: true,
      description:
        'Top-level runtime context properties that should be included in telemetry. When configured, only properties marked as `true` are sent to telemetry integrations. Runtime context properties are excluded unless they are explicitly set to `true`. Lifecycle callbacks and returned results still receive the full `runtimeContext`.',
    },
    {
      name: 'toolsContext',
      type: 'InferToolSetContext<TOOLS>',
...
Marks top-level context properties that should be included in telemetry. When configured, only properties set to `true` are sent to telemetry integrations; if omitted, no properties are sent.
