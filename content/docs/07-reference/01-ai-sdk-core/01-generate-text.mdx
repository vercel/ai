...
<PropertiesTable
  content={[
    ...
    {
      name: 'toolApproval',
      type: 'ToolApprovalConfiguration<TOOLS>',
      isOptional: true,
      description:
        "Per-tool approval configuration for this call. Each tool can be set to `'not-applicable'`, `'approved'`, `'denied'`, or `'user-approval'`, to an object form such as `{ type: 'denied', reason: 'blocked by policy' }`, or to a function that returns one of those statuses based on the tool input and options such as `toolCallId`, `messages`, and `toolContext`. `'not-applicable'` is the default execution path and runs the tool without approval metadata. Use `'approved'`, `'denied'`, or their object forms when you want explicit automatic approval request/response parts in the output. Automatic approvals and denials can include a `reason`, which is forwarded to the emitted approval response. This setting takes precedence over a tool's `needsApproval` default.",
    },
    {
      name: 'stopWhen',
    ...
    {
      name: 'experimental_onModelCallStart',
      type: '(event: ModelCallStartEvent) => PromiseLike<void> | void',
      isOptional: true,
      description:
    ...
    {
      name: 'experimental_onModelCallEnd',
      type: '(event: ModelCallEndEvent) => PromiseLike<void> | void',
      isOptional: true,
      description:
      ...
      properties: [
        {
          ...
          parameters: [
            ...
            },
            {
              name: 'text',
              type: 'string',
              description: 'Generated text from the model call.',
            },
            {
              name: 'reasoning',
              type: 'ReadonlyArray<{ text?: string }>',
              description: 'Reasoning text segments emitted by the model call.',
            },
            {
              name: 'files',
              type: 'ReadonlyArray<GeneratedFile>',
              description: 'Files generated directly by the model call.',
            },
            {
              name: 'toolCalls',
              type: 'ReadonlyArray<TypedToolCall<TOOLS>>',
              description: 'Parsed tool calls emitted by the model call.',
            },
          ],
        },
      ],
      ...
      properties: [
        {
          ...
          parameters: [
            ...
            },
            {
              ...
              type: 'CONTEXT',
              description:
                'The final state of the user-defined shared runtime context object. This reflects any modifications made during the generation lifecycle via `prepareStep`.',
            },
            {
              name: 'toolsContext',
              type: 'InferToolSetContext<TOOLS>',
