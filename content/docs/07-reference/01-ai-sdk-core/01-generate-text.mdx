...
<PropertiesTable
  content={[
    ...
    },
    {
      ...
      type: 'IncludeContext<CONTEXT>',
      isOptional: true,
      description:
        'Top-level runtime context properties that should be included in telemetry. When configured, only properties marked as `true` are sent to telemetry integrations. If omitted, all runtime context properties are included. Lifecycle callbacks and returned results still receive the full `runtimeContext`.',
    },
    {
      name: 'toolsContext',
      type: 'InferToolSetContext<TOOLS>',
...
```ts
type IncludeContext<CONTEXT extends Context> =
  | { [KEY in keyof CONTEXT]?: boolean }
  | undefined;
```

Marks top-level context properties that should be included in telemetry. When configured, only properties set to `true` are sent to telemetry integrations; if omitted, all properties are sent.

### `SensitiveContext`

```ts
type SensitiveContext<CONTEXT extends Context> =
  | { [KEY in keyof CONTEXT]?: boolean }
  | undefined;
```

Marks top-level context properties that should be excluded from telemetry. Set a property to `true` to omit it from telemetry integrations; omitted or `false` properties are still sent.
