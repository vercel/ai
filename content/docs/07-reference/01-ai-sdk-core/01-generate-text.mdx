...
<PropertiesTable
  content={[
    ...
    },
    {
      ...
      isOptional: true,
      description:
        "Per-tool approval configuration for this call. Each tool can be set to `'not-applicable'`, `'approved'`, `'denied'`, or `'user-approval'`, or to a function that returns one of those statuses based on the tool input and options such as `toolCallId`, `messages`, and `toolContext`. `'not-applicable'` is the default execution path and runs the tool without approval metadata. Use `'approved'` or `'denied'` when you want explicit automatic approval request/response parts in the output. This setting takes precedence over a tool's `needsApproval` default.",
    },
    {
      name: 'stopWhen',
