...
`safeValidateUIMessages` is an async function that validates UI messages like [`validateUIMessages`](https://ai-sdk.dev/docs/reference/ai-sdk-core/validate-ui-messages), including message metadata, part metadata, data parts, and tools. Instead of throwing it returns an object with a `success` key and either `data` or `error`.
...
const partMetadataSchema = z.object({
  planId: z.string(),
});
