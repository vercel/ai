...
<PropertiesTable
  content={[
    ...
    {
      name: 'onError',
      type: '(event: OnErrorResult) => void | PromiseLike<void> | { retry: true } | PromiseLike<{ retry: true }>',
      isOptional: true,
      description:
        'Callback that is called when an error occurs during streaming. Return `{ retry: true }` for a provider error event to retry the current model step. Other return values do not request a retry.',
      properties: [
        {
          type: 'OnErrorResult',
          parameters: [
