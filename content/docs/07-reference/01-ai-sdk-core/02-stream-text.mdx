...
<PropertiesTable
  content={[
    ...
    {
      name: 'onError',
      type: '(event: OnErrorResult) => void',
      isOptional: true,
      description:
        'Callback that is called when an error occurs during streaming. Well-formed mid-stream provider error events are exposed as StreamProviderError instances. When streamRetries is explicitly configured, return `{ retry: true }` to retry the current model step. Other return values do not request a retry.',
      properties: [
        {
          type: 'OnErrorResult',
          parameters: [
