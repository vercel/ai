...
<PropertiesTable
  content={[
    ...
    },
    {
      ...
      isOptional: true,
      description:
        'Deprecated. Use experimental_include.rawChunks instead. Whether to include raw chunks from the provider in the stream. When enabled, you will receive raw chunks with type "raw" that contain the unprocessed data from the provider. This allows access to cutting-edge provider features not yet wrapped by the AI SDK. Defaults to false.',
    },
    {
    ...
    },
    {
      ...
      isOptional: true,
      description:
        'Controls inclusion of request body and request messages in step results, and raw provider chunks in the stream. By default, request body is included, request messages are excluded, and raw chunks are excluded. When processing many large payloads (e.g., images), set requestBody to false to reduce memory usage. Set requestMessages to true when you need access to the messages sent to the model. Set rawChunks to true when you need access to raw provider chunks. Experimental feature.',
      properties: [
        {
          type: 'Object',
          parameters: [
