      ...
      properties: [
        {
          ...
          parameters: [
            ...
            {
              name: 'onStepFinish',
              type: "(event: { messages: UIMessage[]; stepNumber: number; responseMessage: UIMessage; stepMessage: UIMessage; isContinuation: boolean; isAborted: boolean; stepParts: UIMessage['parts']; }) => Promise<void> | void",
              isOptional: true,
              description: 'Callback function called when a step finishes (on finish-step). Provides a minimal step message containing only the parts generated in that step, the accumulated response message so far, the full messages array at that point, and flags for continuation and abort status.',
            },
            {
