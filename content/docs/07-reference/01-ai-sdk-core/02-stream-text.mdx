...
<PropertiesTable
  content={[
    ...
    {
      name: 'experimental_deferToolExecution',
      type: 'boolean',
      isOptional: true,
      description:
        'When true, tool calls are collected during the provider stream but their execution is deferred until after the stream finishes. This is needed for workflow/durable agent compatibility where tool execution must happen after the LLM response concludes. Tools that require approval still emit approval requests during the stream and are handled in subsequent steps. Defaults to false.',
    },
    {
