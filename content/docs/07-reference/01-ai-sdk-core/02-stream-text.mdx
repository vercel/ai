...
<PropertiesTable
  content={[
    ...
    },
    {
      ...
      isOptional: true,
      description:
        'Controls inclusion of request body and request messages in step results, and raw provider chunks in the stream. By default, request body is included, request messages are excluded, and raw chunks are excluded. When processing many large payloads (e.g., images), set requestBody to false to reduce memory usage. Set requestMessages to true when you need access to the messages sent to the model. Set rawChunks to true when you need access to raw provider chunks.',
      properties: [
        {
          type: 'Object',
          parameters: [
