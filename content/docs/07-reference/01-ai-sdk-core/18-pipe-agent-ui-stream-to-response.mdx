...
<PropertiesTable
  content={[
    ...
    },
    {
      ...
      isRequired: false,
      description:
        'Optional sandbox environment that is passed through to tool execution. Tools can access it from their execution context.',
    },
    {
      name: 'options',
      type: 'CALL_OPTIONS',
...
1. **Runs the Agent:** Calls the agent’s `.stream` method with the provided UI messages and options, converting them into model messages as needed and passing through options such as `sandbox`.
...
- Pass `sandbox` when your agent tools need a sandbox environment during execution.
