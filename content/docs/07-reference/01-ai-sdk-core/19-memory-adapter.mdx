...
interface MemoryAdapter {
  ...
  readById?(agentId: string, entryId: string): Promise<MemoryEntry | undefined>;

  search?(agentId: string, query: string, options?: MemorySearchOptions): Promise<MemorySearchResult>;

  update(agentId: string, entryId: string, updates: Partial<Omit<MemoryEntry, 'id' | 'agentId' | 'createdAt' | 'updatedAt'>>): Promise<MemoryEntry>;
