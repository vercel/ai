...
In addition, `stream()` accepts:

- `onChunk` (optional): A callback invoked for each stream chunk.
- `onError` (optional): A callback invoked when an error occurs during streaming.
- `onAbort` (optional): A callback invoked when the stream is aborted.
- `experimental_transform` (optional): One or more stream transformations.
