...
names and JSON-serialized inputs, so reordered parallel calls are considered
equal while duplicate calls are preserved.
...
corresponding successful result. Inputs and outputs that cannot be serialized
with `JSON.stringify` do not trigger the condition.
