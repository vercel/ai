...
- `hasRepeatedToolCalls(count, options?)` — stops when consecutive steps repeat the same tool calls
...
<Note>
  ...
  making tool calls. Pair it with `hasRepeatedToolCalls()` to stop a loop that
  gets stuck making identical calls.
</Note>

### Stop Repeated Tool Calls

Use `hasRepeatedToolCalls()` to stop after consecutive steps repeat the same
tool calls. Each step is compared as an order-independent collection of tool
names and deterministically serialized inputs, so reordered object keys and
parallel calls are considered equal while duplicate calls are preserved.
...
A step without tool calls breaks the repetition streak. The condition is
evaluated after the repeated step and its tools have executed, so it prevents
the next model step.

For polling tools, set `compareResults: true` to require both inputs and outputs
to repeat. Repeating the same polling call with a changing result then counts as
progress:
...
When `compareResults` is enabled, every call in the compared steps must have a
corresponding final result or error. Preliminary results are ignored, and steps
with missing or deferred results do not trigger the condition. Values that
cannot be compared without losing information, such as cyclic structures,
functions, symbols, non-finite numbers, `Map`, or `Set`, also do not trigger it.
