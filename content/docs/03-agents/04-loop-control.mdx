...
- `loopIsFinished()` — never triggers, letting the loop run until the agent is naturally finished
...
If you want the agent to run until the model naturally stops making tool calls, use `loopIsFinished()`. This removes the default step limit:
...
const agent = new ToolLoopAgent({
  ...
  stopWhen: loopIsFinished(), // No maximum step limit.
});
...
<Note>
  Use `loopIsFinished()` with caution. Without a step limit, the agent could
