...
const agent = new ToolLoopAgent({
  model: mem0('gpt-6-astra', { user_id: 'user-123' }),
});
...
[Honcho](/providers/community-providers/honcho) gives agents persistent, reasoning-backed memory through middleware plus six tools: `honcho_chat`, `honcho_context`, `honcho_search`, `honcho_search_conclusions`, `honcho_get_representation`, and `honcho_save_conclusion`. Honcho derives conclusions about users and builds evolving representations of them over time.
...
The middleware injects relevant context into the system prompt before generation and persists messages after. Use stable `userId` and `sessionId` across a conversation so each turn reinjects updated context. Honcho is middleware, not a model provider — it works with any AI SDK provider.

See the [Honcho provider documentation](/providers/community-providers/honcho) for full setup and configuration.
