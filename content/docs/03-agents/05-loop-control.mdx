...
const hasAnswer: StopCondition = ({ text, toolCalls }) => {
  // Stop when the model generates text containing "ANSWER:"
  return text?.includes('ANSWER:') ?? false;
};
...
Custom conditions receive the current step information:

```ts
const budgetExceeded: StopCondition = ({ usage }) => {
  const costEstimate =
...
The `prepareStep` callback runs before each step in the loop. Use it to modify settings, manage context, or implement dynamic behavior based on the execution history.
...
const result = await generateText({
  ...
  prepareStep: async ({ messages, stepNumber }) => {
    // Add context instructions for specific steps
    if (stepNumber === 0) {
      return {
        messages: [
          {
            role: 'system',
            content: 'Start by gathering all necessary information',
          },
          ...messages,
        ],
      };
...
prepareStep: async ({
  ...
  stopWhen, // Current stopping conditions
}) => {
...
For scenarios requiring complete control over the agent loop, you can implement your own loop management instead of using `stopWhen` and `prepareStep`. This approach provides maximum flexibility for complex workflows.

[Learn more about manual agent loops in the cookbook →](/cookbook/05-node/55-manual-agent-loop)
