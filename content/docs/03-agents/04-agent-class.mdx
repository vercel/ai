...
The Agent class provides a structured way to encapsulate LLM configuration, tools, and behavior into reusable components. It handles the agent loop for you, allowing the LLM to call tools multiple times in sequence to accomplish complex tasks. This abstraction allows you to define agents once and use them across your application.
...
The Agent class addresses these needs by providing a single place to define your agent's behavior.
...
The Agent class accepts all the same settings as `generateText` and `streamText`, allowing you to configure:
...
To enable agents to call multiple tools in sequence, configure `stopWhen` to allow more steps. After each tool execution, the agent triggers a new generation where the model can call another tool or generate text:
...
## Defining Agent Behavior with System Prompts

System prompts are the primary way to define your agent's behavior, personality, and constraints. They set the context for all interactions and guide how the agent responds to user queries and uses tools.
...
Use system prompts to set boundaries and ensure consistent behavior:
...
The Agent class provides full TypeScript support. You can infer types for UI messages:
...
// Infer the UI message type for client components
...
export function Chat() {
  ...
  // Full type safety for messages and tool invocations
}
```
