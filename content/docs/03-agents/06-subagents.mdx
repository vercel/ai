...
The fan-out happens inside the parent agent's tool: a single tool call accepts an array of inputs and runs the subagent once per item with `Promise.all`. See [`parallel-subagents.ts`](https://github.com/vercel/ai/blob/main/examples/ai-functions/src/agent/openai/parallel-subagents.ts) for a runnable example that pairs this pattern with `toModelOutput` to keep the parent's context compact.
