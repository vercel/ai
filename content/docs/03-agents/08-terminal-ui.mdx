...
You can control how tool calls, reasoning, and assistant response statistics are
shown:
...
- `tools`: Use `"full"`, `"collapsed"`, `"auto-collapsed"`, or `"hidden"` to
  control tool card rendering. Defaults to `"full"`.
- `reasoning`: Use `"full"`, `"collapsed"`, `"auto-collapsed"`, or `"hidden"`
  to control reasoning rendering. Defaults to `"full"`.
- `assistantResponseStats`: Use `"outputTokensPerSecond"` to show output token
