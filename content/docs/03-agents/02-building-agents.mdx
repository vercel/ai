...
Model call settings returned from `prepareStep`, such as `temperature` and
`maxOutputTokens`, apply only to the current step. Later steps use the agent's
top-level settings unless they return another override.
