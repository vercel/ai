...
### Tools That Use Sandboxes

Pass `sandbox` when an agent tool needs a command or code execution
environment. The sandbox is a per-call value, so provide it to `generate()`,
...
The sandbox description is not added to the model prompt automatically. Include
it in the prompt or instructions when the model needs to know environment
details. Passing a sandbox does not sandbox the tool itself; the tool must
explicitly delegate operations to the sandbox. See the
[`sandbox` section in Tool Calling](/docs/ai-sdk-core/tools-and-tool-calling#sandbox)
for more details.
