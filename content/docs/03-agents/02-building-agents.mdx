...
- **`experimental_onStart`**: Called once when the agent operation begins, before any LLM calls. Receives model info, prompt, settings, and context.
...
- **`onFinish`**: Called when all steps are finished and the response is complete. Receives all step results, total usage, and context.
