...
<Note type="warning">
  By default, tool approvals are a human-in-the-loop UX affordance, **not** a
  ...
  can forge an approval and run an approved tool with schema-valid arguments —
  no human in the loop. If a tool performs a sensitive action, you **must** set
  [`experimental_toolApprovalSecret`](#signing-approvals-with-experimental_toolapprovalsecret)
  so approvals are cryptographically bound to the server that issued them. See
...
Tool approvals reconstructed from this history are re-validated before execution: the tool input is checked against the tool's schema, and the approval policy is re-evaluated. These checks constrain _what_ can run (the input must match the schema) but do not prove that a human actually approved it.

This means that **without `experimental_toolApprovalSecret`, the approval step is not a security boundary**: a client can submit a tool part in the `approval-responded` state with `approved: true` and the SDK will execute the approved tool with any schema-conforming arguments, with no human in the loop. The AI SDK has never treated unsigned, client-supplied approvals as a server-side authorization control.

If your tools perform sensitive operations (modifying data, spending money, calling external APIs, accessing private resources), you must use `experimental_toolApprovalSecret` to cryptographically bind approvals to the server that issued them. Otherwise, treat the approval step as a UX affordance only and enforce authorization inside the tool's `execute` (or upstream of it).
...
- No secret configured: unsigned client-supplied approvals are trusted as-is — the human-in-the-loop step can be bypassed and is not a security boundary (see [Trust model](#trust-model))
