...
<Note>
  Upgrading from `@ai-sdk/workflow` 2.0.19 or earlier changes an omitted
  `stopWhen` from unlimited natural completion to a 20-step limit. Workflows
  that need more than 20 steps must configure an explicit condition.
</Note>
