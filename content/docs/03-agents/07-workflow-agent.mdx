...
### Use `needsApproval` for human-in-the-loop tools
...
`needsApproval` also accepts an async function so you can decide per-input whether approval is required (see [Tool Approval](#tool-approval) above).
