...
| **`generate()` method** | Available                 | Available                                       |
...
2. Call `generate()` for a final result, or pass `getWritable()` to `stream()` for live chunks
3. Start the workflow from your API route

For non-streaming usage, call `generate()` with a prompt or messages. It runs
the same durable agent loop and returns the accumulated result without writing
stream chunks:
...
### Use `generate()` for non-streaming results

`WorkflowAgent.generate()` runs the same durable agent loop without writing
stream chunks. Pass a prompt or messages and read `result.messages` /
`result.output` once the promise resolves. Use `stream()` instead when you need
to provide a workflow `writable`.
