...
const agent = new WorkflowAgent({
  ...
  // Telemetry filtering: only top-level keys marked `true` are forwarded
  // to telemetry integrations. Everything else stays private.
  telemetry: {
    isEnabled: true,
    includeRuntimeContext: { requestId: true },
    includeToolsContext: { weather: { defaultUnit: true } },
  },
...
Other options carry over with the same names: `prepareStep`, `onStepFinish`, `onFinish`, `onError`, `toolChoice`, `activeTools`, `timeout`, `experimental_repairToolCall`, and the usual generation settings (`temperature`, `maxOutputTokens`, `topP`, …). `WorkflowAgent` additionally adds `prepareCall` (runs once before the loop) and the `experimental_onStart` / `experimental_onStepStart` / `experimental_onToolExecutionStart` / `experimental_onToolExecutionEnd` lifecycle callbacks documented above.
