...
Persist only the opaque resume state returned by the session lifecycle method.
The chat id can also be the harness `sessionId`, which gives the sandbox a
stable identity across requests and processes.
...
or replay efficiently.

Use `session.stop()` when you want to save resume state and stop the runtime and
sandbox after each turn. The next request resumes from persisted state.
