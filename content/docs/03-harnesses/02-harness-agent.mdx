...
`@ai-sdk/sandbox-vercel` is the supported sandbox provider for bridge-backed
harnesses such as Claude Code and Codex. Pi can also run with
`@ai-sdk/sandbox-just-bash` because it does not need a sandbox-exposed port.
