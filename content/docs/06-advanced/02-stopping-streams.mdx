...
<Note type="warning">
  Request-bound aborts are not compatible with resumable streams. If you're
  using `resume: true` in `useChat`, calling the built-in `stop()` helper,
  closing the tab, or refreshing the page aborts the active HTTP request. If
  your server forwards that signal to `streamText`, the generation stops and
  resumption fails.

  If you need both resumable streams and a manual "Stop" action, implement a
  separate server-managed stop endpoint instead of relying only on the built-in
  `stop()` helper. In serverless or multi-instance deployments, cancellation
  should be coordinated through shared infrastructure rather than per-instance
