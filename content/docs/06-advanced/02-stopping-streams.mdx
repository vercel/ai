...
The `onEnd` callback also receives an `outcome`. When the consumer cancels the
UI message stream before an `abort` part reaches it, such as during a client
disconnect, `outcome.status` is `'cancelled'` and `isAborted` remains `false`.
When the stream observes an `abort` part first, `outcome.status` is `'aborted'`
and `isAborted` is `true`. Check both statuses when the same cleanup should run
for either case.
