---
title: Why Generative User Interfaces
description: Why use generative user interfaces?
---
...
<div className="mb-4">
  <Note>
    {' '}
    While building generative user interfaces are possible with or without the use of React Server Components (RSC), the RSC architecture provides an optimal framework for delivering highly dynamic, personalized experiences. [Learn more](/link to nico's updated streaming react components link).
  </Note>
</div>
...
Generative user interfaces are not deterministic in nature; they depend on the model's generation output. Since these generations are probabilistic in nature, it is possible for every user query to result in a different user interface.
...
Learn how to create generative user interfaces with React Server Components [through `ai/rsc`](/examples/next-app/interface) or the framework-agnostic [`ai/ui` toolkit](/todo/jeremy/example).
