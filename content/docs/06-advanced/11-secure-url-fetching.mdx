...
- **Drops credentials across origins** — `Authorization`/`Cookie` are not sent
  to a host on a different origin than the provider's, including after a
  redirect that crosses origin.
