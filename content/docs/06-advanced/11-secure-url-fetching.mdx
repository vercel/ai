...
When bundling for Node.js, configure your bundler to select the `node` package
export condition (for example, `platform: 'node'` in esbuild or
`exportConditions: ['node']` in `@rollup/plugin-node-resolve`). If a portable
build runs on Node.js, default protected downloads throw before making a request
rather than silently skipping DNS protection.
