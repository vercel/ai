...
per-session run state — lives under a fixed root on your machine, `~/.ai-sdk-harness`,
never inside the project. This is not configurable, so every workspace shares one
predictable location. Bridge dependencies and setup markers (`.harness-bootstrap/…`) and
per-session run state (`.agent-runs/…`) live directly under that root, shared across every
project you run a workspace against.

A workspace additionally keeps a small per-project store there, so its own one-time setup
never re-runs and can be found again later:

```
~/.ai-sdk-harness/projects/<basename>-<hash>/
  manifest.json                 # maps the store back to the project path
  .first-create-<identity>.ok   # marks that setup for this project has run
```

The store is keyed by the project's resolved path, and deleting it just forces that
project's one-time setup to run again.
