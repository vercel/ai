...
that can expose a port, such as `@ai-sdk/sandbox-vercel` or `@ai-sdk/sandbox-sprites`.
Host-runtime harnesses such as Pi can also run with `@ai-sdk/sandbox-just-bash` because
they do not need a sandbox-exposed port.
