...
Use `session.steer()` to send an additional user instruction while a turn is
running:
...
try {
  const result = await agent.stream({
    session,
    prompt: 'Inspect the repository and propose a migration plan.',
  });

  const steering = session.steer(
    'Focus only on the database migration and include a rollback plan.',
  );
...
`steer()` can be called immediately after starting `stream()` or `generate()`;
it waits until the adapter's active-turn control is ready. The runtime may
interrupt its current model work immediately or consume the instruction at its
next safe input boundary. In both cases, redirected output continues on the
original result stream.

Claude Code and Pi currently support active-turn steering. Codex, OpenCode, and
Deep Agents do not; calling `steer()` with those adapters throws
`HarnessCapabilityUnsupportedError`. Calling it without an active turn throws
an error. Steering is separate from aborting a turn with an `AbortSignal` and
from preserving a turn across process boundaries with `suspendTurn()`.
...
- `session.suspendTurn()` is for advanced active-turn continuation across a
  process boundary.
- `session.steer(message)` redirects the active turn when the harness supports
