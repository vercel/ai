...
`sandbox` is optional. When it is omitted, the harness runs **on your own machine**, in
`process.cwd()`, as the current user:
...
This is meant for local development. It reuses the CLI configuration, credentials and
skills the harness already has on your machine, so there is nothing to provision.

<Note type="warning">
  Running without a sandbox provides **no isolation**. The harness has every
  permission the current user has, and each one ships a shell tool that can read
  and write anywhere you can. It is the same trust level as running `claude` or
  `codex` in your terminal. For untrusted input or untrusted output, pass a real
  sandbox provider.
</Note>

A warning is emitted once per process in this mode. Silence it the same way as other AI SDK
warnings, by setting the `AI_SDK_LOG_WARNINGS` global to `false` or to your own logger.

### Generated directories

Running in your working directory means the harness generates state there. Three
directories may appear:

| Directory             | Contents                                                                                                |
| --------------------- | ------------------------------------------------------------------------------------------------------- |
| `.harness-bootstrap/` | Bridge dependencies for bridge-backed harnesses. Hundreds of megabytes; Claude Code's is around 785 MB. |
| `.agent-runs/`        | Per-session adapter state, written by every bridge-backed harness.                                      |
| `.harness-local/`     | One-time setup markers, standing in for the snapshots a hosted sandbox would use.                       |

Each is given a `.gitignore` of `*` when the session starts, so they never appear in
`git status` and cannot be committed by accident. Your own `.gitignore` is left untouched,
and this works whether or not the directory is a git repository. Delete any of them to
force that part to be regenerated.

Because the bootstrap is keyed to the working directory, each new directory you run in pays
for its own bridge install. Reusing a directory keeps that to once.
