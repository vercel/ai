...
- `toolApproval`: static approval status map or generic approval callback for
  host-executed tools. Callbacks receive parsed custom-tool calls, not
  adapter-native built-in calls.
