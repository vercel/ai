...
spawn processes **and** expose a reachable port — either `@ai-sdk/sandbox-vercel` (an
isolated remote sandbox) or `@ai-sdk/sandbox-local-workspace` (the user's own machine,
with no isolation). Host-runtime harnesses such as Pi can also run with
