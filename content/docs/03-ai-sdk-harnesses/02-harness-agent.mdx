...
Bridge-backed harnesses bootstrap into `.harness-bootstrap/` inside the working directory,
and one-time setup markers go in `.harness-local/`. Both are given a `.gitignore` so they
stay out of `git status`; delete either directory to force a clean re-bootstrap.
