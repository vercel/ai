...
Running in your working directory means the harness generates state there. Two directories
may appear:

| Directory             | Contents                                                                                                                    |
| --------------------- | --------------------------------------------------------------------------------------------------------------------------- |
| `.harness-bootstrap/` | Bridge dependencies for bridge-backed harnesses, plus setup markers. Hundreds of megabytes; Claude Code's is around 785 MB. |
| `.agent-runs/`        | Per-session harness state, including the event log a suspended turn is replayed from.                                       |
...
and this works whether or not the directory is a git repository. Delete either to force that
part to be regenerated.
