...
## Capturing Validated Output

Harness agents configured with an AI SDK `Output` specification can persist
their parsed and schema-validated value in the workflow result. Set
`includeOutput: true` on each workflow runner call:
...
import type { ScoreOutput } from './agent';

export async function agentStep(
  state: HarnessWorkflowState<ScoreOutput>,
): Promise<HarnessWorkflowState<ScoreOutput>> {
  'use step';
...
Initialize the state with the same output type and read the value returned by
...
export async function scoringWorkflow(input: HarnessWorkflowInput) {
  'use workflow';

  let state = createHarnessWorkflowState<ScoreOutput>(input);
...
Output capture is disabled by default. This opt-in is important because a
`HarnessAgent` without an output specification still exposes an output promise,
but that promise rejects when read. When enabled, the runner waits for output
only after the harness turn is fully finished. Time slices, semantic step
boundaries, pending tool results, and tool approval pauses do not trigger final
output parsing.

Malformed JSON and values that do not match the configured schema make the
workflow state `failed`; `finalizeHarnessWorkflow()` then throws the validation
error message. The streamed text may already have reached the client before
final validation runs.

<Note>
  `finalResult.output` is persisted as part of the workflow checkpoint. Ensure
  the validated value is supported by Workflow serialization. Schema transforms
  that produce values such as class instances may not be serializable. Treat the
  output as retained workflow data when applying access controls, observability,
  and sensitive-data policies.
</Note>
