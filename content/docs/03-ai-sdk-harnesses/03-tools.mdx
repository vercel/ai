...
call options. `HarnessAgent` validates each entry against the tool's
`contextSchema` before execution and exposes the configured map in step results
...
Only context entries supplied through `toolsContext` are schema-validated by
the harness. This preserves tools that bind context inside an `execute` wrapper.
