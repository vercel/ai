...
Use `toolApproval` for host-executed tools. A static map applies one status per
tool:
...
`approved` and `denied` emit automatic `tool-approval-request` and
`tool-approval-response` parts. Object-form reasons are included on the
automatic response; an approved tool then continues to host execution.

For input-dependent policies, pass AI SDK's generic approval callback:
...
const tools = {
  workspaceFile: tool({
    ...
    execute: async input => {
      // perform the host-side operation
    },
  }),
};

const toolApproval = (async ({ toolCall }) => {
  if (toolCall.dynamic || toolCall.toolName !== 'workspaceFile') {
    return { type: 'denied', reason: 'unknown host tool' };
  }
  if (!toolCall.input.path.startsWith('workspace/')) {
    return { type: 'denied', reason: 'path is outside workspace/' };
  }
  return toolCall.input.operation === 'read' ? 'approved' : 'user-approval';
}) satisfies GenericToolApprovalFunction<
  typeof tools,
  {},
...
The callback is evaluated after custom-tool input parsing and only for
host-executed tools; it does not override adapter-native built-in permissions.
You can reuse the same `GenericToolApprovalFunction` with `streamText` or a
`ToolLoopAgent` configured with the same tools.

The callback context is limited to data held by the host:

- `messages` contains the current result slice's host-visible user input. It is
  not the native agent's complete conversation history and is empty for
  continuation slices.
- `toolsContext` and `runtimeContext` are currently empty objects for
  `HarnessAgent`.
- The callback itself is not serialized into suspended or resumed session
  state. Recreate it with the agent and executable tools in the resuming host.
- An approval response for an existing pending request continues that request
  without evaluating the callback again.

Tool inputs are model-controlled. For authorization-sensitive policies,
validate every relevant field and prefer a deny-by-default fallback.
