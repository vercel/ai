...
Claude Code supports redirecting a running turn with
`session.steer(message)`. The additional user message is delivered through the
active Claude Code query, and output continues on the same result stream.
