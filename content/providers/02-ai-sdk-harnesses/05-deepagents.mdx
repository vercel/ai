...
from the host environment: explicit Anthropic auth first, then AI Gateway
credentials, then ambient Anthropic credentials. When the sandbox supports
additive request transformations, the bridge receives placeholders and the
adapter injects credentials into matching outbound requests. Other sandboxes
retain direct credential forwarding.
