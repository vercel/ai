...
- `resolveModel`: required callback that translates the `model` supplied to
  `HarnessAgent` into implementation launch arguments or environment values.
  Returned arguments replace `args`; returned environment values overlay `env`.
...
const harness = createACP({
  ...
  resolveModel: ({ model }) => ({ args: ['--model', model] }),
  outputSchemaMapping: {
