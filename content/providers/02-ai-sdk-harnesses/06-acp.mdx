...
- `permissionModeMapping`: mappings from all three Harness permission modes to
  advertised ACP session modes or configuration options.
...
The default simple NPM acquisition mode requires an exact package version:
...
response. ACP permission requests become Harness approval requests. A boolean
approval selects `allow_once` and a rejection selects `reject_once`; persistent
choices are never inferred. Host-tool approval remains independent.
...
Grok Build exposes ACP directly through `grok agent stdio`. The
`--always-approve` flag matches the Harness default `permissionMode` of
`allow-all`:
...
authentication. Grok Build does not advertise portable permission settings, so
this profile supports the default `allow-all` mode but not `allow-reads` or
`allow-edits`.
