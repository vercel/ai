...
Pi supports redirecting a running turn with `session.steer(message)`. Pi queues
the additional user instruction through its native steering API, and output
continues on the same result stream.
