...
- `reconnect`: optional timing configuration for reconnecting after an
  established ACP bridge connection drops. Set `maxElapsedMs` for the total
  reconnect window and `initialDelayMs` and `maxDelayMs` for exponential
  backoff.
