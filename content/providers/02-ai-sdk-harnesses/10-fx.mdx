...
- `auth`: selects `auto`, `direct`, or `ai-gateway` authentication, or accepts
  `{ gateway: { apiKey, baseUrl? } }` for a programmatically resolved Gateway
  credential. fx always sends model requests through Vercel AI Gateway, so
...
Pass an explicit Gateway credential when the host resolves authentication at
runtime instead of exposing it through `process.env`:

```ts
const gatewayHarness = createFx({
  auth: {
    gateway: {
      apiKey: await resolveGatewayToken(),
...
The adapter reads the explicit credential without adding it to `process.env` or
including it in persisted ACP lifecycle identity.
