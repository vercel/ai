...
You can create models that call the [ElevenLabs Conversational AI WebSocket API](https://elevenlabs.io/docs/conversational-ai/api-reference/conversations/websocket)
using the `.experimental_realtime()` factory method. The model ID is your ElevenLabs agent
ID.

```ts
import { elevenLabs } from '@ai-sdk/elevenlabs';

const model = elevenLabs.experimental_realtime('your-agent-id');
```

Realtime sessions run in the browser and require a signed URL created on your
server with `elevenLabs.experimental_realtime.getToken()`:

```ts
const token = await elevenLabs.experimental_realtime.getToken({
  model: 'your-agent-id',
});
```

ElevenLabs agents must have tools registered on the agent. Passing
`sessionConfig.tools` to `elevenLabs.experimental_realtime.getToken()` currently does not
register tools dynamically. See [Realtime](/docs/ai-sdk-core/realtime) for the
