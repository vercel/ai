...
You can create models that call the [ElevenLabs isolation API](https://elevenlabs.io/voice-isolator)
using the `.isolation()` factory method.

```ts
const model = elevenlabs.isolation();
```
...
| Model     | Isolate Speech      | Isolate Sound Effects |
| --------- | ------------------- | --------------------- |
| `default` | <Check size={18} /> | <Cross size={18} />   |
