...
using the `.realtime()` factory method. The model ID is your ElevenLabs agent
ID.

```ts
import { elevenLabs } from '@ai-sdk/elevenlabs';

const model = elevenLabs.realtime('your-agent-id');
```

Realtime sessions run in the browser and require a signed URL created on your
server with `elevenLabs.realtime.getToken()`:

```ts
const token = await elevenLabs.realtime.getToken({
  model: 'your-agent-id',
});
```

ElevenLabs agents must have tools registered on the agent. Passing
`sessionConfig.tools` to `elevenLabs.realtime.getToken()` currently does not
