...
<Note>
  ElevenLabs agents do not accept dynamic tool definitions over the realtime
  WebSocket. Register client tools on the agent in ElevenLabs first, with names
  and schemas matching your AI SDK tools. Passing `sessionConfig.tools` to
  `elevenLabs.experimental_realtime.getToken()` is ignored; the SDK executes a
  registered tool when the agent emits `client_tool_call`.
</Note>
