...
ElevenLabs transcription models support an `audio` argument which can be a `DataContent` (`string | Uint8Array | ArrayBuffer | Buffer`) or a `URL`.
...
- **additionalFormats** _array of objects_

  A list of additional formats to export the transcript to.
  Supported formats include: `'docx'`, `'html'`, `'pdf'`, `'segmented_json'`, `'srt'`, and `'txt'`.
  Each format has its own configuration options.
