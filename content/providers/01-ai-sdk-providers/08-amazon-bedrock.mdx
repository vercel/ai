...
- **embeddingFamily** _'titan' | 'cohere' | 'nova'_

  Explicitly identifies the embedding model family. Set this to `cohere` when
  using an application inference profile ARN, because its opaque identifier
  does not contain the underlying model name. This option also preserves
  Cohere's multi-value request batching.
...
For example, configure the family explicitly when using a Cohere application
inference profile:
