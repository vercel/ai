...
TwelveLabs Pegasus 1.2 is automatically detected from its Bedrock model ID and
uses the model's native `InvokeModel` API rather than the Converse API. Provide
exactly one video as inline data or an `s3://` URL, alongside the analysis
prompt:
  ...
  messages: [
    {
      ...
      content: [
        { type: 'text', text: 'Summarize this video.' },
        {
          type: 'file',
          mediaType: 'video/mp4',
          url: 's3://my-bucket/videos/example.mp4',
        },
      ],
    },
  ],
...
Pegasus also supports AI SDK structured output through its native JSON-schema
response format. Tool calling and conversational history are not supported by
the Pegasus API. You can use `streamText` for incremental video-analysis
responses.
