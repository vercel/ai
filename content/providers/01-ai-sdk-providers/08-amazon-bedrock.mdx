...
The Bedrock Mantle provider offers access to models through Amazon Bedrock's OpenAI-compatible Mantle API at `https://bedrock-mantle.{region}.api.aws/v1`. This endpoint supports both the Chat Completions API and the Responses API, and provides access to models that may only be available through the Mantle endpoint (such as `openai.gpt-oss-120b`).
...
You can create models using the Mantle provider instance. By default, the provider creates models using the Responses API:

```ts
const model = bedrockMantle("openai.gpt-oss-120);
```

You can also explicitly select the Chat Completions API or the Responses API:

```ts
const chatModel = bedrockMantle.chat("openai.gpt-oss-120);
const responsesModel = bedrockMantle.responses("openai.gpt-oss-120);
```
...
const { text } = await generateText({
  model: bedrockMantle("openai.gpt-oss-120),
...
| --------------------------------- | ------------------- | ------------------- |
| `anthropic.claude-mythos-preview` | <Check size={18} /> | <Check size={18} /> |
| `openai.gpt-oss-20b`              | <Check size={18} /> | <Check size={18} /> |
| `openai.gpt-oss-120b`             | <Check size={18} /> | <Check size={18} /> |
| `openai.gpt-oss-safeguard-20b`    | <Check size={18} /> | <Check size={18} /> |
| `openai.gpt-oss-safeguard-120b`   | <Check size={18} /> | <Check size={18} /> |
