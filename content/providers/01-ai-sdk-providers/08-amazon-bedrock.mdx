...
For Anthropic models, Bedrock can request object generation either through its **native** structured output (`output_config.format`) or through a **tool-based** JSON path. By default (`structuredOutputMode: 'auto'`) the provider uses native structured output when the model supports it and falls back to the tool path otherwise.

Some Bedrock partitions and regions — notably **AWS GovCloud (`us-gov-*`)** — do not accept the native `output_config.format` field and reject the request with `output_config.format: Extra inputs are not permitted`. Set `structuredOutputMode: 'jsonTool'` to force the tool-based path on those endpoints:
...
const result = await generateObject({
  model: amazonBedrock('us-gov.anthropic.claude-sonnet-4-5-20250929-v1:0'),
...
- `auto` _(default)_ — native `output_config.format` when the model supports it, otherwise the tool-based JSON path.
- `jsonTool` — always use the tool-based JSON path.
