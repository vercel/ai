...
- **preserveThinking** _boolean_
  ...
  `preserve_thinking` is enabled. Append AI SDK `responseMessages` unchanged when
  continuing the conversation. Model support and defaults vary; see Alibaba's
...
By default, the AI SDK omits reasoning from previous assistant messages when
sending follow-up requests. With `preserveThinking`, historical reasoning is
instead replayed as Alibaba `reasoning_content`, so supported models can build
on their earlier thought process:
...
discarded before the provider can serialize them as `reasoning_content`.
...
- It is only supported by selected Alibaba models, and model support and
  defaults vary. See Alibaba's
  [preserved-thinking documentation](https://docs.qwencloud.com/developer-guides/text-generation/thinking#preserve-thinking-in-multi-turn)
