...
<Note>
  DeepSeek reasoning works with both `generateText` and `streamText`. With
  `generateText`, reasoning content is available in `result.content` parts. With
  `streamText`, use `result.fullStream` to access reasoning and text parts as
  they stream.
</Note>
...
DeepSeek currently only supports text generation models. The provider does not support:

- **Embedding models**: Use other providers like OpenAI or Cohere for text embeddings
- **Image models**: Use other providers like OpenAI or Fireworks for image generation
- **Image input**: DeepSeek models do not support vision capabilities
