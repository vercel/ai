...
Images in tool results are supported as well. When a tool returns `content`
output that includes inline images or image URLs (for example a screenshot
returned from `toModelOutput`), the provider sends them as image content parts
of the tool message, so that vision models can read them. `imageDetail` applies
to these parts too. Tool result content without images is sent as JSON text.
