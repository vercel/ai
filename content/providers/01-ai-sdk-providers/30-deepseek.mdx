...
Both APIs are stateless and support text generation, object generation,
tool calling and [image input](#file-uploads) on the vision models. The
Responses API additionally supports DeepSeek's server-side
...
Images are supported on both APIs, so the same message works with
`deepSeek.responses('deepseek-v4-flash-vision-exp')`. Next to provider
references, you can also pass image URLs and image bytes directly.
