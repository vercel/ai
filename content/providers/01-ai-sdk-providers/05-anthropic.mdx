...
<Note>
  Different versions of the text editor tool are available depending on your
  Claude model: - **textEditor_20250728**: Latest version for Claude 4 models
  (Sonnet 4, Opus 4, Opus 4.1) with optional `maxCharacters` parameter -
  **textEditor_20250429**: Previous version for Claude 4 models -
  **textEditor_20250124**: For Claude Sonnet 3.7 - **textEditor_20241022**: For
  Claude Sonnet 3.5 (deprecated) Claude 4 models do not support the `undo_edit`
  command.
</Note>

**For Claude 4 models (Sonnet 4, Opus 4, Opus 4.1) - Latest Version:**

```ts
const textEditorTool = anthropic.tools.textEditor_20250728({
  max_characters: 10000, // Optional: control file truncation when viewing large files
  execute: async ({
    command,
    path,
    file_text,
...
**For Claude 4 models (Sonnet 4, Opus 4, Opus 4.1) - Previous Version:**
...
**For Claude Sonnet 3.5 (deprecated):**
...
- `command` ('view' | 'create' | 'str_replace' | 'insert' | 'undo_edit'): The command to run. Note: `undo_edit` is only available in Claude 3.5 Sonnet and earlier models, not in Claude 4 models.
...
**Configuration Options (textEditor_20250728 only):**

- `max_characters` (number, optional): Controls truncation when viewing large files. Only available in `textEditor_20250728` and later versions.

**Example with maxCharacters:**
...
const textEditorTool = anthropic.tools.textEditor_20250728({
  ...
  execute: async ({ command, path, view_range, ...params }) => {
    switch (command) {
      case 'view':
        const content = await fs.readFile(path, 'utf-8');
        // The max_characters truncation is handled automatically by the tool
        return content;
      case 'create':
        await fs.writeFile(path, params.file_text);
...
const response = await generateText({
  model: anthropic('claude-opus-4-20250514'),
  prompt:
    'Please examine the large log file at /var/log/application.log and summarize any errors',
  tools: {
...
**Complete Example: Story Creation with Claude Sonnet 4**

Here's a comprehensive example that demonstrates using Claude Sonnet 4 with the text editor tool to create, edit, and improve a story:

```ts
import { anthropic } from '@ai-sdk/anthropic';
import { generateText } from 'ai';

async function createStoryWithSonnet4() {
    ...
    execute: async ({
      ...
      switch (command) {
        ...
        case 'str_replace':
          ...
          if (!existingContent) return `Error: File "${path}" not found`;
          const newContent = existingContent.replace(
            old_str || '',
            new_str || '',
          );
          fileSystem.set(path, newContent);
          return `Text replaced in ${path}`;
        // ... handle other commands
      }
    },
  });
  ...
  const result = await generateText({
    ...
    prompt: `Create a short story about a robot discovering emotions. 
             Save it as "robot_story.txt" and then improve it by adding more details.`,
    tools: {
      str_replace_based_edit_tool: textEditorTool,
    },
  });
  ...
  console.log(result.text);
  console.log('Files created:', Array.from(fileSystem.keys()));
}
```

When using the Text Editor Tool, make sure to name the key in the tools object correctly:

- **Claude 4 models (Sonnet 4, Opus 4, Opus 4.1)**: Use `str_replace_based_edit_tool`
- **Claude Sonnet 3.7**: Use `str_replace_editor`
- **Claude Sonnet 3.5 (deprecated)**: Use `str_replace_editor`

```ts
// For Claude 4 models with latest text editor (recommended)
const textEditorTool = anthropic.tools.textEditor_20250728({
  max_characters: 10000, // Optional: limit file viewing to 10k characters
  execute: async ({
    command,
    path,
    file_text,
...
const response = await generateText({
  model: anthropic('claude-opus-4-20250514'),
  prompt:
    "Create a new file called example.txt, write 'Hello World' to it, and then view the file contents",
  tools: {
    str_replace_based_edit_tool: textEditorTool, // Claude 4 tool name
  },
});

// For Claude Sonnet 3.7
const sonnet37TextEditorTool = anthropic.tools.textEditor_20250124({
  execute: async ({
    command,
    path,
    file_text,
...
const sonnet37Response = await generateText({
  model: anthropic('claude-3-7-sonnet-20250219'),
  prompt:
    "Create a new file called example.txt, write 'Hello World' to it, and then view the file contents",
  tools: {
    str_replace_editor: sonnet37TextEditorTool, // Sonnet 3.7 tool name
  },
});

// For Claude Sonnet 3.5 (deprecated)
const legacyTextEditorTool = anthropic.tools.textEditor_20241022({
  execute: async ({
    command,
    path,
    file_text,
...
const legacyResponse = await generateText({
  model: anthropic('claude-3-5-sonnet-20241022'),
  prompt:
    "Create a new file called example.txt, write 'Hello World' to it, and then view the file contents",
  tools: {
    str_replace_editor: legacyTextEditorTool, // Sonnet 3.5 tool name
  },
});
```
