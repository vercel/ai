...
For Claude Opus 4.5, use `computer_20251124` which adds the `zoom` action for detailed screen region inspection:
...
const computerTool = anthropic.tools.computer_20251124({
  ...
  enableZoom: true, // Enable the zoom action
...
- `enableZoom` (boolean, optional): Enable the zoom action. Default: `false`.

The `zoom` action takes a `region` parameter with coordinates `[x1, y1, x2, y2]` defining the top-left and bottom-right corners of the area to inspect at full resolution.

#### Other Models (computer_20250124, computer_20241022)

For other supported models, use `computer_20250124` or `computer_20241022`:
