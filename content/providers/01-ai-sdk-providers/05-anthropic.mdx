...
const textEditorTool = anthropic.tools.textEditor_20250728({
  maxCharacters: 10000, // Optional: control file truncation when viewing large files
  execute: async ({
    command,
    path,
    file_text,
...
- `maxCharacters` (number, optional): Controls truncation when viewing large files. Only available in `textEditor_20250728` and later versions.
  ...
  execute: async ({ command, path, view_range, ...params }) => {
    switch (command) {
      case 'view':
        ...
        // The maxCharacters truncation is handled automatically by the tool
        return content;
      case 'create':
...
const textEditorTool = anthropic.tools.textEditor_20250728({
  maxCharacters: 10000, // Optional: limit file viewing to 10k characters
  execute: async ({ command, path, file_text, insert_line, new_str, old_str, view_range }) => {
