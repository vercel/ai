...
- `enableBetaFineGrainedToolStreaming` _boolean_

  Enables the Anthropic `fine-grained-tool-streaming-2025-05-14` beta, which skips buffering
  and JSON validation when streaming tool calls to reduce latency.

  <Note type="warning">
    Enabling this beta can cause parsing errors with some tool input schemas due to invalid
    JSON output from the LLM. See [the beta docs](https://docs.anthropic.com/en/docs/agents-and-tools/tool-use/fine-grained-tool-streaming) for more information.
  </Note> 
