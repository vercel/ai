...
Agent Skills enable Claude to perform specialized tasks like document processing (PPTX, DOCX, PDF, XLSX) and data analysis. Skills run in a sandboxed container and require the code execution tool to be enabled.
...
- **bigquery** - Query and analyze BigQuery data
- **nda-review** - Review and analyze NDAs

To use skills, you need to:

1. Enable the code execution tool
2. Specify the skills in `providerOptions`
3. Include the required beta headers
...
const result = await generateText({
  ...
  prompt: 'Create a quarterly business report with documents and spreadsheets',
  providerOptions: {
    anthropic: {
      skills: [
...
const result = await generateText({
  ...
  providerOptions: {
    anthropic: {
      skills: [
        { type: 'custom', skill_id: 'my-custom-skill-id', version: '1.0' },
      ],
      betas: [
        'code-execution-2025-08-25',
        'skills-2025-10-02',
...
<Note>
  ...
  container with code execution capabilities. If no version is specified,
  `'latest'` is used by default.
</Note>

<Note>
  For more information about Agent Skills, see [Anthropic's Agent Skills
