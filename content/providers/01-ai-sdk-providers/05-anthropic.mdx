...
The first argument is the model id, e.g. `claude-haiku-4-5`.
...
- `anthropic.languageModel('claude-haiku-4-5')` - Creates a language model
- `anthropic.chat('claude-haiku-4-5')` - Alias for `languageModel`
- `anthropic.messages('claude-haiku-4-5')` - Alias for `languageModel`
...
Anthropic requires tool input schemas with an object root and no root-level
`anyOf`, `oneOf`, or `allOf`. The provider automatically wraps incompatible custom
tool schemas in an object and unwraps the generated arguments. Your `inputSchema`,
input validation, and `execute` function keep their original shape.
...
const result = await generateText({
  ...
  tools: {
    lookup: tool({
      ...
      execute: async input => {
        // Receives the original union input, without the provider's wrapper.
  ...
  prompt: 'Look up item 123.',
});
```

The provider also wraps input examples and tool calls sent in conversation
history. Returned tool calls include `providerMetadata.anthropic.toolInputWrapped`
so their inputs can be wrapped again when replayed without the tool definition.
Preserve provider metadata when storing and replaying messages; AI SDK response
messages already include it.

For wrapped tools, `streamText` buffers arguments until the tool call completes,
then emits the unwrapped input as one delta. Ordinary object-root tools retain
incremental input streaming. Raw request and response bodies retain the wrapper.

Local JSON-pointer references are relocated with the schema. Schemas requiring
wrapping that contain resource identifiers (`$id`), anchors, dynamic references,
or external references throw `UnsupportedFunctionalityError`; wrap these schemas
explicitly in an object property. Wrapping addresses root shape restrictions;
model-specific and strict-mode restrictions still apply.

Message Batches also require explicit wrapping: batch results are retrieved
without the original schemas needed to unwrap them. Batch submission fails
locally if a tool needs automatic wrapping.

This behavior applies to the Anthropic adapter. Using Anthropic through AI
Gateway depends on the adapter deployed by Gateway.
...
With `claude-opus-5-5` and `claude-fable-5-1`, use an empty system message to
change effort from the next user turn. A separate system message containing text
can provide a turn-scoped reminder with `clearAt: 'next_user_message'`: its text
