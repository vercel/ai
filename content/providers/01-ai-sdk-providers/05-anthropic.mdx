...
<Note>
  ...
  - **textEditor_20250728**: Latest version for Claude 4 models with optional `maxCharacters` parameter
...
**For Claude 4 models (Opus & Sonnet) - Latest Version:**
...
**For Claude 4 models (Opus & Sonnet) - Previous Version:**
...
const legacyResponse = await generateText({
  ...
  prompt:
    "Create a new file called example.txt, write 'Hello World' to it, and then view the file contents",
  tools: {
    str_replace_editor: legacyTextEditorTool, // Earlier models tool name
  },
});
```
