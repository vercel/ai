...
Anthropic introduced an `effort` option with `claude-opus-4-5`. It affects thinking, text responses, and function calls. It defaults to `high`. You can set it to `medium` or `low` to save tokens and to lower time-to-last-token latency (TTLT).
