...
If you need more control over tool search behavior, you can implement your own client-side tool search. To return tool references that Anthropic's API understands, use the `contentType: 'tool_reference'` provider option in your tool's `toModelOutput` function:

```ts
import { anthropic, AnthropicToolOptions } from '@ai-sdk/anthropic';
import { generateText, tool } from 'ai';
import { z } from 'zod';
...
const result = await generateText({
  ...
  tools: {
    ...
    tool_search: tool({
      ...
      inputSchema: z.object({
        query: z.string().describe('Regex pattern to search for tools'),
      }),
      execute: async ({ query }) => {
        ...
        // Return tool references in the expected format
        return matches.map(t => ({
          type: 'tool_reference' as const,
          toolName: t.name,
        }));
      },
      // Convert output to tool_reference format for Anthropic
      toModelOutput: async ({ output }) => ({
        ...
        providerOptions: {
          ...
          } satisfies AnthropicToolOptions,
        },
      }),
    }),

    // Deferred tools that can be discovered
    get_weather: tool({
      description: 'Get the current weather at a specific location',
      inputSchema: z.object({
        location: z.string().describe('The city and state'),
...
When `contentType: 'tool_reference'` is set, the SDK validates that the output is an array of `{ type: 'tool_reference', toolName: string }` objects and converts it to the format expected by Anthropic's API.
