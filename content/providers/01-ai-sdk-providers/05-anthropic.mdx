...
const result = await generateText({
  ...
  prepareStep: ({ steps }) => {
    ...
    return {
      providerOptions: {
        anthropic: { container: { id: containerId } },
      },
    };
  },
});
...
When using programmatic tool calling across multiple steps, you need to preserve the container ID between steps using `prepareStep`. The container ID is available in `providerMetadata.anthropic.container.id` after each step completes.
