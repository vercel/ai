...
`status.type` is `available` when the classifier ran; `unsupported` means the API key is not enabled for the beta. Inside `status.tool_uses`, each tool call id maps to `evaluated` (with an `outcome` of `not_flagged` or `flagged`, the latter with an `explanation` such as `[Data Exfiltration]`), `skipped`, or `unavailable` (a transient classifier failure). Unknown keys are preserved as the beta evolves. When streaming, the verdicts arrive on the final `message_delta` event and are exposed on the `finish` part's provider metadata.
