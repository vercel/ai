...
Anthropic supports search result blocks and custom content documents that enable Claude to cite specific passages in its responses, matching the citation quality of web search. You can provide search results as file parts with the `providerOptions` property:
      ...
      content: [
        {
          ...
          data: new TextEncoder().encode(
            'Search result snippet about quarterly revenue growth.',
          ),
          providerOptions: {
            anthropic: {
              type: 'search_result',
...
- **citations** _object_
  ...
  Set `{ enabled: true }` to enable citations. Anthropic's API requires this setting to be consistent across all search results in a request; inconsistent values result in an API error.

- **content** _array_

  Custom content blocks (`{ type: 'text', text: string }`). If not provided, a single text block is derived from the file `data`.

#### Search Results in Tool Results

Tools can return search results directly in their results, so that RAG tool output becomes citable without any additional message content. Use the `toModelOutput` function of the tool to return a `content` output whose text parts carry the search result provider options:
...
const result = await generateText({
  ...
  prompt: 'What does the documentation say about deployment?',
});
```

Because the search results live inside the tool result, this also works in turns that mix server tools (such as web search) and client tools: the follow-up message that carries the tool results does not need any additional content blocks.

#### Custom Content Documents

For pre-chunked text blocks that should be used as-is for citations, use `source: { type: 'content', content: [...] }` with a `text/plain` file part. Claude cites these blocks via `content_block_location` citations:
      ...
      content: [
        {
          type: 'file',
          mediaType: 'text/plain',
          data: new TextEncoder().encode('Placeholder text for the file part.'),
          providerOptions: {
            anthropic: {
              source: {
                type: 'content',
...
For more details, see [Anthropic's Search Results documentation](https://platform.claude.com/docs/en/build-with-claude/search-results) and [Citations documentation](https://platform.claude.com/docs/en/build-with-claude/citations).
