...
Anthropic offers a [fast mode](https://platform.claude.com/docs/en/build-with-claude/fast-mode) for `claude-opus-4-6` that delivers 2.5x faster output token speeds at 6x standard pricing.

You can enable it using the `speed` provider option. The required beta header is added automatically.
...
// The response confirms whether fast mode was used
const anthropicMetadata = providerMetadata?.anthropic;
console.log(anthropicMetadata?.usage?.speed); // 'fast' or 'standard'
```

The API echoes `speed: 'fast'` in the response usage to confirm fast mode was activated.
