...
Code execution errors throw exceptions that you can catch:
...
const codeExecutionErrors = result.content?.filter(
  content =>
    content.type === 'tool-result' &&
    content.toolName === 'code_execution' &&
    content.isError,
);

if (codeExecutionErrors?.length > 0) {
  console.log(
    'Code execution error code:',
    codeExecutionErrors[0].result.errorCode,
  );
}
```
