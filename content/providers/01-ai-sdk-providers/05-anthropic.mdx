...
The signed compaction block must be the first message and must be replayed
without changing its text or signature. Remove the messages it summarizes
before continuing. Do not combine the `compaction` provider option with
`contextManagement`.
