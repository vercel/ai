...
submits a job and polls until it completes, then returns the resulting MP4 URL.
The video is not downloaded by the provider.
...
<Note>
  Result URLs are time-limited and signed. Download and persist the video to
  your own storage promptly after generation.
</Note>
...
Any other value still maps to the nearer tier, with a warning reporting the snap.
Use `providerOptions.blackForestLabs.resolution` to set the tier directly.
