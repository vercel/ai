...
The provider polls internally every 2 seconds for up to 10 minutes. To override
those defaults, create a provider instance with `pollIntervalMillis` or
`pollTimeoutMillis`. The FLUX 3 video API does not currently expose a webhook
callback input.
