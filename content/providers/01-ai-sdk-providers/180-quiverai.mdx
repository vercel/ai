...
<<<<<<< HEAD
[QuiverAI](https://quiver.ai/) provides Arrow language models that can reason about and create SVG documents, plus native SVG generation and vectorization endpoints. The AI SDK provider exposes Arrow 2 through `generateText` and `streamText` and preserves the existing image-model factories.
=======
[QuiverAI](https://quiver.ai/) generates SVG documents directly from text prompts, vectorizes raster images into clean SVGs, and edits and animates existing SVGs. The QuiverAI provider for the AI SDK exposes these operations through `generateImage`.
>>>>>>> origin/main
...
- **operation** _'generate' | 'vectorize' | 'edit' | 'animate'_
...
<<<<<<< HEAD
Model availability and supported operations depend on your organization and API key permissions. Consult QuiverAI's [model catalog](https://docs.quiver.ai/developers/models) for current capabilities and billing. Editing and animation are not yet exposed by this provider.
=======
Model availability and supported operations depend on your organization and API key permissions. Consult QuiverAI's [model catalog](https://docs.quiver.ai/developers/models) for current capabilities and billing. The Responses API is not yet exposed by this provider.
>>>>>>> origin/main
