...
<Note>
  MiniMax `mm_file://` handles are only forwarded as-is for `referenceAudio`.
  They cannot be used for `image`, `frameImages`, or `inputReferences`: those
  inputs go through the AI SDK's file handling, which base64-decodes any string
  that is not an `http(s)://` or `data:` URL. Pass those inputs as public URLs,
  data URIs, or binary data instead.
</Note>
