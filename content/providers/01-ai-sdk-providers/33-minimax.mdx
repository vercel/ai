...
- **serviceTier** _'standard' | 'priority'_

  Request admission tier, sent as `service_tier`. `'priority'` ensures priority
  admission so the request is processed ahead of other requests, at 1.5x the
  standard price. Defaults to `'standard'` when omitted.
