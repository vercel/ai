...
e.g. `Kore`, `Puck`, `Zephyr`, or `Charon`, or a custom `voice_...` ID or
`voicekey_...` replication key. Custom voices require Gemini 3.8 TTS. Voice names
are case-sensitive. The default voice is `Kore`.
...
accepts either `prebuiltVoiceConfig: { voiceName: 'Kore' }` or
`voice: 'voice_...'`, but not both. Google currently supports up to two prebuilt
voices in a multi-speaker request. For dialogue with designed or replicated
voices, generate each speaker's turns separately and concatenate the audio.
...
Use `google.voices()` to create a voice persona once, then reuse the returned
`voice_...` ID in speech requests with minimal or empty style instructions.
This replaces long audio profiles or director's notes in the transcript.
...
- `create({ store, voice })`: prompted voices require `store: true` and
  `voice.prompted.input`. Replicated voices use `type: 'replicated'` and
  `replicated: { consentAudio, sourceAudio }`, where each audio value contains
  base64 `data` and `mimeType`. Replicated voices may omit `store` or set it to
  `false` to receive a client-managed `key` instead of a stored `id`.
- `get({ id })`: retrieve a stored voice by its `voice_...` ID.
- `list({ pageSize, pageToken, ...filters })`: returns `voices` and
  `nextPageToken`. Pass that token to retrieve the next page, preserving the same
  filters. Filters include `search` (a string) and arrays for `type`,
  `languageCode`, `regionCode`, `accent`, `gender`, `persona`, `context`, and
  `pitch` (`low`, `medium`, or `high`).
- `delete({ id })`: delete a stored voice.

Creation also accepts `voice.model`, `voice.sampleAudio: { data, mimeType }`, and
discovery metadata: `displayName`, `description`, `languageCode`, `regionCode`,
`accent`, `gender`, `persona`, `context`, and `pitch`. Returned voices expose
`sampleAudio` when Google supplies a preview, and `expireTime` when applicable.
All operations accept `headers` and `abortSignal` and use the provider's
configured API key, base URL, and fetch implementation.
