...
warning to `result.warnings`. The warning identifies the removed keyword and
its location as a JSON Pointer, for example `/properties/code/pattern`.
...
Constraints that may be removed include string patterns and maximum lengths,
numeric bounds and multiples, array uniqueness and size bounds, object property
constraints, conditional schemas, and negated schemas. The converted schema can
therefore accept values that the original schema rejects. Continue validating
generated structured outputs and tool inputs against the original schema.
