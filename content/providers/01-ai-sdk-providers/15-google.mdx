...
Create a voice persona with Google's [Voice design](https://ai.google.dev/gemini-api/docs/voice-design)
tools, then pass the existing `voice_...` ID as `voice` in speech requests.
Use minimal or empty style instructions instead of putting long audio profiles
or director's notes in the transcript. Voice creation and management are not
