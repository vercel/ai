...
For language models, it uses Gemini structured output for Choice, Score, and Boolean questions. Choose a model
...
Use `google.evaluationModel('gemini-embedding-2')` for experimental Choice evaluation
...
The built-in embedding ID is `gemini-embedding-2`. Its inputs use Google's
`task: classification | query: ` prefix; do not pass the older `taskType` option.
Embedding options such as `outputDimensionality` go in `providerOptions.google`.
The factory's model-ID type includes both language and embedding model literals
for autocomplete. Other IDs retain the language backend by default. For a custom
embedding ID using the same encoding, select the backend explicitly:

```ts
google.evaluationModel('custom-embedding-id', { modelType: 'embedding' });
```
