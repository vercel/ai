...
<Note>
  Omni-family models (e.g. `gemini-omni-flash-preview`, which returns video as a
  file part) are only served by the Interactions API, so `google(...)` and
  `google.languageModel(...)` route them through `interactions()` automatically
  — you do not need to call `google.interactions(...)` explicitly for them.
</Note>
