...
The `google.files` client provides methods for uploading and managing files using the [Gemini File API](https://ai.google.dev/gemini-api/docs/files). This is essential for processing large media files (up to 2GB) that exceed the 20MB inline request limit.
...
const file = await google.files.upload({
  file: readFileSync('./video.mp4'),
  mimeType: 'video/mp4',
  displayName: 'my-video.mp4', // optional
});

console.log(file.uri); // Use this URI in your prompts
```

#### Waiting for Processing

Videos and audio files require processing before they can be used. Use `waitForProcessing()` to poll until the file is ready:
...
const readyFile = await google.files.waitForProcessing(file.name, {
  pollInterval: 2000, // optional, defaults to 1000ms
  maxWaitTime: 300000, // optional, defaults to 5 minutes
});

// Now use the file in a prompt
const result = await generateText({
  model: google('gemini-2.5-flash'),
  messages: [
    {
...
const { files, nextPageToken } = await google.files.list({
  pageSize: 10, // optional
  pageToken: nextPageToken, // optional, for pagination
});
...
- **`PROCESSING`** - File is being processed (videos/audio)
- **`ACTIVE`** - File is ready to use
- **`FAILED`** - Processing failed

<Note>
  The Files API is useful for large files that exceed the 20MB inline limit. For
  smaller files, you can include them directly in your prompts using the `file`
  content type.
</Note>
