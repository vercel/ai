...
- **propertyOrdering** _string[]_

  Optional. Array of property names specifying the order of properties in structured output.
  This ensures consistent property ordering in JSON responses and only applies to object schemas.

  When specified, properties in the generated JSON will appear in the exact order provided,
  which is useful for applications requiring predictable output formats.
  ...
  // Properties will appear in order: name, email, age
  ```

  For more information, see [Google's property ordering documentation](https://ai.google.dev/gemini-api/docs/structured-output#property-ordering).
