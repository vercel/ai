...
With [External API Grounding](https://docs.cloud.google.com/vertex-ai/generative-ai/docs/grounding/grounding-with-your-search-api),
the model can ground its responses using your own external search API. This enables you to connect Gemini to your existing search infrastructure,
whether it's a custom simple search API or an Elasticsearch deployment.

<Note>
  External API Grounding is only supported with Vertex Gemini models. You must
  use the Google Vertex provider (`@ai-sdk/google-vertex`) instead of the
  standard Google provider (`@ai-sdk/google`) to use this feature.
</Note>

#### Simple Search API

Use a custom search API that implements the [Simple Search spec](https://docs.cloud.google.com/vertex-ai/generative-ai/docs/grounding/grounding-with-your-search-api#simple-search):

```ts highlight="8,17-26"
import { createVertex } from '@ai-sdk/google-vertex';
import { GoogleGenerativeAIProviderMetadata } from '@ai-sdk/google';
import { generateText } from 'ai';
...
const { text, sources, providerMetadata } = await generateText({
  model: vertex('gemini-2.5-flash'),
  tools: {
    external_api: vertex.tools.externalApi({
      apiSpec: 'SIMPLE_SEARCH',
      endpoint: 'https://your-search-api.example.com/search',
      authConfig: {
        apiKeyConfig: {
          name: 'x-api-key',
  ...
  prompt: 'What are the latest updates from our internal knowledge base?',
});
...
Connect to your Elasticsearch deployment for grounding:

```ts highlight="8,17-30"
import { createVertex } from '@ai-sdk/google-vertex';
import { GoogleGenerativeAIProviderMetadata } from '@ai-sdk/google';
import { generateText } from 'ai';
...
const { text, sources, providerMetadata } = await generateText({
  ...
  tools: {
    external_api: vertex.tools.externalApi({
      ...
      endpoint: 'https://your-elasticsearch.example.com',
      authConfig: {
        httpBasicAuthConfig: {
          credentialSecret:
            'projects/my-project/secrets/elastic-credentials/versions/latest',
        },
      },
      params: {
        elasticSearchParams: {
  ...
  prompt: 'Search our product catalog for wireless headphones.',
});
```

#### Authentication Options

External API Grounding supports multiple authentication methods:

| Method                       | Description                                       | Use Case                                         |
| ---------------------------- | ------------------------------------------------- | ------------------------------------------------ |
| `apiKeyConfig`               | API key authentication via header, query, or body | Simple API key-based services                    |
| `httpBasicAuthConfig`        | HTTP Basic authentication using Secret Manager    | Services requiring username/password             |
| `googleServiceAccountConfig` | Google service account authentication             | Google Cloud services or services using GCP auth |
| `oauthConfig`                | OAuth 2.0 access token authentication             | OAuth-protected APIs                             |
| `oidcConfig`                 | OpenID Connect authentication                     | OIDC-compatible identity providers               |
...
authConfig: {
  apiKeyConfig: {
    ...
    apiKeyString: 'your-api-key',       // direct API key (or use apiKeySecret for Secret Manager)
    httpElementLocation: 'HTTP_IN_QUERY', // where to place the key
  },
}
```

The `httpElementLocation` can be one of:

- `HTTP_IN_QUERY` - As a query parameter
- `HTTP_IN_HEADER` - As a request header
- `HTTP_IN_PATH` - In the URL path
- `HTTP_IN_BODY` - In the request body
- `HTTP_IN_COOKIE` - As a cookie

##### Using Secret Manager

For production deployments, store your credentials in Google Cloud Secret Manager:

```ts
authConfig: {
  apiKeyConfig: {
    name: 'x-api-key',
    apiKeySecret: 'projects/my-project/secrets/my-api-key/versions/latest',
    httpElementLocation: 'HTTP_IN_HEADER',
  },
}
```
...
When using `ELASTIC_SEARCH` as the API spec, you can configure the following parameters:

| Parameter        | Type   | Description                                     |
| ---------------- | ------ | ----------------------------------------------- |
| `index`          | string | The Elasticsearch index to search               |
| `searchTemplate` | string | The search template to use for queries          |
| `numHits`        | number | Maximum number of results to return from search |

#### Grounding Metadata

When External API Grounding is enabled, the response includes grounding metadata with information about how external search results were used. The metadata structure follows the same format as other grounding methods:

- **`groundingChunks`** - Retrieved context chunks from your external API
- **`groundingSupports`** - Details about how response parts are supported by search results
- **`retrievalMetadata`** - Information about the retrieval process
...
```json
{
  "groundingMetadata": {
    "groundingChunks": [
      {
        "retrievedContext": {
          "uri": "https://your-api.example.com/doc/123",
          "title": "Product Documentation",
          "text": "The product supports real-time synchronization across all devices."
        }
      }
    ],
    "groundingSupports": [
