...
## Converting Messages to Google Format

You can convert AI SDK messages to Google's native message format using the `convertToGoogleGenerativeAIMessages` function. This is useful when you want to use Google's native APIs directly while still using the AI SDK's message format.

A common use case is leveraging Google's token counting API to estimate costs before making requests:

```ts
import { convertToGoogleGenerativeAIMessages } from '@ai-sdk/google';
...
const googlePrompt = convertToGoogleGenerativeAIMessages(messages);

// Use with Google's native token counting API
const model = genAI.getGenerativeModel({ model: 'gemini-2.5-flash' });
const { totalTokens } = await model.countTokens({
  contents: googlePrompt.contents,
  systemInstruction: googlePrompt.systemInstruction,
});
