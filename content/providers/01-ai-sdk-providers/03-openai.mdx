...
<Note type="warning">
  If you set `reasoningEffort` on a non-reasoning model, the AI SDK will strip
  the setting and log a console warning instead of throwing an error. This means
  the option can be silently ignored. See [Unsupported Feature
  Warnings](/docs/ai-sdk-ui/error-handling#unsupported-feature-warnings) to
  learn how to detect this during development.
</Note>
...
<Note type="warning">
  If you set `reasoningEffort` on a non-reasoning model (or a model the SDK does
  not recognize as a reasoning model), the setting will be silently stripped and
  a console warning logged. This can be a gotcha when testing private or early
  access models. See [Unsupported Feature
  Warnings](/docs/ai-sdk-ui/error-handling#unsupported-feature-warnings) for how
