...
delegation**, using `experimental_useRealtime` for the browser runtime. Your
application owns the agent and tool execution; Live delegations do not invoke
`onToolCall`. For a low-level server adapter, obtain server-only connection
...
Send the initial configuration with `session-start`. Live startup settings are
immutable: `session-update` is rejected. Use `input-audio-mute` and
`input-audio-unmute` for microphone control. Return application context through
...
It covers the JSON/PCM16 WebSocket relay runtime, microphone and playback ownership,
`session.delegations`, `sendEvent` for application context, and `close()` for bounded
finalization. Use `wss:` for production relays. Continuous conversation semantics
do not imply browser support for every provider codec or transport.
The [Realtime guide](/docs/ai-sdk-core/realtime) covers legacy token-based,
turn-based sessions.
