...
Boolean questions using the supported text models below.

The factory accepts only the following evaluation model IDs. Each has an explicit
reasoning default; unlisted IDs, including future releases and deployment aliases,
throw `NoSuchModelError` at model creation.

| Model ID        | Evaluation reasoning effort |
| --------------- | --------------------------- |
| `gpt-5`         | `minimal`                   |
| `gpt-5-mini`    | `minimal`                   |
| `gpt-5-nano`    | `minimal`                   |
...
These defaults disable thinking where supported or use the model's lowest
supported effort. Evaluation may still produce reasoning tokens. Native reasoning
controls in `providerOptions` override the evaluation default; unrelated options
preserve it. See [evaluation reasoning defaults](/docs/ai-sdk-core/evaluation#provider-models).
