...
OpenAI's `gpt-image-2` model supports powerful image editing capabilities. Pass input images via `prompt.images` to transform, combine, or edit existing images.
...
Combine multiple reference images into a single output. `gpt-image-2` supports up to 16 input images:
...
<Note>
  ...
  base64-encoded strings. For `gpt-image-2`, each image should be a `png`,
...
You can pass optional `providerOptions` to the image model. These are prone to change by OpenAI and are model dependent. For example, the `gpt-image-2` model supports the `quality` option:
