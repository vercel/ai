...
- `reasoningEncryptedContent` (optional)  
  ...
  When a reasoning item produces multiple summary parts (e.g. with `reasoningSummary: 'auto'` or `'detailed'`), every persisted reasoning UI part for that item carries the same `reasoningEncryptedContent`, so persisting and replaying reasoning is robust to downstream filtering, reordering, or partial save/restore.
