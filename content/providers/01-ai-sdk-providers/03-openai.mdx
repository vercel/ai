...
- **transport** _'http' | 'websocket'_
  ...
  Transport for native OpenAI Responses calls. Defaults to `'http'`.
...
The OpenAI provider supports [WebSocket mode](https://developers.openai.com/api/docs/guides/websocket-mode) for both `generateText` and `streamText`:
...
A single `generateText` or `streamText` invocation owns one WebSocket connection. The SDK reuses that connection across sequential model calls within the invocation, such as the steps in a multi-step tool-calling loop, and closes it when the invocation finishes. Each model call sends its complete current input.

The current implementation does not yet optimize follow-up calls into input deltas.

<Note>
  This transport is server-only and uses the runtime's global `WebSocket`. The
  runtime must support handshake headers in the constructor's second argument.
</Note>

`store: false` is supported. `conversation` and `previousResponseId` are not supported with WebSocket transport; use HTTP when you need either option.
