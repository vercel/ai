...
Custom tools let you constrain model output to a grammar (regex or Lark syntax),
making them useful for generating structured text like SQL queries, code snippets,
or any output that must match a specific pattern.
Unlike function tools which use JSON Schema parameters, custom tools use a `format`
field specifying the output grammar and return a raw string matching that grammar.
...
for (const toolCall of result.toolCalls) {
  ...
  console.log(`Input: ${toolCall.input}`); // raw string, e.g. "SELECT * FROM users WHERE age > 25"
}
```
...
- **format** _object_ (required) - The output format constraint:
  - **type** _'grammar'_ - The type of format constraint (always `'grammar'`).
  - **syntax** _'regex' | 'lark'_ - The grammar syntax. Use `'regex'` for regular expression patterns or `'lark'` for [Lark parser grammar](https://lark-parser.readthedocs.io/).
  - **definition** _string_ - The grammar definition string (a regex pattern or Lark grammar).
