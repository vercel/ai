...
SDP exchange with OpenAI using the server's project key. The browser receives the
...
- `delegation`: use `{ type: 'client' }` for application-owned work, or configure a
  Responses backend with a required `model`. Omission or null selects client mode.
- `delegation.responses`: supports `instructions`, function/web-search `tools`,
  `toolChoice`, `parallelToolCalls`, `maxOutputTokens`, `serviceTier`, `reasoning`
  and `text.verbosity`. Only the provider converts camelCase options to wire names.
...
- `client.dataChannel`: WebRTC startup permission policy with
  ...
  nested backend events. Keep readiness, errors, and terminal usage observable for
  the session features your application enables.

Use `Experimental_OpenAIRealtimeModelLiveUpdateOptions` for sparse backend settings
updates. Omitted settings remain unchanged; supported explicit nulls are preserved.
The voice model, voice, history, initial instructions and delegation mode cannot
be replaced by a backend update. Continuous captions, delegated activity and voice
duration are distinct; a completed backend response does not mean speech was heard.
