...
For language models, it uses the Responses API with structured output to answer Choice, Score, and
...
Use `openai.evaluationModel('text-embedding-3-large')` for experimental Choice evaluation
using embeddings. The provider selects the embedding backend automatically.
...
result.answers.department.choice; // 'billing' | 'technical' | 'account'
```

The adapter selects the criterion with the highest cosine similarity and returns
no probability distribution. It supports **Choice only**. Boolean and Score
questions, including mixed requests, fail before network access. Instructions
are encoded as semantic context; the adapter does not execute arbitrary rules,
negations, or reasoning instructions. Validate classification quality on your
own examples. See [embedding evaluation](/docs/ai-sdk-core/evaluation#embedding-backed-evaluation)
for encoding, caching, usage, and capability details.

The built-in embedding ID is `text-embedding-3-large`. Embedding options such as
`dimensions` go in `providerOptions.openai`. The factory's model-ID type includes
both language and embedding model literals for autocomplete. Other IDs retain
the language backend by default. For a custom embedding deployment:

```ts
openai.evaluationModel('custom-embedding-id', { modelType: 'embedding' });
```
