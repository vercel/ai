...
GPT Image models accept a `moderation` level on edits as well as on generations. The default, `auto`, is the standard filtering; `low` applies less restrictive filtering:
...
const { images } = await generateImage({
  ...
  prompt: {
    text: 'Draw the same scene at dusk',
    images: [imageBuffer],
  },
  providerOptions: {
    openai: {
