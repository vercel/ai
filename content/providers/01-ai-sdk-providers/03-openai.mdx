...
It uses the Responses API with structured output to answer Choice and Score
questions. Choose a model that supports structured output, such as `gpt-5.6-luna`.
...
distributions are returned, and Boolean questions are rejected before a request
because the adapter cannot provide their required probability of true.
