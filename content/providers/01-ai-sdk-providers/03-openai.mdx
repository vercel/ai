...
- **toolSearch** _boolean_

  Enable OpenAI [tool search](https://platform.openai.com/docs/guides/tools-tool-search) to let the model dynamically discover and select from a large set of tools. When enabled, a `tool_search` entry is appended to the tools array. Use together with `deferLoading` on individual tool provider options. Currently only available for `gpt-5.4` models.
...
OpenAI's [tool search](https://platform.openai.com/docs/guides/tools-tool-search) enables models to dynamically discover and select from a large set of tools. Instead of loading all tool definitions into the context window upfront, the model searches your tool catalog and loads only the tools it needs.

<Note>Tool search is currently only available for `gpt-5.4` models.</Note>

You can enable tool search by setting the `toolSearch` provider option to `true`. Tools that should be deferred (not loaded upfront) can be marked with `deferLoading: true` in their provider options.
...
const result = await generateText({
  ...
  tools: {
    get_weather: tool({
      ...
      // Mark as deferred - model discovers this via tool_search
      providerOptions: {
...
When `toolSearch` is enabled, a `{ type: "tool_search" }` entry is appended to the tools array sent to OpenAI. Tools with `deferLoading: true` are sent with `defer_loading: true`, meaning their definitions are not loaded into the context upfront and are instead discovered on-demand by the model.
