...
- **webSocket** _WebSocketConstructor_
  ...
  Custom WebSocket implementation for OpenAI Responses WebSocket transport
  and streaming transcription. Defaults to `globalThis.WebSocket`.
  Use this option to provide a runtime-specific WebSocket implementation.
...
- **transport** _'http' | 'websocket'_

  The transport used for OpenAI Responses API calls. Defaults to `'http'`.
  Set to `'websocket'` to reuse one WebSocket across the model calls in a
  single `generateText` or `streamText` execution.
...
Select the native WebSocket transport with the `transport` OpenAI provider
option. It works with both `generateText` and `streamText`:
...
The provider opens the connection lazily and stores it in the session for the
outer `generateText` or `streamText` execution. Every model step in that
execution reuses the connection. The SDK closes it when the execution finishes,
fails, or is aborted, so no manual cleanup is required. Separate top-level
executions do not share a WebSocket.

The provider uses `globalThis.WebSocket` in Node.js, Bun, and Deno by default.
Those built-in constructors receive `{ protocols, headers }` as their second
argument. Pass `webSocket` to `createOpenAI` when your runtime needs a
...
const openai = createOpenAI({
  webSocket: WebSocket as unknown as OpenAIProviderSettings['webSocket'],
});
```

Injected `ws`-compatible constructors retain their usual three-argument
signature: URL, protocols, then `{ headers }`.

<Note>
  WebSocket transport is available only for OpenAI Responses language models.
  Chat Completions, embeddings, and other OpenAI APIs continue to use HTTP.
</Note>

OpenAI limits each WebSocket connection to 60 minutes. If a connection closes
before the current model call has emitted output, the provider reconnects once
and replays the complete locally available context. If output has already been
emitted, it surfaces the connection error instead of risking duplicated output.
A chain started with an explicit `previousResponseId` cannot be replayed because
the earlier context is not available locally, so it fails safely if its
