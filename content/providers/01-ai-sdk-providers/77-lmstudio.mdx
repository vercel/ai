---
title: LM Studio
description: Learn how to use LM Studio.
---

# LM Studio Provider

<Note style={{ paddingTop: 0, paddingBottom: 0 }}>
  LM Studio is supported via OpenAI API compatibility - the OpenAI provider is
  used in the examples below.
</Note>

The [LM Studio](https://lmstudio.ai/docs) provider contains language model support for the LM Studio API.
...
```
###### LM Studio uses port `1234` by default, but you can change in the [app's Developer tab](https://lmstudio.ai/docs/basics/server).
...
LM Studio language models can also be used in the `streamText` and `streamUI` functions
