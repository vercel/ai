...
| `s2.1-pro`      | <Check size={18} /> |                                             |
| `s2.1-pro-free` |                     | Free developer tier; the Fish Audio default |
...
The Fish Audio speech-to-text endpoint exposes no model selector, so the model
ID is optional and defaults to the synthetic value `'default'`.
...
- **language** _string_
  ...
  Language of the audio. Treat this as a hint only: Fish Audio detects the
  language regardless, and setting this has been observed not to override that
  detection.
  Optional.
