...
- **maxWaitTimeInSeconds** _number_ - Maximum time in seconds to wait for the prediction to complete in sync mode. By default, Replicate uses [sync mode](https://replicate.com/docs/topics/predictions/create-a-prediction#timeout-duration) with a 60-second timeout. Set to a positive number to use a custom duration, or set to `0` to disable sync mode entirely (returns immediately with prediction in "starting" or "processing" state). When not specified, uses the default 60-second wait.
