...
The xAI Grok provider is available via the `@ai-sdk/spacexai` module. You can
install it with

<InstallPackages packages="@ai-sdk/spacexai" />
...
If you need a customized setup, you can import `createSpaceXAI` from `@ai-sdk/spacexai`
...
<Note>
  Since AI SDK 7, `spacexai(modelId)` uses the xAI Responses API by default. To
  use the [Chat Completions
  API](https://docs.x.ai/docs/api-reference#chat-completions) (legacy), use
  `spacexai.chat(modelId)`.
</Note>
...
`providerOptions.spacexai.reasoningEffort`. This works for both the Responses API
(default) and the Chat Completions API (`spacexai.chat()`).
...
const { text } = await generateText({
  ...
  providerOptions: {
    spacexai: { reasoningEffort: 'medium' },
  },
});
```
...
`providerOptions.spacexai.serviceTier` requests higher scheduling priority, which
...
console.log(providerMetadata?.spacexai?.serviceTier);
```
...
tier back from `providerMetadata.spacexai.serviceTier` rather than assuming the
...
const model = spacexai.experimental_realtime('grok-voice-latest');
```

Realtime sessions run in the browser and require a short-lived token created on
your server with `spacexai.experimental_realtime.getToken()`:

```ts
const token = await spacexai.experimental_realtime.getToken({
...
The xAI Responses API is the default when using `spacexai(modelId)` (since AI SDK 7). You can also use `spacexai.responses(modelId)` explicitly. This enables the model to autonomously orchestrate tool calls and research on xAI's servers.

```ts
const model = spacexai.responses('grok-4.6');
```
...
const { text } = await generateText({
  ...
  tools: {
    view_image: spacexai.tools.viewImage(),
  },
});
```
...
const { text } = await generateText({
  ...
  tools: {
    view_x_video: spacexai.tools.viewXVideo(),
  },
});
```
...
const { text } = await generateText({
  ...
  tools: {
    weather_server: spacexai.tools.mcpServer({
...
const { stream } = streamText({
  ...
  tools: {
    ...
    data_service: spacexai.tools.mcpServer({
...
- **serviceTier** _'default' | 'priority'_
  ...
  Scheduling priority for the request. `'priority'` buys lower time-to-first-token and faster inter-token latency at a premium per-token price. The tier xAI actually applied comes back on `providerMetadata.spacexai.serviceTier`, and is `'default'` when priority capacity was unavailable. See [Priority Processing](https://docs.x.ai/developers/advanced-api-usage/priority-processing).
...
You can pass xAI-specific controls using `providerOptions.spacexai`:
...
- **withTimestamps** _boolean_
  ...
  and total duration are exposed via `providerMetadata.spacexai` (see below).
...
const result = await generateSpeech({
  ...
  text: 'Hello world.',
  providerOptions: {
    spacexai: { withTimestamps: true } satisfies SpaceXAISpeechModelOptions,
  },
});

const { traceId, duration, audioTimestamps } = result.providerMetadata.spacexai;
```
...
```ts
const model = spacexai.transcription();
```
...
You can customize the image generation behavior with provider-specific settings via `providerOptions.spacexai`:
...
The xAI-hosted video URL is available in `providerMetadata.spacexai.videoUrl`.
...
const step1VideoUrl = step1.providerMetadata?.spacexai?.videoUrl as string;
...
The following provider options are available via `providerOptions.spacexai`.
You can validate the provider options using the `SpaceXAIVideoModelOptions` type.
