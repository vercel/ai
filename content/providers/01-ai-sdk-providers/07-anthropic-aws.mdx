...
Model IDs are identical to the first-party Anthropic API — no Bedrock-style prefix. See the [Anthropic provider docs](/providers/ai-sdk-providers/anthropic#language-models) for the full list of supported models, language model options, prompt caching, computer use, web search, code execution, and Agent Skills. All of these work identically here because Claude Platform on AWS uses Anthropic's runtime directly.
...
For request-level IAM policy examples (e.g. denying batch inference while allowing real-time inference), see the [Claude Platform on AWS documentation](https://platform.claude.com/docs/en/api/claude-platform-on-aws/iam-actions).
