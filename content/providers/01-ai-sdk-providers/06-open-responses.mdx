...
## Experimental Registered Extensions

Open Responses implementations can add namespaced tools, items, and streaming
events. Register an `Experimental_OpenResponsesExtension` when an endpoint
documents these wire formats and you want the generic adapter to encode and
decode them. This API is experimental and may change in a future release.
...
This lets a codec determine execution ownership from the individual item or
event instead of applying one value to the entire extension.
...
Tool, item, and event capabilities are independent. For example, an extension
that only decodes response receipts can provide `itemTypes` and `decodeItem`
without registering `toolType` or `encodeTool`. Paired fields must always be
provided together.
