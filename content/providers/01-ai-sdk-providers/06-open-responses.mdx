...
`bareToolType`, `bareItemTypes`, and `bareEventTypes` accept only
non-namespaced values. They do not enable unknown JSON passthrough: a bare
discriminator is handled only when it exactly matches a registered value.
Core Open Responses discriminators and duplicate registrations are rejected.
