...
- **reasoningReplay** _'full' | 'id-and-summary'_

  Controls whether stateless history replay includes all reasoning fields or
  only opaque item IDs and safe summaries. Defaults to `full`.
...
- **supportedReasoningEfforts** _readonly string[]_

  Optional allowlist for provider-native reasoning effort values.

- **supportedReasoningSummaries** _readonly string[]_

  Optional allowlist for provider-native reasoning summary values.
...
Open Responses custom tools return a caller-executed raw string, optionally
constrained by a regex or Lark grammar:
...
The endpoint must support the standard Open Responses `custom` tool,
`custom_tool_call`, and `custom_tool_call_output` wire types.
