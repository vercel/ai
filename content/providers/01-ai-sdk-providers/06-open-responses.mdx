...
For every decoded item, the provider stores the original JSON item in provider
metadata. When you append `result.response.messages` to a later request, that
item is replayed once, in its original position and with all opaque fields
preserved. This enables lossless manual-history round trips for hosted-tool
receipts.
...
in provider metadata and can therefore reach persisted messages, telemetry,
logs, or UI payloads.
