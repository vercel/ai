...
<Note>
  `frequencyPenalty` and `stopSequences` are not supported by reasoning models.
  `presencePenalty` is not supported by `grok-3` and reasoning models. The AI
  SDK removes unsupported settings and includes unsupported warnings in the
  response.
</Note>
