...
This provider supports five video generation modes: text-to-video, image-to-video, video editing, video extension, and reference-to-video (R2V).
...
<Note>
  Reference-to-video supports `duration`, `aspectRatio`, and `resolution`. It is
  mutually exclusive with `image` (image-to-video), `videoUrl` (editing), and
...
- **videoUrl** _string_
  ...
  to describe the desired edits to the video. Mutually exclusive with
  `extensionUrl` and `referenceImageUrls`.

- **extensionUrl** _string_

  URL of a source video to extend from its last frame. The extension
  continues the video with new content described by the prompt. Mutually
  exclusive with `videoUrl` and `referenceImageUrls`.
...
- **referenceImageUrls** _string[]_
  ...
  images. Mutually exclusive with `videoUrl` and `extensionUrl`.
