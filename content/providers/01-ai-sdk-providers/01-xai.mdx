...
for await (const item of getBatchResults({ model, batch })) {
  if (item.status === 'succeeded') {
    console.log(item.id, item.text, item.content);
  } else {
...
Successful items include ordered, Core-normalized `content` parts while `text` remains the concatenation of text parts. Non-text-only results therefore have `text: ''` but retain their reasoning, files, sources, tools, custom content, and part-level provider metadata in `content`. Retrieval parses tool inputs when possible but does not execute tools or fetch remote files. Avoid logging `content` indiscriminately because it can contain sensitive or untrusted provider output.
