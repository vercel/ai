...
const { video } = await generateVideo({
  model: xai.video('grok-imagine-video'),
  prompt:
    'The comic cat from <IMAGE_0> and the comic dog from <IMAGE_1> ' +
...
Use `<IMAGE_0>`, `<IMAGE_1>`, etc. in your prompt to reference specific images — the tags are zero-indexed and follow the order of the reference images you provide. Up to 7 reference images are supported per request.

With the `grok-imagine-video-1.5` model you can also provide a single reference audio track via `referenceAudioUrls`. Tag the speaker with `<AUDIO_0>` in the prompt so the model knows which voice to use:
...
const { video } = await generateVideo({
  ...
  prompt:
    'The creator from <IMAGE_0> holds the product from <IMAGE_1> and talks ' +
    'about it in the voice from <AUDIO_0>. Casual phone-camera framing.',
  duration: 10,
  aspectRatio: '9:16',
  providerOptions: {
    xai: {
      mode: 'reference-to-video',
      resolution: '720p', // reference-to-video is capped at 720p
      referenceImageUrls: [
...
`inputReferences` accepts audio as well as images. References with an `audio/*`
media type are routed to the reference audio track rather than sent as reference
images, so you can supply both kinds in one array with `grok-imagine-video-1.5`:
...
const { video } = await generateVideo({
  ...
  prompt:
    'The creator from <IMAGE_0> holds the product from <IMAGE_1> and talks ' +
    'about it in the voice from <AUDIO_0>.',
  inputReferences: [
    'https://example.com/creator.jpg',
    'https://example.com/product.jpg',
    { data: 'https://example.com/voice.mp3', mediaType: 'audio/mpeg' },
  ],
  duration: 10,
  aspectRatio: '9:16',
});
...
At most one audio reference is used; any extras are ignored with a warning. A
reference with no media type is treated as an image.
...
<Note>
  ...
  and `grok-imagine-video-1.5` models; optional reference audio
  (`referenceAudioUrls`) and native 1080p require `grok-imagine-video-1.5`.
</Note>
...
- **referenceImageUrls** _string[]_
  ...
  `<IMAGE_0>`, `<IMAGE_1>`, etc. (zero-indexed) in the prompt to reference
  specific images. Used with `mode: 'reference-to-video'`.

- **referenceAudioUrls** _string[]_

  Optional reference audio URL (max 1) for reference-to-video (R2V)
  generation, supported by the `grok-imagine-video-1.5` model. Include
  `<AUDIO_0>` in the prompt so the model knows which speaker should talk.
  Used whenever reference-to-video is selected — either via
  `mode: 'reference-to-video'` or via the top-level `inputReferences` option —
  so it does not require `referenceImageUrls` alongside it. Reference audio
  must accompany at least one reference image; on its own it is ignored with
  a warning. An audio reference passed through `inputReferences` takes
