...
- **reasoningEffort** _'low' | 'medium' | 'high' | 'xhigh'_
  ...
  Reasoning effort for reasoning models. `xhigh` is supported by `grok-4.6`.
...
- **reasoningEffort** _'low' | 'medium' | 'high' | 'xhigh'_
  ...
  Control the reasoning effort for the model. Higher effort may produce more thorough results at the cost of increased latency and token usage. `xhigh` is supported by `grok-4.6`. Defaults to `high` for models that support reasoning effort.
