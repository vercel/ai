...
xAI models support Live Search functionality, allowing them to query real-time data from various sources and include it in responses with citations. This feature is available at no additional cost until June 5, 2025.
...
You can specify which data sources to search from using the `sources` array:
...
- **allowedWebsites** _string[]_: Max 5 allowed websites (cannot be used with `excludedWebsites`)
- **excludedWebsites** _string[]_: Max 5 excluded websites (cannot be used with `allowedWebsites`)
...
- **xHandles** _string[]_: Array of X/Twitter handles to search
...
<Note>
  Live Search is currently in beta and available at no additional cost until
  June 5, 2025. You will still be charged for inference tokens. During the beta
  period, search request data may be shared with xAI to improve services.
</Note>
