...
- **searchParameters** _object_
  ...
  Deprecated. Kept for compatibility and ignored by the SDK

  xAI returns `410 Gone` when `search_parameters` is sent on both Chat Completions and Responses. Use server-side tools (`web_search`, `x_search`) for search
