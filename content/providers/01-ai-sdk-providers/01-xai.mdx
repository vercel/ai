...
<Note>
  Since AI SDK 7, `xai(modelId)` uses the xAI Responses API by default.
  `xai.chat(modelId)` is deprecated because xAI has deprecated its [Chat
  Completions API](https://docs.x.ai/docs/api-reference#chat-completions).
</Note>
