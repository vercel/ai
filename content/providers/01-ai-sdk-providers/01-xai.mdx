...
- **searchParameters** _object_

  Configure Live Search request parameters (`mode`, `returnCitations`, `fromDate`, `toDate`, `maxSearchResults`, `sources`).

  xAI currently recommends server-side tools (`web_search`, `x_search`) for Responses API search. `searchParameters` currently returns `410 Gone` with a deprecation message.
