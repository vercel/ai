...
<Note>
  ...
  documents zero-indexed tags (`<IMAGE_0>` … `<IMAGE_N>`, plus `<AUDIO_0>`).
  Follow the convention for the model you are calling.
</Note>
...
- **referenceAudioUrls** _string[]_
  ...
  precedence over this option. Passing more than one URL here is rejected as
  an invalid argument, whereas extra audio references supplied through
  `inputReferences` are truncated to the first one with a warning.
