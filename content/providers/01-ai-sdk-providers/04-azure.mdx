...
MAI-Transcribe-2 is a Microsoft Speech model, not an Azure OpenAI deployment. It requires
a Microsoft Foundry (Speech) resource in a region where MAI-Transcribe is available:
...
such as `https://eastus.api.cognitive.microsoft.com`.

<Note>
  `baseURL` and `apiVersion` are shared by all models of a provider instance. If
  you set them for Azure OpenAI (for example a URL ending in `/openai/v1` or
  `2025-04-01-preview`), create a separate provider instance for
  MAI-Transcribe-2. `useDeploymentBasedUrls` only affects OpenAI requests.
</Note>

The following options are available through `providerOptions.azure`:

- **api** _'openai' | 'speech'_
...
- **timestamps** _'word' | 'segment' | 'none'_
  Timing granularity. The AI SDK defaults to `segment` so that results include
  timed segments (Azure's own default is `none`). `segment` returns one segment
...
- **transcribeStyle** _'verbatim' | 'clean'_
...
- **diarization** _object_
  Set `enabled` to `true` to identify speakers. Speaker IDs are returned in
  provider metadata. `maxSpeakers` is not supported by MAI-Transcribe-2.
...
MAI-Transcribe-2 is a file transcription model: it accepts WAV, MP3, or FLAC files
shorter than 2 hours and smaller than 250 MB, and does not support streaming
transcription. The model is in public preview. See
