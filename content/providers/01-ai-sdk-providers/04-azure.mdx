...
- **speechBaseURL** _string_

  URL prefix for Azure Speech transcription requests (MAI-Transcribe-2), for example
  a regional endpoint such as `https://eastus.api.cognitive.microsoft.com`.
...
MAI-Transcribe-2 is a multilingual speech-to-text model from Microsoft AI, served through Azure.
...
MAI-Transcribe-2 requires a Microsoft Foundry (Speech) resource in a region where it is available:
...
warnings when used with the OpenAI API. Unknown or unsupported `providerOptions.azure`
keys (for example `diarization.maxSpeakers`) are rejected before the request is sent.
