...
<Note type="warning">
  The `networkPolicy` container option is currently not supported on the
  Microsoft platform (Azure OpenAI). At this time, Azure OpenAI Code Interpreter
  does not provide outbound external network access. As a result, outbound
  networking cannot be enabled or configured via the `networkPolicy` property.
</Note>
