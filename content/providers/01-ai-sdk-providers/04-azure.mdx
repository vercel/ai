...
Azure OpenAI Responses can use tool search through `azure.tools.toolSearch`,
following the same hosted and client-executed patterns as the OpenAI Responses
provider.

Mark deferred function tools with `providerOptions.openai.deferLoading: true` so
the model can discover and load them on demand.

##### Hosted Tool Search

Use hosted tool search when the candidate tools are already present in the
request:
...
Use client-executed tool search when tool discovery depends on runtime state:
...
const result = await generateText({
  ...
  tools: {
    ...
    get_weather: tool({
      ...
      execute: async ({ location }) => ({ location, temperature: 64 }),
      providerOptions: {
        openai: { deferLoading: true },
      },
    }),
...
<Note>
  Tool search is only supported with the default responses API, and is not
  supported when using `azure.chat` or `azure.completion`.
</Note>

<Note>
  Azure availability for tool search depends on the deployed Responses model and
  service support in your resource/API version.
</Note>
