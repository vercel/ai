...
`azure.transcription('mai-transcribe-2')` uses the Azure Speech API.
The model ID is matched case-insensitively and sent to Azure as `MAI-Transcribe-2`.
...
Use a Speech resource in a [supported region](https://learn.microsoft.com/en-us/azure/ai-services/speech-service/regions).
...
and custom fetch are also supported.

Speech requests use `https://{resourceName}.cognitiveservices.azure.com/speechtotext/transcriptions:transcribe`
with API version `2025-10-15`. An explicit `baseURL` supplies the Speech URL prefix
(without `/speechtotext/transcriptions:transcribe`); an explicit `apiVersion` overrides
the Speech default. An OpenAI URL ending in `/openai/v1` is not a Speech URL prefix.
`useDeploymentBasedUrls` only affects OpenAI requests.

The following options are available through `providerOptions.azure`:

- **api** _'openai' | 'speech'_: Overrides model-based API selection for this request.
  Set `openai` for an OpenAI deployment named `mai-transcribe-2`, or `speech` to
  send another model ID to Speech. Unknown Speech model IDs are sent unchanged.
- **timestamps** _'word' | 'segment' | 'none'_: Speech timing granularity. Defaults
  to `segment`. `word` also includes word timing in provider metadata; `none`
  allows responses without timed segments.
- **transcribeStyle** _'verbatim' | 'clean'_: Keeps or removes fillers. Azure defaults to `verbatim`.
- **locales** _string[]_: Exactly one language code, such as `['en']`. Omit for automatic detection.
- **diarization** _{ enabled: boolean }_: Enables speaker identification.
- **phraseList** _{ phrases: string[] }_: Recognition hints for names and terminology.
...
warnings when used with the OpenAI API.

Results include transcript text, timed phrase segments, duration, and an ISO-639-1
language code when the reported phrase locales identify one language. Mixed-language
results leave `language` undefined. Speaker IDs, exact locales, confidence, and words
are preserved in `providerMetadata.azure.phrases`; use the exported
`AzureTranscriptionProviderMetadata` type to access them. Phrase timing is in milliseconds
in metadata and seconds in normalized `segments`.

MAI-Transcribe-2 uses file transcription; streaming is not supported by this adapter.
The model is in public preview. See [Azure's MAI guide](https://learn.microsoft.com/en-us/azure/ai-services/speech-service/mai-transcribe)
for supported formats, regions, and diarization limits.

AI Gateway forwards these provider options, including `azure.api`. Using this model
through Gateway additionally requires support in the hosted Gateway service; the
Azure provider implementation does not add a Gateway catalog entry.
...
| `mai-transcribe-2`       | <Check />     | <Check /> | <Check /> | <Check /> |
