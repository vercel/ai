...
The Azure OpenAI Responses API supports server-side context compaction. When enabled, the server automatically compresses conversation context when token usage crosses a configured threshold. This is useful for long-running conversations or agent loops where you want to stay within token limits without manually managing context.

The compaction item returned by the server is opaque and encrypted. The AI SDK handles it automatically: compaction items are returned as custom parts with `providerMetadata`, and when passed back in subsequent requests they are sent as compaction input items.
...
<Note>
  Server-side compaction is only supported with the default responses API, and
  is not supported when using `azure.chat` or `azure.completion`.
</Note>

##### Detecting Compaction in Streams

When using `streamText`, you can detect compaction by checking for custom parts whose `providerMetadata.azure.type` is `'compaction'`:
...
for await (const part of result.fullStream) {
  switch (part.type) {
    case 'custom': {
      const isCompaction = part.providerMetadata?.azure?.type === 'compaction';
      if (isCompaction) {
        // ... your logic
      }
      break;
...
When using `useChat` or other UI hooks, compaction items appear as custom parts with `providerMetadata`. You can detect and style them differently in your UI:
...
```tsx
{
  message.parts.map((part, index) => {
    if (part.type === 'custom') {
      const isCompaction =
        (part.providerMetadata?.azure as { type?: string } | undefined)
          ?.type === 'compaction';
