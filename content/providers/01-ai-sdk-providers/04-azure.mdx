...
Set `environment.type` to `'containerAuto'` to run commands in an OpenAI-hosted container. No `execute` callback is needed — Azure OpenAI handles execution server-side:

<Note type="warning">
  The Azure OpenAI Responses API shell tool currently does not support `networkPolicy`.
</Note>
...
<Note type="warning">
  The Azure OpenAI Responses API shell tool currently does not support using Skills by `skill_id`.
</Note>
