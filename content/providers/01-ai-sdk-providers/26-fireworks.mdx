...
- **stabilityApiKey** _string_

  Stability AI API key. Some models are backed by Stability AI systems and may require this key.
  It defaults to the `STABILITY_API_KEY` environment variable.
...
Fireworks also provides access to some of Stability AI's models. These models
require a Stability AI API key. You can get a Stability AI API key from
[Stability AI](https://platform.stability.ai).

The Stability api key defaults to the value of the `STABILITY_API_KEY`
environment variable.

<Note>
  The Fireworks provider supports setting two different api keys: the `apiKey`
  and the `stabilityApiKey`. The `apiKey` is used for most Fireworks models,
  while the `stabilityApiKey` is only used for Stability AI models.
</Note>
...
const { image } = await generateImage({
  ...
  prompt: 'A sunrise over the Bahamas',
});
```

The key can also be set through the provider options:

```ts
import { createFireworks } from '@ai-sdk/fireworks';
import { experimental_generateImage as generateImage } from 'ai';

const fireworks = createFireworks({ stabilityApiKey: 'stability-api-key' });
const { image } = await generateImage({
  model: fireworks.image('accounts/stability/models/sd3-turbo'),
  prompt: 'A sunrise over the Bahamas',
});
...
The following Stability AI models are available:

| Model ID                               | Description                        |
| -------------------------------------- | ---------------------------------- |
| `accounts/stability/models/sd3-turbo`  | Fast inference optimized for speed |
| `accounts/stability/models/sd3-medium` | Balanced quality and speed         |
| `accounts/stability/models/sd3`        | Highest quality model              |

<Note>
  Stability AI models require a `STABILITY_API_KEY` environment variable or
  explicit `stabilityApiKey` in the provider options. You can get an API key
  from [Stability AI](https://platform.stability.ai).
</Note>
...
| `accounts/stability/models/sd3-turbo`                        | Aspect Ratio             |
| `accounts/stability/models/sd3-medium`                       | Aspect Ratio             |
| `accounts/stability/models/sd3`                              | Aspect Ratio             |
