...
The [Mistral AI](https://mistral.ai/) provider contains language model support for the Mistral chat API and transcription model support for the Mistral audio transcription API.
...
You can create models that call the [Mistral audio transcription API](https://docs.mistral.ai/api/endpoint/audio/transcriptions) using the `.transcription()` factory method.

```ts
const model = mistral.transcription('voxtral-mini-latest');
```

You can use Mistral transcription models with the `experimental_transcribe` function:
...
Mistral transcription models support additional provider options. You can pass them as provider options and utilize `MistralTranscriptionModelOptions` for typing:
...
The following optional provider options are available for Mistral transcription models:
...
- **timestamp_granularities** _'segment' | 'word'_

  Timestamp granularities to include in the transcription response.

  Mistral notes that `timestamp_granularities` is currently not compatible with `language`; the SDK sends the options as provided and lets the Mistral API validate unsupported combinations.
...
- **contextBias** _string[]_
  ...
  Words or phrases to guide the model toward correct spellings of names, technical terms, or domain-specific vocabulary.
