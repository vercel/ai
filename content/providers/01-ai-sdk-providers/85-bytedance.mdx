...
Seedream reports token usage for image generation, exposed through `result.usage`:
...
<Note>
  ModelArk reports no input token count for image generation, so `inputTokens`
  is always `undefined`. The `generated_images` field returned by the API is an
  image counter rather than a token count and is not mapped into `usage` — use
  `images.length` instead.
</Note>
