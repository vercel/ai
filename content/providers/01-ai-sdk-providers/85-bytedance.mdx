...
ByteDance Seedream performs edits on the same endpoint as generation. Pass input
images via `prompt.images` to transform an existing image (image-to-image):
...
<Note>
  The output format is requested as `b64_json` automatically so the SDK receives
  the image bytes. Additional ModelArk fields can also be passed through
...
| Seedream 5.0 Lite | `seedream-5-0-260128`          | Text-to-image, single/multi image-to-image, batch generation. Sizes: 2K, 3K, 4K. Formats: png, jpeg. Up to 14 references.             |
