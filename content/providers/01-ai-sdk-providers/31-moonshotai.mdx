...
Native structured outputs are enabled for Kimi models and the official Moonshot
V1 text, auto, and vision-preview models.
...
- **logprobs** _boolean_
  ...
  Whether to return log probabilities for generated tokens. Returned values are
  available at `result.providerMetadata.moonshotai.logprobs`.

- **topLogprobs** _number_

  Number of most likely tokens to return at each token position, from `0` to
  `20`. Setting this option automatically enables `logprobs`.
...
Kimi K3 can load function tools at a specific point in a conversation. Use an
empty system message with `dynamicTools`; the provider omits the placeholder
content and normalizes the tool schemas before sending them. Other models are
rejected locally because Moonshot documents dynamic loading only for K3.

```ts
import type { MoonshotAISystemMessageProviderOptions } from '@ai-sdk/moonshotai';
