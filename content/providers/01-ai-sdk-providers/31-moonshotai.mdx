...
Native structured outputs are enabled for Moonshot models whose model ID starts
with `kimi-k2.5`.

For other Moonshot models, structured output generation uses JSON-object
fallback behavior instead of schema-constrained decoding.

For best reliability, it is strongly recommended to include your schema
requirements in your prompt/instructions in addition to passing the schema
through `Output`.
