...
The Cerebras provider supports several factory methods:
...
const model = cerebras.languageModel('llama3.1-8b');

// Chat model factory  
const model = cerebras.chat('llama3.1-8b');
```
...
<Note>
  ...
  8192 tokens in the Free Tier. You can also pass any available provider
  model ID as a string if needed.
</Note>

## Limitations

The Cerebras provider does not support:

- **Embedding models** - Use `textEmbeddingModel()` will throw a `NoSuchModelError`
- **Image generation** - Use `imageModel()` will throw a `NoSuchModelError`
