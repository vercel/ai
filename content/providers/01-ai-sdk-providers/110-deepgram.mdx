...
The first argument is the model id. Deepgram embeds the voice and language directly in the model ID (e.g., `aura-2-helena-en`).

```ts
const model = deepgram.speech('aura-2-helena-en');
```

Alternatively, pass a bare voice family ID (`aura-2` or `aura`) and select the voice and language with the `generateSpeech` `voice` and `language` options. The provider composes the upstream model ID as `<family>-<voice>-<language>` (language defaults to `en`):
...
`modelUuid`, `charCount` (the billed character count), and `requestId`.

The `generateSpeech` `speed` option is passed through to Deepgram's `speed`
parameter (note that Deepgram does not support it for all languages).
`instructions` is not supported and is ignored with a warning.

### Model Capabilities

| Model                                                                            |
| -------------------------------------------------------------------------------- |
| `aura-2` (voice family — pass `voice` and `language` to `generateSpeech`)        |
| `aura` (voice family — pass `voice` and `language` to `generateSpeech`)          |
| `aura-2-<voice>-en` (41 English voices, e.g. `aura-2-thalia-en`)                 |
| `aura-2-<voice>-es` (17 Spanish voices, e.g. `aura-2-celeste-es`)                |
| `aura-2-<voice>-nl` (9 Dutch voices, e.g. `aura-2-rhea-nl`)                      |
| `aura-2-<voice>-de` (7 German voices, e.g. `aura-2-julius-de`)                   |
| `aura-2-<voice>-it` (10 Italian voices, e.g. `aura-2-livia-it`)                  |
| `aura-2-<voice>-ja` (5 Japanese voices, e.g. `aura-2-fujin-ja`)                  |
| `aura-2-<voice>-fr` (2 French voices, e.g. `aura-2-agathe-fr`)                   |
| `aura-<voice>-en` (12 Aura-1 English voices, e.g. `aura-asteria-en`)             |
