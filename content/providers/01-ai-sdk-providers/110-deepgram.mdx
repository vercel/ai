...
<Note type="warning">
  **Breaking change in `@ai-sdk/deepgram` 3.1:** full voice model IDs (e.g.
  `deepgram.speech('aura-2-helena-en')`) are no longer accepted. Migrate by
  passing the family ID and selecting the voice and language via options:
  `deepgram.speech('aura-2')` with `voice: 'helena'`, `language: 'en'`.
</Note>
