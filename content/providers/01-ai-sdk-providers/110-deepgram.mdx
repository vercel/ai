...
<Note>
  Full voice model IDs (e.g. `deepgram.speech('aura-2-helena-en')`) still
  pass through, but the family ID + `voice`/`language` form above is the
  recommended path — it matches voice selection in the other speech
  providers.
</Note>
