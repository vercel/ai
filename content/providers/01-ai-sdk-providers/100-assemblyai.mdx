...
The legacy `best` and `nano` models are sent using AssemblyAI's deprecated
`speech_model` request parameter. All newer models (e.g. `universal-3-5-pro`) are sent using the
[`speech_models`](https://www.assemblyai.com/docs/pre-recorded-audio/select-the-speech-model)
