...
**Not Supported:**
- `/sync/v1` endpoints (to avoid double `/v1` issue with Performance Client)
...
// /sync/v1 endpoints are not supported for embeddings (to avoid double /v1)
try {
  const baseten = createBaseten({
    modelURL:
      'https://model-xyz123.api.baseten.co/environments/production/sync/v1',
  });
  baseten.textEmbeddingModel(); // This will throw an error
} catch (error) {
  // Error: "Not supported. You must use a /sync endpoint for embeddings."
}
