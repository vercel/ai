...
Use `cohere.evaluationModel('embed-v4.0')` for experimental Choice evaluation
...
const model = cohere.evaluationModel('embed-v4.0');
...
Cohere evaluation uses embeddings, with `embed-v4.0` suggested by autocomplete;
custom embedding IDs are also accepted. Defaults are `inputType: 'classification'`
and `truncate: 'NONE'`. Override embedding options in `providerOptions.cohere`,
for example `outputDimension: 1024`. Overriding the input type changes the
embedding task and requires revalidating classification quality.
