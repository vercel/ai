...
You can attach billing labels to Google Vertex Anthropic requests through the `vertex` provider options:
...
const { text } = await generateText({
  ...
  prompt: 'Write a short summary of this document.',
  providerOptions: {
    vertex: {
      labels: {
        team: 'data-platform',
...
The labels are JSON-encoded and sent as a base64-encoded value in the `X-Vertex-AI-Labels` request header. `providerOptions.googleVertex.labels` is also supported for consistency with the provider name.
