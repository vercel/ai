...
Use a `file-url` part with a full `mediaType` in the tool's `toModelOutput` result:
...
The Vertex caller must have access to the referenced object. Direct GCS
tool-result forwarding is not supported for older Gemini models, missing or
wildcard MIME types, or MIME types outside the list above.

HTTP(S) tool-result files are downloaded and sent as inline data, including when
the file part specifies a `mediaType`.
...
| ----------------------- | ------------------- | ------------------- | ------------------- | ------------------- |
| `gemini-3.8-flash`      | <Check size={18} /> | <Check size={18} /> | <Check size={18} /> | <Check size={18} /> |
...
```ts
const result = await transcribe({
  model: vertex.transcription('chirp_3'),
  audio: await readFile('audio.wav'),
  providerOptions: {
    googleVertex: {
      region: 'us',
