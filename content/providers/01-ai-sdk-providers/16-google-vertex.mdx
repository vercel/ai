...
- **responseLogprobs** _boolean_

  Optional. If true, returns log probabilities for the generated tokens in `providerMetadata`.

- **logprobs** _number (1-20)_

  Optional. Only valid if `responseLogprobs` is true.
  Specifies the number of top logprobs to return at each decoding step.

  <Note>
    Logprobs are only supported with `generateText`, not `streamText`.
  </Note>

  Consult [Google's Documentation](https://cloud.google.com/vertex-ai/generative-ai/docs/reference/rest/v1/GenerationConfig) for usage details.
...
Google Vertex AI supports returning log probabilities for generated tokens. This is useful for analyzing model confidence, filtering outputs, or evaluating generation quality.
...
// Access logprobs from providerMetadata
const logprobsResult = providerMetadata?.vertex?.logprobsResult;
const avgLogprobs = providerMetadata?.vertex?.avgLogprobs;

console.log('Chosen tokens:', logprobsResult?.chosenCandidates);
console.log('Top candidates:', logprobsResult?.topCandidates);
console.log('Average logprobs:', avgLogprobs);
```

The `logprobsResult` contains:

- **chosenCandidates**: Array of chosen tokens with their log probabilities
- **topCandidates**: Array of decoding steps, each with top-K candidate tokens

<Note>
  Logprobs are only supported with `generateText`, not `streamText`. Gemini 3
  models require the `global` location.
</Note>
