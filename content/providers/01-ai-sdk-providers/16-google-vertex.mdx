...
[Vertex AI Search grounding](https://cloud.google.com/vertex-ai/generative-ai/docs/grounding/grounding-with-vertex-ai-search)
retrieves information from a Vertex AI Search data store or engine. Provide one
fully-qualified `datastore` or `engine` resource name:
...
const result = await generateText({
  ...
  tools: {
    vertex_ai_search: googleVertex.tools.vertexAiSearch({
      datastore:
...
When using an engine, you can additionally configure `dataStoreSpecs` to apply
filters to individual data stores.

#### External API Grounding

[External API grounding](https://cloud.google.com/vertex-ai/generative-ai/docs/grounding/grounding-with-your-search-api)
lets Gemini retrieve information from a search endpoint that implements
Vertex AI's simple search or Elasticsearch-compatible specification:
...
const result = await generateText({
  ...
  tools: {
    external_api: googleVertex.tools.externalApi({
      ...
      authConfig: {
        apiKeyConfig: { apiKeyString: process.env.SEARCH_API_KEY! },
      },
    }),
  },
  prompt: 'What is our return policy?',
...
For an Elasticsearch-compatible endpoint, set `apiSpec` to `ELASTIC_SEARCH`
and provide `elasticSearchParams` with the index, search template, and number
of hits.
