...
[Fine-tuned (tuned) Gemini models](https://cloud.google.com/vertex-ai/generative-ai/docs/deploy/overview)
are deployed to an endpoint and are addressed by their endpoint id rather than a
published model name. Pass the endpoint resource with the `endpoints/` prefix and
the provider will route the request to the deployed endpoint
...
base model the endpoint was tuned from, so all standard call settings and
provider options continue to work.

<Note>
  Fine-tuned models require standard Google Cloud credentials and the
  project/location of the deployed endpoint (via `GOOGLE_VERTEX_PROJECT` /
  `GOOGLE_VERTEX_LOCATION` or `createGoogleVertex`). [Express
  Mode](#express-mode) API keys are not supported for endpoint models.
</Note>
