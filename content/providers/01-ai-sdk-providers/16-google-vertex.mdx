...
published model name. Prefix the endpoint id with `finetuned/` and the provider
will route the request to the deployed endpoint
...
The request and response format is identical to the base model the endpoint was
tuned from, so all standard call settings and provider options continue to work.
...
<Note>
  ...
  Mode](#express-mode) API keys are not supported for fine-tuned models.
</Note>
