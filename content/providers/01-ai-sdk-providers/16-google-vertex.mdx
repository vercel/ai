...
3. **Web Search Tool**: Provides access to real-time web content.

<Note>
  Only a subset of Anthropic tools are supported on Google Vertex. Tools like
  Computer Use, Code Execution, Memory, and Web Fetch are not available. Use the
  regular `@ai-sdk/anthropic` provider if you need access to all Anthropic
  tools.
</Note>
