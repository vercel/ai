...
<Note>
  For projects in a Google Cloud organization, you must enable the
  [`structured_outputs` partner model feature](https://docs.cloud.google.com/gemini-enterprise-agent-platform/models/control-model-access#enable-structured-outputs)
  for the model to use native structured outputs or strict tools.
</Note>
...
<Note>
  Google Vertex Anthropic supports strict mode on tool definitions. Setting
  `strict: true` on a tool passes the strict setting through to Vertex.
</Note>
