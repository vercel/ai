...
Legacy Sonar model IDs select Agent API presets rather than the original Sonar
models. A preset can choose a different underlying model and set of tools, so
its cost, latency, and output can differ from the legacy model with the same
alias.
