...
- **zeroDataRetention** _boolean_
  ...
  Restricts routing requests to providers that have zero data retention agreements with Vercel for AI Gateway. BYOK credentials are skipped when `zeroDataRetention` is set to `true` to ensure that requests are only routed to providers that support ZDR compliance.
...
const { text } = await generateText({
  model: 'anthropic/claude-sonnet-4.6.6',
