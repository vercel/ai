...
- **models** _GatewayModelFallback[]_

  Specifies fallback models in attempt order. String entries keep their existing behavior: the gateway tries them when the primary model fails or is unavailable. Evaluation requests can also use one conditional `{ model, when }` entry to retry a successful but uncertain evaluation with another model. If the primary model fails, the gateway also advances to the conditional entry under its existing error fallback behavior. Conditional entries are rejected on non-evaluation requests.
  ...
  Combine conditions with `{ any: [...] }`, `{ all: [...] }`, or `{ atLeast: { count, conditions: [...] } }`. Each condition list must be nonempty. `atLeast.count` must be an integer from `1` through the number of conditions. Gateway runtime validation enforces question kinds, numeric bounds, nesting depth, and the v1 limit of one conditional entry. String error fallbacks may follow that entry.
