...
const gatewayMetadata = result.providerMetadata?.gateway as
  | GatewayProviderMetadata
  | undefined;
for (const attempt of gatewayMetadata?.routing?.modelAttempts ?? []) {
  console.log(attempt.canonicalSlug, attempt.triggeredBy, attempt.usage);
}
```

When the condition matches, `providerMetadata.gateway.routing.modelAttempts`
lists the primary and fallback attempts. The successful attempt of each stage
carries its own `generationId`, `usage`, and cost fields, and the first
fallback attempt lists the questions that triggered it in `triggeredBy`.
