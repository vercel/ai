...
Successful items from `experimental_getBatchResults` include ordered, Core-normalized `content` parts for text, reasoning, files, sources, tools, custom content, and part-level provider metadata. The `text` property remains a convenience concatenation of text parts and is an empty string for non-text-only results. Retrieval parses tool inputs when possible, but it does not execute tools, submit tool results, or fetch remote files. Reasoning, tool payloads, and provider metadata can contain sensitive or untrusted data, so avoid logging `content` indiscriminately.
