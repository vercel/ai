...
The Tako Search tool enables models to retrieve [source-grounded data cards and live web results](https://docs.tako.com/documentation/integrating-tako/search/overview) through the AI Gateway. Use it for current financial, economic, company, sports, demographic, and web research when structured data or citations are useful.
...
- **sources.data** _object_ - Configure curated data cards with `count`, `nodeIds`, `strict`, `contentFormat`, and `maxRows`. `strict` requires at least one `nodeIds` value.
- **sources.web** _object_ - Configure web results with `count`, `category`, `includeDomains`, `excludeDomains`, `publishedAfter`, `publishedBefore`, and result/content character limits.
- **includeContents** _boolean_ - Available on each source. It inlines the underlying card data or extracted web content so the model can reason over it. Inline data may add a usage-based charge.
