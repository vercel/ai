...
- **requireModelTags** _Array&lt;'implicit-caching'&gt;_

  Restricts routing to provider models that carry all of the specified capability tags. Currently supports `'implicit-caching'`, which limits routing to models that perform automatic (implicit) prompt caching. Applies to both BYOK and system credentials, since the capability is a property of the model rather than the credential. If no provider model for the requested model satisfies the tags, the request fails. Unsupported tag values are rejected.

  Example: `requireModelTags: ['implicit-caching']` will only route to models that support implicit caching.
...
#### Require Model Tags Example

Set `requireModelTags` to restrict routing to provider models that carry the specified capability tags. This applies to both BYOK and system credentials, since the capability is a property of the model rather than the credential. Currently `'implicit-caching'` is supported, which limits routing to models that perform automatic prompt caching. If no provider model for the requested model satisfies the tags, the request fails.
