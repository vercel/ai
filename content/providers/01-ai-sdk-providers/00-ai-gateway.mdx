...
Use it with the same setup-endpoint and `experimental_useRealtime` flow as any other realtime provider (see [Realtime](/docs/ai-sdk-core/realtime)). Your server creates a short-lived token, and the browser connects through the Gateway:
...
<Note>
  ...
  auth token is sent via the `Sec-WebSocket-Protocol` subprotocol and the model
  id rides the `?ai-model-id=` query — the WebSocket transport of the
  `Authorization` and `ai-model-id` headers the HTTP routes use.
</Note>
...
The `GatewayProviderOptions` type and a `parseGatewayProviderOptions` validator are exported from `@ai-sdk/gateway` so the same shape is shared between the client (which sets the options) and any server that reads them (e.g. validating the `providerOptions.gateway` carried on a `session.update`):

```ts
import { parseGatewayProviderOptions } from '@ai-sdk/gateway';

// returns typed GatewayProviderOptions | undefined, throws on invalid input
const options = await parseGatewayProviderOptions(sessionConfig.providerOptions);
```
