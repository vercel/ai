...
Artifact generation is experimental and currently uses the AI SDK v7 Gateway
protocol. Once an artifact model is enabled in your Gateway catalog, use
`.artifact()` to call it. For example, a catalog entry for Fal's text-to-3D
endpoint can be called as follows:
...
const { artifact } = await generateArtifact({
  model: gateway.artifact('fal/tripo3d/h3.1/text-to-3d'),
  prompt: 'A low-poly wooden airship',
});

console.log(artifact.filename, artifact.mediaType);
```

The Gateway transport preserves multiple artifact URLs or payloads together
with their media types, filenames, and roles. Long-running requests use an SSE
heartbeat while the provider's accepted job is polled.
