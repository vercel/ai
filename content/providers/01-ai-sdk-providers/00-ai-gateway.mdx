...
You can query usage data programmatically using the [Advanced Reporting API](https://vercel.com/docs/ai-gateway/capabilities/advanced-reporting). The reporting API is only available for Vercel Pro and Enterprise plans. The API supports grouping by `model`, `user`, `tag`, `provider`, or `credential_type`, and filtering by date range, user, model, tags, and more.
