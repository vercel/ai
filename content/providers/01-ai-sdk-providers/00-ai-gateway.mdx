...
The `model` passed to `startTextBatch` is the default for all requests. AI
Gateway currently requires every request in a batch to use the same model and
throws before submission when request-level model overrides differ.
