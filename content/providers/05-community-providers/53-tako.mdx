...
[Tako](https://tako.com) is a live-data search engine. A single call searches both the live web
and Tako's own structured data, returning web results alongside knowledge cards — charts and
metrics that carry their sources and freshness metadata. The
[`@takoviz/ai-sdk`](https://www.npmjs.com/package/@takoviz/ai-sdk) package integrates Tako with
the AI SDK as a set of tools.

It covers what a web search tool covers, and adds a structured layer on top. For a question
that turns on a number, you get the value, the source it came from, how current it is, and a
chart you can render — instead of prose to parse. That layer is deepest in:

- Finance and markets, company KPIs, and earnings
- Macroeconomics — inflation, unemployment, GDP, policy rates
- Website and app traffic, demographics, energy, real estate, health
- Sports, weather, elections, and prediction markets

Both surfaces are configurable per call, so you can search the web, the structured data, or
both. Learn more in the [Tako documentation](https://docs.tako.com).
...
| Tool             | What it does                                                       |
| ---------------- | ------------------------------------------------------------------ |
| `takoSearch()`   | Retrieval: Tako knowledge cards plus web results, no synthesis     |
| `takoAnswer()`   | Retrieval plus an LLM-synthesized, cited answer                    |
| `takoContents()` | Downloads the data behind a result — a card's rows or a page's text |
