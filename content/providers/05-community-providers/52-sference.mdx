...
models (GLM, DeepSeek, Kimi, and more), exposing an OpenAI-compatible
...
- **Open-weight models, managed**: run GLM, DeepSeek, Kimi and friends
...
const model = sference("deepseek-ai/DeepSeek-V4.1-Flash");
const model = sference.chat("deepseek-ai/DeepSeek-V4.1-Flash");
const model = sference.languageModel("deepseek-ai/DeepSeek-V4.1-Flash");
```
...
sference surfaces chain-of-thought from reasoning models (DeepSeek, Kimi, GLM,
…) as AI SDK `reasoning` parts. Control it with the standard AI SDK `reasoning`
option — `none` disables thinking, any other level enables it. (Exception: the
GLM-5.3 family always reasons — disabling thinking there is rejected with a
400.)
