...
Here is an example implementation of a chat application that uses both the AI SDK and a composed LangChain chain together with the
...
It takes this stream and uses the AI SDK's [`StreamingTextResponse`](/docs/reference/stream-helpers/streaming-text-response)
to pipe text to the client and then the AI SDK's `useChat` to handle the chat UI.
...
Then, we use the AI SDK's [`useChat`](/docs/reference/ai-sdk-ui/use-chat) method:
