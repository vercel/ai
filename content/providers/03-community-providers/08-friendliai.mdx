...
- **hostedTools** _Array_
- **hostedTools.type** _string_

  Optional. The hosted tools to use with the model. The available types are `calculator` and `web_search`.
...
const friendliai = createFriendliAI({
  baseURL: 'https://inference.friendli.ai/dedicated/v1',
  apiKey: process.env.FRIENDLI_API_KEY,
...
If you are using `@friendliai/ai-provider`, you can use the `hostedTools` option.
