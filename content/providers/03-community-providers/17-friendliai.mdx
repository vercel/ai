...
It creates language model objects that can be used with the `generateText` and `streamText` functions, and supports structured data generation with `Output`.
...
FriendliAI language models can also be used in the `streamText` function and support structured data generation with `Output`
