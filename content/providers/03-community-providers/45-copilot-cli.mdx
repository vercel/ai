...
<Note>
  Model availability depends on your GitHub Copilot subscription tier (Free, Pro, Pro+, Business, Enterprise).
</Note>
...
<Note>
  Object generation shows ❌ because this provider uses prompt-based structured
...
```ts
import { createCopilotProvider } from 'ai-sdk-provider-copilot';
import { createMemoryCache, wrapWithCache } from 'ai-sdk-provider-copilot/cache';
import { generateText } from 'ai';
...
const copilot = createCopilotProvider({
  telemetry: {
    ...
    recordInputs: false, // Set to true to record prompts (careful with PII)
