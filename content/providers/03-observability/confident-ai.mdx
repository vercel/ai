...
<Note>
  For users in the EU region set `CONFIDENT_OTEL_ENDPOINT="https://eu.otel.confident-ai.com/v1/traces"`
</Note>

### 3. Initialize Tracing

Call `init()` once when your app starts, before any AI SDK call. You don't have to create a tracer or pass any `telemetry` options to your calls:
...
Then launch your entry-point file with the `confident-trace/register` preload so the SDK can hook the `ai` package as Node loads it. `init()` handles export, the preload handles instrumentation — you need both:

```bash
node --import tsx --import confident-trace/register index.ts
```
...
```json filename="package.json"
{
  "scripts": {
    ...
    "dev": "node --import tsx --import confident-trace/register index.ts"
  }
}
```

Your AI SDK calls stay exactly as they are. Every span carries the `Vercel AI SDK` integration label, nests under whatever span is active and captures:
...
- **No traces at all:** confirm your start command includes `--import confident-trace/register` and that `init()` runs before the first AI SDK call. `runtime.getInstrumentationStatus()` reports whether the `ai` hook attached — `not observed` means the package had not been loaded yet.
