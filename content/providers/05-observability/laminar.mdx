...
export async function register() {
  ...
  if (process.env.NEXT_RUNTIME === 'nodejs') {
    const { registerTelemetryIntegration } = await import('ai');
    const { OpenTelemetryIntegration } = await import('@ai-sdk/otel');
    const { Laminar, getTracer } = await import('@lmnr-ai/lmnr');

    Laminar.initialize({
      projectApiKey: process.env.LMNR_PROJECT_API_KEY,
    });
    registerTelemetryIntegration(new OpenTelemetryIntegration({ tracer: getTracer() }));
  }
}
```
