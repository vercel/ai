...
This must be done once in your application, as early as possible, but _after_ other tracing libraries (e.g. `@vercel/otel`, `@sentry/node`) are initialized.

Read more in Laminar [docs](https://docs.lmnr.ai/tracing/introduction#1-get-your-project-api-key).
...
Laminar can work with `@vercel/otel` to trace AI SDK calls. The default Laminar setup is the following:
...
- regular Next.js traces are sent via `@vercel/otel` to your Telemetry backend such as Datadog or New Relic,
...
Laminar can work with `@sentry/node` to trace AI SDK calls. The default Laminar setup is the following:
