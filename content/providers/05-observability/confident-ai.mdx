...
Register the `OpenTelemetry` globally at application startup so the AI SDK emits OpenTelemetry spans that Confident AI can collect:
...
registerTelemetryIntegration(new OpenTelemetry({ tracer }));
