...
[Confident AI](https://confident-ai.com/) is an LLM observability and evaluation platform for monitoring and improving AI applications in both development and production.
...
The `deepeval-ts` package provides a `configureAiSdkTracing` helper that returns a tracer compatible with the AI SDK's telemetry API. Here's how you can setup observability for your LLM application in 3 simple steps:
...
To set up tracing with Confident AI, you can simply import and call the `configureAiSdkTracing` function to get a `tracer`:
...
Pass the `tracer` into the `experimental_telemetry` field of any AI SDK call to record your traces in Confident AI.
