...
Install `@ai-sdk/otel` and register the `OpenTelemetryIntegration` at the top of your route files. The AI SDK emits a span only when you opt in with `experimental_telemetry`:
...
export async function POST(req: Request) {
  ...
  const otelTracer = trace.getTracer('next-app');

  return await otelTracer.startActiveSpan('chat-evaluate', async span => {
    try {
      /* 1️⃣ generate answer */
