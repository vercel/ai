...
Install `@ai-sdk/otel` and register the `OpenTelemetryIntegration` at the top of your route files to enable OpenTelemetry span collection, then enable telemetry on each request that you want to trace:
...
- You need to be on `"ai": "^7.0.0"` and install `@ai-sdk/otel` to use the telemetry feature. In case of any issues, please update to the latest version.
