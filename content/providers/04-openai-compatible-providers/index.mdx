...
When the backend returns a `usage` object in the images response, it is exposed through `result.usage` as `inputTokens`, `outputTokens` and `totalTokens`. This works for both image generation and image editing.
...
Many OpenAI-compatible image endpoints do not report token counts. In that case `usage` is `undefined`, which is expected rather than an error.
