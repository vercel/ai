...
- **supportsBatch** _boolean_

  Set to `true` only if the provider implements the OpenAI-style Files and
  Batches APIs and accepts JSONL requests for `/v1/chat/completions`.
...
Some OpenAI-compatible providers support asynchronous text generation through
the OpenAI-style Files and Batches APIs. Enable batch support explicitly because
OpenAI compatibility does not include these endpoints by default.
...
to check status or retrieve results from another process. Results can arrive in
a different order from the input requests, so match each result by its `id`.

The shared implementation requires OpenAI-style `/files` and `/batches`
endpoints. It uploads JSONL lines for `/v1/chat/completions` and reads OpenAI-style
output and error files. A provider with a different batch protocol needs its own
batch model implementation.

When you provide a `webhookUrl`, the provider returns an unsupported warning and
starts the batch without a webhook.
