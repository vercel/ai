---
title: Send a PDF to an LLM
description: Learn how to send a PDF to an LLM using the AI SDK and Next.js
tags: ['next', 'multi-modal']
---

# Sending a PDF to an LLM

<Note>
  This will only work with models that support PDFs. Currently limited to select
  models using the
  [Anthropic](/providers/ai-sdk-providers/anthropic#pdf-support) and
  [Google](/providers/ai-sdk-providers/google-generative-ai#file-inputs)
  providers.
</Note>

With select models, you can send PDFs as part of your prompt. Let's create a simple Next.js application that allows a user to upload a PDF send it to an LLM for summarization.
