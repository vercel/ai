...
<Note type="warning">
  The approval state is replayed through client-controlled message history, so
  by default it is a UX affordance, not a security boundary — a malicious client
  can forge an approval and run the tool with schema-valid arguments. For tools
  that perform sensitive actions, set `experimental_toolApprovalSecret` to bind
  approvals to the server that issued them. See
  [Tool Approvals → Security Considerations](/docs/agents/tool-approvals#security-considerations).
</Note>
