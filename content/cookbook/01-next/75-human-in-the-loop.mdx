...
On the backend, create a route handler (API Route) that returns a `DateStreamResponse`. Within the execute function, call streamText, passing in the messages (sent from the client), finally merge the resulting generation into the data stream.
