...
description: Build a custom memory tool with filesystem persistence and system prompt injection, using either structured actions or a bash-backed interface.
---
...
2. **Zod v4** for tool input schemas
3. **Optional for Route B (bash-backed)**: **[just-bash](https://github.com/nichochar/just-bash)** for command execution and AST parsing

Install dependencies for both routes:

```bash
bun add ai just-bash zod
```
...
- Keep memory operations invisible in user-facing replies.`,
  ...
  execute: async input => {
    try {
      ...
      return { output: `Memory action failed: ${error.message}` };
    }
  },
});
```
...
When the model decides it needs to store or recall information, it calls the `memory` tool with a bash command. The `ToolLoopAgent` executes the tool and feeds the result back, continuing until the model produces a final text response.
...
- [just-bash](https://github.com/nichochar/just-bash) for the JavaScript-based bash interpreter and AST parser
...
### Appendix: Helper Functions {#appendix-helper-functions}
