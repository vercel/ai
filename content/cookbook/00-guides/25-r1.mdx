...
<Note>
  DeepSeek's `deepseek-reasoner` API alias no longer serves DeepSeek R1. It
  currently maps to `deepseek-v4-flash` in thinking mode and will be retired on
  2026-07-24 at 15:59 UTC. This guide is retained as a reference for the R1
  model family; use a third-party provider that still serves R1 to run its R1
  examples.
</Note>
