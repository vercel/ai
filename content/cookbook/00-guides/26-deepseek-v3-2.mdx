...
<Note>
  DeepSeek's `deepseek-chat` and `deepseek-reasoner` API aliases no longer serve
  DeepSeek V3.2. They currently map to `deepseek-v4-flash` and will be retired
  on 2026-07-24 at 15:59 UTC. This guide is retained as a reference for DeepSeek
  V3.2; use `deepseek-v4-flash` or `deepseek-v4-pro` for the latest DeepSeek API
  models.
</Note>
