...
<Note>
  Get your API key from the [Zenrows
  dashboard](https://app.zenrows.com/account/settings).
</Note>

First, install the Zenrows AI SDK tools:

```bash
pnpm install @zenrows/ai-sdk-tools
```

Then, you can import and pass the tools into `generateText`, `streamText`, or your agent. Zenrows provides three tools: `scrapeUrl` returns a page as Markdown, `extractData` returns named fields as JSON, and `takeScreenshot` returns the rendered page as an image. All three handle JavaScript rendering and anti-bot protection, so the same call works on pages that block ordinary requests:
...
For more configuration options and customization, see the [Zenrows AI SDK tools documentation](https://github.com/ZenRows/vercel-zenrows#readme).
