...
<Note>
  Get your API key from the [Valyu Platform](https://platform.valyu.ai).
</Note>

First, install the Valyu AI SDK tools:

```bash
pnpm install @valyu/ai-sdk
```

Then, you can import and pass it into `generateText`, `streamText`, or your agent. Valyu provides `webSearch` for general web search, plus specialized domain-specific tools including `financeSearch`, `paperSearch`, `bioSearch`, `patentSearch`, `secSearch`, `economicsSearch`, and `companyResearch`:
...
For more customization options, visit the [Valyu AI SDK Documentation](https://docs.valyu.ai/integrations/vercel-ai-sdk). You can also try the tools without writing any code with our open-source [AI SDK Playground](https://ai-sdk.valyu.ai).
...
const { text } = await generateText({
  model: 'anthropic/claude-sonnet-4.5', // can be any model that supports tools
  prompt: 'What happened in San Francisco last week?',
  tools: {
    webSearch,
  },
...
[Valyu](https://valyu.ai) provides an API for searching across web and proprietary data sources. Let's look at how you can build a custom search tool using Valyu:

```bash
pnpm install valyu-js
```
