...
<Note>Get your API key from the [Tavily Dashboard](https://app.tavily.com).</Note>
...
const { text } = await generateText({
  ...
  prompt: 'When was the latest update to Vercel AI SDK?',
  tools: {
...
For more customization options over your agent's web-access functionality, visit the [Tavily AI SDK Documentation](https://docs.tavily.com/integrations/vercel).
