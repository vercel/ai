...
In this example, you'll learn how to stream responses along with tool calls from OpenAI's [Assistant](https://platform.openai.com/docs/assistants/overview) using `ai/rsc`.

## Client

In your client component, you will create a simple chat interface that allows users to send messages to the assistant and receive responses. The assistant's responses will be streamed in two parts: the status and the text. The status will be displayed as a message status, and the text will be displayed as the assistant's text response.
...
In your server actions, you will create a function that sends a user's message to the assistant and streams the assistant's response. The function will return the status and text of the assistant's response.

The run queue is used to manage multiple runs in the same thread. The function will create a new thread if one does not exist and send the user's message to the assistant. If a thread already exists, the function will send the user's message to the assistant in the existing thread. The function will then stream the assistant's response and update the status and text of the assistant's response.
