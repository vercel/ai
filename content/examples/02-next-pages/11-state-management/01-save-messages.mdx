...
You can save the chat to an external databse using the `onFinish` callback handler present in the `streamText` function.
...
> **Note**: This example assumes that the database table schema has a `messages` column that stores the chat messages as a JSON string as `Array<CoreMessage>`. You can modify the `saveChat` function to suit your database schema.
