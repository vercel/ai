...
> **Note**: This example assumes that the database table schema has a `messages` column that stores the chat messages as a JSON string as `Array<CoreMessage>`. You can modify the `convertToUIMessages` function to suit your database schema.
