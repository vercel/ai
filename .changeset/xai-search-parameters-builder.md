...
Add `xaiSearchParameters()` — a typed builder for xAI Live Search config on Grok models. Returns a `searchParameters` object (with `mode` defaulted to `'on'`, matching xAI's documented default) to pass to `providerOptions.xai.searchParameters`. Use `maxSearchResults` to bound cost, plus typed `sources` for `web` / `x` / `news` / `rss` filters.
