...
Because the gateway proxies to heterogeneous backends that accept different parameter subsets, the provider detects the model family and drops parameters a backend is known to reject (e.g. penalties/`seed` for Anthropic and Llama, `temperature`/`topP`/`stop` for `gpt-5` reasoning models, `reasoningEffort` for Anthropic/Gemini), surfacing an AI SDK warning instead of failing the request. Unknown models are passed through unchanged.
