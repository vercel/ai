...
- `getServerConnection({ intent? })` returns the upstream WebSocket `{ url, headers, protocols? }` for a server-initiated connection (header auth with the long-lived credential), covering `conversation` / `transcription` / `translation` endpoint variants. It is the server-side sibling of the browser-only `getWebSocketConfig`.
