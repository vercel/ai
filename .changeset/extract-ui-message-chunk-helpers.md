...
The new `toUIMessageStreamResponse` and `pipeTextStreamToUIMessageStreamResponse` helpers turn a `streamText` `fullStream` into a UI message stream response directly. This lets `result.toUIMessageStreamResponse(options)` and `result.pipeUIMessageStreamToResponse(response, options)` migrate to standalone helper calls with `stream: result.fullStream`.
