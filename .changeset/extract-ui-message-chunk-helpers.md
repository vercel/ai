...
The new `pipeTextStreamToUIMessageStreamResponse` helper pipes a `streamText` `fullStream` to a Node.js response directly, so `result.pipeUIMessageStreamToResponse(response, options)` can be migrated to a single standalone helper call.
