...
- When adding a harness, you must add all example files for that harness that the other already existing harness adapters include.
  ...
  3. Make sure the file contents are identical, except for the specific harness adapter import and usage, and occasional harness-specific nuances (e.g. whether or not to configure open ports in the sandbox used).
