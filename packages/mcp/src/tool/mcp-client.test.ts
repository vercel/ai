...
describe('MCPClient', () => {
  ...
  describe('elicitation', () => {
    it('should advertise elicitation capability when callback provided', async () => {
      const mockTransport = new MockMCPTransport();
      ...
      client = await createMCPClient({
        transport: mockTransport,
        elicitation: {
          onCreate: async () => ({ action: 'accept', content: {} }),
        },
      });

      // Check that initialize was called with elicitation capability
      const initializeCall = sendSpy.mock.calls.find(
        call =>
          'method' in call[0] &&
          call[0].method === 'initialize' &&
          'params' in call[0],
      );
      expect(initializeCall).toBeDefined();
      const params = (initializeCall![0] as any).params;
      expect(params.capabilities).toHaveProperty('elicitation');
      expect(params.capabilities.elicitation).toEqual({});
    });

    it('should not advertise elicitation capability when callback not provided', async () => {
      const mockTransport = new MockMCPTransport();
      const sendSpy = vi.spyOn(mockTransport, 'send');
      createMockTransport.mockImplementation(() => mockTransport);
      ...
      // Check that initialize was called without elicitation capability
      const initializeCall = sendSpy.mock.calls.find(
        call =>
          'method' in call[0] &&
          call[0].method === 'initialize' &&
      ...
      expect(initializeCall).toBeDefined();
      const params = (initializeCall![0] as any).params;
      expect(params.capabilities).not.toHaveProperty('elicitation');
    });

    it('should handle elicitation/create request and call callback', async () => {
      const mockTransport = new MockMCPTransport();
      const sendSpy = vi.spyOn(mockTransport, 'send');
      createMockTransport.mockImplementation(() => mockTransport);

      const onCreateMock = vi.fn(async () => ({
      ...
      // Simulate server sending elicitation/create request
      const requestId = 123;
      ...
      // Verify response was sent back to server
      const responseCall = sendSpy.mock.calls.find(
        call =>
          'id' in call[0] && call[0].id === requestId && 'result' in call[0],
      );
      expect(responseCall).toBeDefined();
      const response = responseCall![0] as any;
    ...
    it('should return error when elicitation/create called without handler', async () => {
      const mockTransport = new MockMCPTransport();
      const sendSpy = vi.spyOn(mockTransport, 'send');
      createMockTransport.mockImplementation(() => mockTransport);
      ...
      const errorCall = sendSpy.mock.calls.find(
        call =>
          'id' in call[0] && call[0].id === requestId && 'error' in call[0],
      );
      expect(errorCall).toBeDefined();
      const error = errorCall![0] as any;
      expect(error.error.code).toBe(-32000);
      expect(error.error.message).toBe('Method not supported');
    });
