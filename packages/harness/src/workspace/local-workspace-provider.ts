 ...
 * project directory that already exists and outlives them. `resumeSession`
 * therefore just rebinds to the same root.
 *
 * Harness SDK state (bootstrap recipes and their dependencies, per-session
 * run state) resolves through `harnessV1StateDirectory()` like every other
 * provider — a fixed, shared `~/.ai-sdk-harness` root, never the project.
 * This provider additionally keeps its own per-project bookkeeping (a
 * `manifest.json` mapping the project back from its store, and the
 * `onFirstCreate` idempotency marker) in a project-keyed subdirectory of
 * that same root, so a project's setup never re-runs and can be found again
 * later.
 *
...
export class LocalWorkspaceProvider implements HarnessV1SandboxProvider {
  ...
  createSession = async (options?: {
    ...
    await this.runFirstCreateOnce({
      // One-time setup runs rooted at this project's own state directory,
      // never the project, so a caller's relative-path side effects land in
      // the store. The harness's own bootstrap recipe resolves through
      // `harnessV1StateDirectory()` instead, independent of this cwd — see
      // that function's doc for why. The child set is shared so anything the
      // hook spawns is reaped with the session.
