...
// The state root is not configurable in production; tests redirect it by
// mocking `homedir()` instead, so every test's state stays out of the
// developer's real ~/.ai-sdk-harness.
...
describe('the central state store', () => {
  it('keeps every project store under the state root, never the project', async () => {
    const { projectPath } = await createTempProject();
    await startSession({ path: projectPath });

    const stateDirectory = localWorkspaceStateDirectory(projectPath);
    expect(stateDirectory).toContain(stateRoot);
    expect(stateDirectory).not.toContain(projectPath);
  ...
  it('keys the store by basename and path hash, mappable back via manifest.json', async () => {
    ...
    const stateDirectory = localWorkspaceStateDirectory(projectPath);
    expect(basename(stateDirectory)).toMatch(/^my-app-2024-[0-9a-f]{8}$/);

    const manifest = JSON.parse(
      readFileSync(join(stateDirectory, 'manifest.json'), 'utf8'),
    );
  ...
  it('gives unrelated projects with the same basename distinct stores', async () => {
    ...
    await startSession({ path: alpha.projectPath });
    await startSession({ path: beta.projectPath });

    expect(localWorkspaceStateDirectory(alpha.projectPath)).not.toBe(
      localWorkspaceStateDirectory(beta.projectPath),
    );
  });
