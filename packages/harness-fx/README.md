...
fx uses Vercel AI Gateway. Set `AI_GATEWAY_API_KEY` or `VERCEL_OIDC_TOKEN` to authenticate, or pass a programmatically resolved credential with `createFx({ auth: { gateway: { apiKey } } })`.
