...
describe('custom provider name support', () => {
  ...
  describe('doGenerate', () => {
    it('should accept providerOptions with custom provider name key', async () => {
      ...
      // Verify model.provider uses custom name
      expect(customModel.provider).toBe('my-custom-openai.chat');
      ...
      // Verify the user option was sent to the API
    ...
    it('should accept providerOptions with canonical "openai" key for backward compatibility', async () => {
      ...
      await customModel.doGenerate({
        prompt: TEST_PROMPT,
        providerOptions: {
          // Canonical key should still work
          openai: {
    ...
    it('should give precedence to custom provider name key over canonical "openai" key', async () => {
      prepareCustomJsonResponse();
      ...
      await customModel.doGenerate({
        prompt: TEST_PROMPT,
        providerOptions: {
          // Both keys provided - custom should win
          openai: {
            user: 'canonical-user-id',
          },
          'my-custom-openai': {
            user: 'custom-user-wins',
          },
        },
      });
    ...
    it('should include custom provider name key in providerMetadata output', async () => {
      ...
      // Both keys should have the same data
      expect(result.providerMetadata?.openai).toStrictEqual({
        acceptedPredictionTokens: 10,
        rejectedPredictionTokens: 5,
      });
      expect(result.providerMetadata?.['my-custom-openai']).toStrictEqual({
        acceptedPredictionTokens: 10,
        rejectedPredictionTokens: 5,
      });
    });

    it('should only include "openai" key when using default provider name', async () => {
      prepareCustomJsonResponse({
        usage: {
          prompt_tokens: 4,
          total_tokens: 34,
      ...
      // Default provider (name = 'openai')
      const defaultProvider = createOpenAI({
        apiKey: 'test-api-key',
      });
      const defaultModel = defaultProvider.chat('gpt-4o-mini');

      const result = await defaultModel.doGenerate({
        prompt: TEST_PROMPT,
      });

      // Should only have 'openai' key, no duplicate
      expect(result.providerMetadata).toHaveProperty('openai');
  ...
  describe('doStream', () => {
    ...
    it('should include custom provider name key in providerMetadata in finish chunk', async () => {
      ...
      expect(finishChunk?.providerMetadata?.['my-custom-openai']).toStrictEqual(
        {
          acceptedPredictionTokens: 10,
          rejectedPredictionTokens: 5,
        },
