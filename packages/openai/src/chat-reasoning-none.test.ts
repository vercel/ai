  ...
  method => {
    it.each(cases)('$name', async testCase => {
      ...
      const response = isStreaming
        ? readFileSync('src/chat/__fixtures__/openai-text.chunks.txt', 'utf8')
            .split('\n')
            .filter(Boolean)
            .map(line => `data: ${line}\n\n`)
            .join('')
        : readFileSync('src/chat/__fixtures__/openai-text.json', 'utf8');
      ...
      const model = createOpenAI({ apiKey: 'test', fetch }).chat(
        testCase.modelId,
      );
