...
export function isEqual<Left extends unknown, Right extends unknown>(
  _expect: TIsEqual<Left, Right>,
) {}
...
describe('typescript', () => {
  ...
  it('should validate valid object', async () => {
    const result = (await T.validate!(validObject)) as {
      success: true;
      value: typeof validObject;
    };
  ...
  it('should reject invalid object', async () => {
    const result = (await T.validate!(invalidObject)) as {
      success: false;
      error: Error;
    };
