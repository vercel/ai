...
export class ValidationError extends Error {
  constructor(
    public readonly errors: (TJsonSchemaError | TStandardSchemaError)[],
  ) {
...
export function schema<
  const Input extends unknown,
  Output extends unknown = Static<Input>,
>(
  input: Input,
  options: TErrorOptions = { format: 'json-schema', locale: 'en_US' },
): Schema<Output> {
  const validator = compile(input);
  if (!validator.isJsonSchema()) {
    throw Error(
      'Schema is not TypeScript, JSON Schema or Standard JSON Schema',
    );
  }
  return jsonSchema(validator.toJsonSchema() as Record<string, unknown>, {
    validate: value =>
      validator.check(value)
        ...
        : {
            success: false,
            error: new ValidationError(validator.errors(value, options)),
          },
  }) as never;
}
