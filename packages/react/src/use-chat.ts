...
export function useChat<UI_MESSAGE extends UIMessage = UIMessage>({
  ...
  // it across renders, but each method call reads through to the latest transport.
  ...
  if (
    ...
    stableTransportRef.current = {
      sendMessages: args => {
        const transport = userTransportRef.current;
        if (transport === undefined) {
          throw new Error(
            'useChat: transport was provided initially but is now undefined',
          );
        }
        return transport.sendMessages(args);
      },
      reconnectToStream: args => {
        ...
        return transport.reconnectToStream(args);
      },
    };
  }
