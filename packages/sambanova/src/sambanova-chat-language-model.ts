...
export class SambaNovaChatLanguageModel implements LanguageModelV1 {
  readonly specificationVersion = 'v1';

  readonly supportsStructuredOutputs = false;
  readonly defaultObjectGenerationMode = 'json';

  readonly modelId: SambaNovaChatModelId;
  readonly settings: SambaNovaChatSettings;

  private readonly config: SambaNovaChatConfig;

  constructor(
    modelId: SambaNovaChatModelId,
    settings: SambaNovaChatSettings,
    config: SambaNovaChatConfig,
  ) {
  ...
  private getArgs({
    ...
    const baseArgs = {
      ...
      // messages:
      messages: convertToSambaNovaChatMessages(prompt),
    };
  ...
  async doGenerate(
    ...
    return {
      ...
      finishReason: mapSambaNovaFinishReason(choice.finish_reason),
      usage: {
  ...
  async doStream(
    ...
    return {
      stream: response.pipeThrough(
        new TransformStream<
          ParseResult<z.infer<typeof sambanovaChatChunkSchema>>,
          LanguageModelV1StreamPart
        >({
...
const sambanovaChatResponseSchema = z.object({
  id: z.string().nullish(),
...
// limited version of the schema, focussed on what is needed for the implementation
// this approach limits breakages when the API changes and increases efficiency
const sambanovaChatChunkSchema = z.union([
  z.object({
    id: z.string().nullish(),
    created: z.number().nullish(),
    model: z.string().nullish(),
