import { mkdir, mkdtemp, readFile, writeFile } from 'node:fs/promises';
import { tmpdir } from 'node:os';
import { join } from 'node:path';
import { describe, expect, it, vi } from 'vitest';
import {
  createOpenCodeGitLabSubscriptionConfig,
  createOpenCodeGitLabSubscriptionRequestTransformations,
  createOpenCodeSubscriptionAuthContent,
  createOpenCodeSubscriptionRequestTransformations,
  readOpenCodeSubscription,
  requestOpenCodeGitLabDirectAccess,
  resolveOpenCodeGitLabSubscriptionModel,
} from './opencode-subscription';

describe('readOpenCodeSubscription', () => {
  it.each([
    {
      providerId: 'xai',
      record: {
        type: 'oauth',
        access: 'xai-access',
        refresh: 'xai-refresh',
        expires: Date.now() + 60 * 60 * 1000,
      },
      accessToken: 'xai-access',
    },
    {
      providerId: 'github-copilot',
      record: {
        type: 'oauth',
        access: 'github-token',
        refresh: 'github-token',
        expires: 0,
      },
      accessToken: 'github-token',
    },
    {
      providerId: 'poe',
      record: {
        type: 'oauth',
        access: 'poe-key',
        refresh: 'poe-key',
        expires: Date.now() + 60 * 60 * 1000,
      },
      accessToken: 'poe-key',
    },
    {
      providerId: 'opencode-go',
      record: { type: 'api', key: 'go-key' },
      accessToken: 'go-key',
    },
    {
      providerId: 'gitlab',
      record: {
        type: 'oauth',
        access: 'gitlab-access',
        refresh: 'gitlab-refresh',
        expires: Date.now() + 60 * 60 * 1000,
      },
      accessToken: 'gitlab-access',
    },
  ])('reads $providerId from OPENCODE_AUTH_CONTENT', async testCase => {
    await expect(
      readOpenCodeSubscription({
        providerId: testCase.providerId,
        env: {
          OPENCODE_AUTH_CONTENT: JSON.stringify({
            [testCase.providerId]: testCase.record,
          }),
        },
      }),
    ).resolves.toMatchObject({
      providerId: testCase.providerId,
      accessToken: testCase.accessToken,
    });
  });

  it('refreshes an expiring OpenAI record in the XDG auth store', async () => {
    const homeDirectory = await mkdtemp(join(tmpdir(), 'opencode-auth-'));
    const authDirectory = join(homeDirectory, '.local', 'share', 'opencode');
    const authPath = join(authDirectory, 'auth.json');
    await mkdir(authDirectory, { recursive: true });
    await writeFile(
      authPath,
      JSON.stringify({
        openai: {
          type: 'oauth',
          access: 'old-access',
          refresh: 'old-refresh',
          expires: Date.now() + 60_000,
          accountId: 'account-1',
        },
      }),
    );
    const fetch = vi.fn(
      async () =>
        new Response(
          JSON.stringify({
            access_token: 'new-access',
            refresh_token: 'new-refresh',
            expires_in: 3600,
          }),
          { status: 200 },
        ),
    );

    await expect(
      readOpenCodeSubscription({
        providerId: 'openai',
        env: {},
        homeDirectory,
        fetch,
      }),
    ).resolves.toEqual({
      providerId: 'openai',
      accessToken: 'new-access',
      accountId: 'account-1',
    });
    expect(fetch).toHaveBeenCalledWith(
      'https://auth.openai.com/oauth/token',
      expect.objectContaining({
        body: expect.stringContaining('refresh_token=old-refresh'),
      }),
    );
    await expect(readFile(authPath, 'utf8')).resolves.toContain('new-refresh');
  });

  it('reads the GitLab plugin auth store on Windows', async () => {
    const homeDirectory = await mkdtemp(join(tmpdir(), 'opencode-auth-'));
    const authDirectory = join(homeDirectory, '.opencode');
    await mkdir(authDirectory, { recursive: true });
    await writeFile(
      join(authDirectory, 'auth.json'),
      JSON.stringify({
        gitlab: {
          type: 'oauth',
          access: 'gitlab-access',
          refresh: 'gitlab-refresh',
          expires: Date.now() + 60 * 60 * 1000,
          enterpriseUrl: 'https://gitlab.example.com',
        },
      }),
    );

    await expect(
      readOpenCodeSubscription({
        providerId: 'gitlab',
        env: {},
        homeDirectory,
        platform: 'win32',
      }),
    ).resolves.toEqual({
      providerId: 'gitlab',
      accessToken: 'gitlab-access',
      enterpriseUrl: 'https://gitlab.example.com',
    });
  });
});

describe('OpenCode GitLab direct access', () => {
  it('requests an AI access token from GitLab on the host', async () => {
    const fetch = vi.fn(async () =>
      Response.json({
        token: 'direct-access-token',
        headers: { 'x-gitlab-token': 'routing-token' },
      }),
    );

    await expect(
      requestOpenCodeGitLabDirectAccess({
        accessToken: 'oauth-access-token',
        instanceUrl: 'https://gitlab.example.com/',
        aiGatewayUrl: 'https://gateway.example.com/',
        fetch,
      }),
    ).resolves.toEqual({
      accessToken: 'direct-access-token',
      headers: { 'x-gitlab-token': 'routing-token' },
      aiGatewayUrl: 'https://gateway.example.com',
    });
    expect(fetch).toHaveBeenCalledWith(
      'https://gitlab.example.com/api/v4/ai/third_party_agents/direct_access',
      {
        method: 'POST',
        headers: {
          Authorization: 'Bearer oauth-access-token',
          'Content-Type': 'application/json',
        },
        body: JSON.stringify({
          feature_flags: { DuoAgentPlatformNext: true },
        }),
      },
    );
  });

  it('rejects invalid direct access credentials without exposing them', async () => {
    await expect(
      requestOpenCodeGitLabDirectAccess({
        accessToken: 'sensitive-oauth-token',
        fetch: async () => Response.json({ token: 'sensitive-token' }),
      }),
    ).rejects.toThrow(
      'OpenCode GitLab direct access returned invalid credentials.',
    );
  });

  it('brokers the AI access token and response headers for both proxy APIs', () => {
    expect(
      createOpenCodeGitLabSubscriptionRequestTransformations({
        directAccess: {
          accessToken: 'host-direct-access-token',
          headers: {
            'x-gitlab-token': 'routing-token',
            'x-api-key': 'must-not-be-forwarded',
            Authorization: 'must-not-be-forwarded',
          },
          aiGatewayUrl: 'https://cloud.gitlab.test',
        },
        sandboxAccessToken: 'sandbox-access-token',
      }),
    ).toEqual([
      {
        match: {
          host: 'cloud.gitlab.test',
          path: { startsWith: '/ai/v1/proxy/anthropic' },
          headers: [
            {
              key: { exact: 'Authorization' },
              value: { exact: 'Bearer sandbox-access-token' },
            },
          ],
        },
        transform: {
          headers: {
            'x-gitlab-token': 'routing-token',
            Authorization: 'Bearer host-direct-access-token',
          },
        },
      },
      {
        match: {
          host: 'cloud.gitlab.test',
          path: { startsWith: '/ai/v1/proxy/openai/v1' },
          headers: [
            {
              key: { exact: 'Authorization' },
              value: { exact: 'Bearer sandbox-access-token' },
            },
          ],
        },
        transform: {
          headers: {
            'x-gitlab-token': 'routing-token',
            Authorization: 'Bearer host-direct-access-token',
          },
        },
      },
    ]);
  });

  it('configures native providers for each GitLab chat API', () => {
    const config = createOpenCodeGitLabSubscriptionConfig({
      openCodeConfig: {
        provider: { custom: { options: { apiKey: 'custom' } } },
      },
      sandboxAccessToken: 'sandbox-access-token',
      aiGatewayUrl: 'https://cloud.gitlab.test',
    });
    const providers = config.provider as Record<
      string,
      { npm?: string; api?: string; models?: Record<string, { id: string }> }
    >;

    expect(providers.custom).toEqual({ options: { apiKey: 'custom' } });
    expect(providers['ai-sdk-gitlab-anthropic']).toMatchObject({
      npm: '@ai-sdk/anthropic',
      api: 'https://cloud.gitlab.test/ai/v1/proxy/anthropic/v1',
      models: {
        'duo-chat-sonnet-4-5': {
          id: 'claude-sonnet-4-5-20250929',
        },
      },
    });
    expect(providers['ai-sdk-gitlab-openai-chat']).toMatchObject({
      npm: '@ai-sdk/openai-compatible',
      models: {
        'duo-chat-gpt-5-4': { id: 'gpt-5.4-2026-03-05' },
      },
    });
    expect(providers['ai-sdk-gitlab-openai-responses']).toMatchObject({
      npm: '@ai-sdk/openai',
      models: {
        'duo-chat-gpt-5-6-sol': { id: 'gpt-5.6-sol' },
      },
    });
  });

  it('maps GitLab chat models and rejects workflow models', () => {
    expect(
      resolveOpenCodeGitLabSubscriptionModel({
        model: 'gitlab/duo-chat-sonnet-4-5',
        provider: 'gitlab',
      }),
    ).toBe('ai-sdk-gitlab-anthropic/duo-chat-sonnet-4-5');
    expect(
      resolveOpenCodeGitLabSubscriptionModel({
        model: 'duo-chat-gpt-5-4',
        provider: 'gitlab',
      }),
    ).toBe('ai-sdk-gitlab-openai-chat/duo-chat-gpt-5-4');
    expect(() =>
      resolveOpenCodeGitLabSubscriptionModel({
        model: 'gitlab/duo-workflow',
        provider: 'gitlab',
      }),
    ).toThrow('because it authenticates over WebSocket');
  });
});

describe('OpenCode sandbox authentication', () => {
  it('creates a sanitized non-expiring OAuth record', () => {
    const content = createOpenCodeSubscriptionAuthContent({
      authentication: {
        providerId: 'openai',
        accessToken: 'host-access',
        accountId: 'account-1',
      },
      accessToken: 'sandbox-access',
    });

    expect(JSON.parse(content)).toEqual({
      openai: {
        type: 'oauth',
        access: 'sandbox-access',
        refresh: 'host-managed',
        expires: Number.MAX_SAFE_INTEGER,
        accountId: 'account-1',
      },
    });
    expect(content).not.toContain('host-access');
  });

  it('brokers the sandbox token at the subscription request route', () => {
    expect(
      createOpenCodeSubscriptionRequestTransformations({
        authentication: {
          providerId: 'openai',
          accessToken: 'host-access',
          accountId: 'account-1',
        },
        sandboxAccessToken: 'sandbox-access',
      }),
    ).toEqual([
      {
        match: {
          host: 'chatgpt.com',
          path: { startsWith: '/backend-api/codex' },
          headers: [
            {
              key: { exact: 'Authorization' },
              value: { exact: 'Bearer sandbox-access' },
            },
          ],
        },
        transform: {
          headers: {
            Authorization: 'Bearer host-access',
            'ChatGPT-Account-Id': 'account-1',
          },
        },
      },
    ]);
  });
});
