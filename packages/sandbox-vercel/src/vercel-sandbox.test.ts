...
describe('VercelSandbox', () => {
  describe('description', () => {
    it('mentions the sandbox name', () => {
      ...
      expect(new VercelSandbox(sandbox).description).toContain('sbx_test');
    });
  });
  ...
  describe('runCommand', () => {
    it('wraps the command in bash -c and maps stdout/stderr/exitCode', async () => {
      ...
      const vsbx = new VercelSandbox(sandbox);
    ...
    it('forwards workingDirectory as cwd', async () => {
      ...
      await new VercelSandbox(sandbox).runCommand({
        command: 'ls',
    ...
    it('throws on pre-aborted signal', async () => {
      ...
      ac.abort();
      await expect(
        new VercelSandbox(sandbox).runCommand({
          command: 'echo',
  ...
  describe('file I/O', () => {
    it('writeBinaryFile creates parent dirs and writes Uint8Array', async () => {
      ...
      await new VercelSandbox(sandbox).writeBinaryFile({
    ...
    it('writeTextFile encodes utf-8 and delegates', async () => {
      ...
      await new VercelSandbox(sandbox).writeTextFile({
        path: '/work/hello.txt',
        content: 'hi',
      });
    ...
    it('readBinaryFile returns the buffer bytes', async () => {
      ...
      const out = await new VercelSandbox(sandbox).readBinaryFile({
        path: '/work/x',
      });
    ...
    it('readBinaryFile returns null when SDK returns null', async () => {
      ...
      expect(
        await new VercelSandbox(sandbox).readBinaryFile({ path: '/missing' }),
      ).toBeNull();
    });
    ...
    it('readTextFile honours startLine/endLine', async () => {
      ...
      const out = await new VercelSandbox(sandbox).readTextFile({
        path: '/x',
        startLine: 2,
        endLine: 3,
      });
    ...
    it('readFile/writeFile round-trip through Web streams', async () => {
      ...
      await new VercelSandbox(sandbox).writeFile({
      ...
      const stream = await new VercelSandbox(sandbox).readFile({
        path: '/work/streamed.txt',
      });
