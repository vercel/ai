...
describe('AnthropicMessagesLanguageModel', () => {
  ...
  describe('doStream', () => {
    ...
    it('should parse iterations from streaming compaction response', async () => {
      ...
      // Usage should sum iterations
      expect(finishPart?.usage?.inputTokens).toBe(60385 + 612);
      expect(finishPart?.usage?.outputTokens).toBe(522 + 2819);
    });
...
describe('mid-conversation tool changes', () => {
  ...
  it('should send clear_at and per-turn effort with their beta headers', async () => {
    ...
    expect(betaHeader).toContain('mid-conversation-effort-2026-08-01');
  });
});
