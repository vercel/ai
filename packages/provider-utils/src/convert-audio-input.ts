 ...
 */
export function convertAudioInput(audio: Blob | File | Uint8Array | ArrayBuffer | Buffer | URL | string) {
  ...
  const processStringInput = async (input: string) => {
    ...
    try {
      ...
      throw new TypeValidationError({
        value: audio,
        cause: new Error('Invalid audio string format. Expected URL or base64 encoded data'),
      });
    }
  };

  // Helper to fetch from URL
  const fetchFromUrl = async (url: URL): Promise<{ data: ArrayBuffer, mimeType: string }> => {
    const response = await fetch(url.toString());
    if (!response.ok) {
      throw new Error(`Failed to fetch audio from URL: ${response.status} ${response.statusText}`);
    }
    
    const contentType = response.headers.get('content-type') || inferMimeType(url.pathname);
  ...
  const convert = async (): Promise<{ data: ArrayBuffer, mimeType: string }> => {
  ...
  return {
     ...
     */
    toFile: async (filename = 'audio.wav', mimeType?: string): Promise<File> => {
      const { data, mimeType: inferredType } = await convert();
