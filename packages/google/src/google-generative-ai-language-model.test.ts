...
describe('doGenerate', () => {
  ...
  it('should use newest request behavior for an unknown future Gemini model', async () => {
    prepareJsonResponse({
      content: 'done',
      url: TEST_URL_GEMINI_99_PRO,
    });

    const futureModel = provider.chat('gemini-99-pro-preview');

    await futureModel.doGenerate({
      providerOptions: {
        google: {
          thinkingConfig: {
            thinkingLevel: 'high',
  ...
  it('should pass penalty settings for Gemini 2.0 models', async () => {
    prepareJsonResponse({ url: TEST_URL_GEMINI_2_0_PRO });
  ...
  it('should pass penalty settings for Vertex Gemini 2.5 models', async () => {
    prepareJsonResponse({ url: TEST_URL_GEMINI_2_5_FLASH });
