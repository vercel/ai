...
describe('GoogleBatchLanguageModel', () => {
  ...
  it('returns schema warnings for recursive tools with batch request IDs', async () => {
    server.urls[urls.create].response = {
      type: 'json-value',
      body: operation(
        {
    ...
    const inputSchema = {
      ...
      properties: {
        ...
        kind: { type: 'string', const: 'condition' },
        mode: {
          oneOf: [
            { type: 'string', enum: ['strict'] },
            { type: 'string', enum: ['relaxed'] },
          ],
        },
        ...
        prompt: [
          {
            ...
            content: [
              {
                type: 'text' as const,
                text: 'Search the condition tree.',
              },
            ],
          },
        ],
    ...
    expect(result.warnings).toEqual([
      {
        requestId: 'recursive-tool',
        warning: {
          type: 'unsupported',
          feature: 'JSON Schema constraint "const"',
          details:
            'The constraint at "/properties/kind/const" is not supported by the Google function parameter JSON Schema surface. The schema is sent unchanged, but this constraint may not restrict values generated by the model.',
        },
      },
      {
        requestId: 'recursive-tool',
        warning: {
          ...
          details:
            'The Google function parameter JSON Schema surface treats "oneOf" as "anyOf" at "/properties/mode/oneOf". Values matching multiple branches may be accepted.',
        },
      },
      {
        requestId: 'recursive-tool',
        warning: {
          ...
          details:
            'The constraint at "/properties/score/multipleOf" is not supported by the Google function parameter JSON Schema surface. The schema is sent unchanged, but this constraint may not restrict values generated by the model.',
        },
      },
    ]);
    expect(
      (await server.calls[0].requestBodyJson).batch.inputConfig.requests
        .requests[0].request.tools[0].functionDeclarations[0],
    ).toEqual({
      name: 'search',
      description: 'Search with a condition tree',
