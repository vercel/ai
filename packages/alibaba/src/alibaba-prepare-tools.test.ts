...
describe('prepareTools', () => {
  ...
  it('should throw for an unrecognized tool choice type', () => {
    // The switch ends in an exhaustiveness check, so this is unreachable
    // through the public types and needs a cast to exercise. Worth covering:
    // it is the one branch where a provider's copy of this function could
    ...
    expect(() =>
      prepareTools({
        ...
        toolChoice: {
          type: 'unrecognized',
        } as unknown as LanguageModelV4CallOptions['toolChoice'],
      }),
