...
export class AlibabaEmbeddingModel implements EmbeddingModelV2<string> {
  ...
  > {
    ...
    // The AI SDK embedding interface requires dense number arrays.
      ...
      .map(item =>
        item.sparse_embedding == null
          ...
          : {
              ...
              sparseEmbedding: item.sparse_embedding.map(sparseItem => ({
                index: sparseItem.index,
                value: sparseItem.value,
                ...(sparseItem.token == null
                  ? {}
                  : { token: sparseItem.token }),
              })),
            },
      )
