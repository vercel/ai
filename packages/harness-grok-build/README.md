...
Set `XAI_API_KEY` for direct authentication. Set `AI_GATEWAY_API_KEY` or `VERCEL_OIDC_TOKEN` to use AI Gateway instead, or pass a programmatically resolved credential with `createGrokBuild({ auth: { gateway: { apiKey } } })`.
