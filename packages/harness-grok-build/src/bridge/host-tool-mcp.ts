#!/usr/bin/env node
// MCP-stdio tool server spawned by the grok CLI when an `mcpServers` entry is
// passed to `session/new`. Exposes host-defined tools over MCP-stdio and
// round-trips each call to the bridge's HTTP relay.
//
 ...
 * tsup.config.ts and resolved at runtime from the node_modules the bridge
 * installs *inside the sandbox* from `src/bridge/package.json` (and its pinned
 * `pnpm-lock.yaml`). Keep import, tsup `external`, and bridge package.json in
 * sync.
 */
