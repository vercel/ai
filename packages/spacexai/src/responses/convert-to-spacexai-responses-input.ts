...
import { spacexaiFilePartProviderOptions } from '../spacexai-file-part-options';
import {
...
import type {
  ...
  SpaceXAIResponsesInput,
  SpaceXAIResponsesUserMessageContentPart,
} from './spacexai-responses-api';

export async function convertToSpaceXAIResponsesInput({
  prompt,
}: {
  prompt: LanguageModelV4Message[];
  store?: boolean;
}): Promise<{
  input: SpaceXAIResponsesInput;
  inputWarnings: SharedV4Warning[];
}> {
  const input: SpaceXAIResponsesInput = [];
  ...
  for (const message of prompt) {
    switch (message.role) {
      ...
      case 'user': {
        const contentParts: SpaceXAIResponsesUserMessageContentPart[] = [];
                ...
                case 'data': {
                  if (getTopLevelMediaType(block.mediaType) === 'image') {
                    ...
                    const filePartOptions = await parseSpaceXAIProviderOptions({
                      ...
                      schema: spacexaiFilePartProviderOptions,
                    });
      ...
      case 'tool': {
        for (const part of message.content) {
          ...
          let outputValue: SpaceXAIResponsesFunctionCallOutput['output'];
          switch (output.type) {
            case 'text':
            case 'error-text':
