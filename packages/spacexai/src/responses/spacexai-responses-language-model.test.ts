...
import { SpaceXAIResponsesLanguageModel as XaiResponsesLanguageModel } from './spacexai-responses-language-model';
import type { SpaceXAILanguageModelResponsesOptions as XaiLanguageModelResponsesOptions } from './spacexai-responses-language-model-options';
...
describe('SpaceXAIResponsesLanguageModel', () => {
  const server = createTestServer({
    'https://api.x.ai/v1/responses': {},
  });

  function prepareJsonFixtureResponse(filename: string) {
    filename = filename.replace(/^xai-/, 'spacexai-');
