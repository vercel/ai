...
import {
  spacexaiVideoModelOptionsSchema,
  type SpaceXAIParsedVideoModelOptions,
...
interface SpaceXAIVideoModelConfig {
  provider: string;
...
type SpaceXAIVideoCallOptions = VideoModelV4CallOptions;

function getFirstFrameImage(
  options: SpaceXAIVideoCallOptions,
...
function fileToSpaceXAIUrl(file: VideoModelV4File): string {
  if (file.type === 'url') {
    return file.url;
  }
...
function resolveReferences(
  options: SpaceXAIVideoCallOptions,
  spacexaiOptions: SpaceXAIParsedVideoModelOptions | undefined,
  ...
  if (options.inputReferences != null && options.inputReferences.length > 0) {
    ...
    return imageFiles.length > 0
      ? imageFiles.map(reference => ({ url: fileToSpaceXAIUrl(reference) }))
      : undefined;
  }

  if (
    spacexaiOptions?.referenceImageUrls != null &&
    spacexaiOptions.referenceImageUrls.length > 0
  ) {
    return spacexaiOptions.referenceImageUrls.map(url => ({ url }));
  }
...
function hasImageInputReference(options: SpaceXAIVideoCallOptions): boolean {
...
function resolveVideoMode(
  options: SpaceXAIVideoCallOptions,
  spacexaiOptions: SpaceXAIParsedVideoModelOptions | undefined,
): SpaceXAIParsedVideoModelOptions['mode'] | undefined {
  if (spacexaiOptions?.mode != null) {
    return spacexaiOptions.mode;
  }

  if (spacexaiOptions?.videoUrl != null) {
    return 'edit-video';
  }
...
export class SpaceXAIVideoModel implements VideoModelV4 {
  ...
  constructor(
    readonly modelId: SpaceXAIVideoModelId,
    private config: SpaceXAIVideoModelConfig,
  ) {}
  ...
  private async buildRequestBody(options: VideoModelV4CallOptions): Promise<{
    ...
    effectiveMode: SpaceXAIParsedVideoModelOptions['mode'] | undefined;
  }> {
    const warnings: SharedV4Warning[] = [];

    const spacexaiOptions = (await parseSpaceXAIProviderOptions({
      providerOptions: options.providerOptions,
      schema: spacexaiVideoModelOptionsSchema,
    })) as SpaceXAIParsedVideoModelOptions | undefined;

    const effectiveMode = resolveVideoMode(options, spacexaiOptions);
    ...
    if (
      isEdit &&
      (spacexaiOptions?.resolution != null || options.resolution != null)
    ) {
      warnings.push({
        type: 'unsupported',
        feature: 'resolution',
    ...
    if (allowResolution && spacexaiOptions?.resolution != null) {
    ...
    if (isEdit) {
      body.video = { url: spacexaiOptions!.videoUrl };
    }
    ...
    if (startImage != null) {
      if (isVideoFile(startImage)) {
        ...
        body.image = { url: fileToSpaceXAIUrl(startImage) };
      }
    }
    ...
    if (hasReferenceImages) {
      const referenceImages = resolveReferences(
        options,
        spacexaiOptions,
        warnings,
      );
      ...
      const referenceVoiceIds = spacexaiOptions?.referenceVoiceIds;
    ...
    if (
      spacexaiOptions?.referenceVoiceIds != null &&
      spacexaiOptions.referenceVoiceIds.length > 0 &&
      !hasReferenceImages
    ) {
      warnings.push({
        type: 'unsupported',
    ...
    if (!isExtension && spacexaiOptions?.user !== undefined) {
      body.user = spacexaiOptions.user;
    }

    if (spacexaiOptions != null) {
      for (const [key, value] of Object.entries(spacexaiOptions)) {
        if (
          ![
            'mode',
            'pollIntervalMs',
  ...
  async doStart(
    ...
    const { value: createResponse, responseHeaders } = await postJsonToApi({
      ...
      successfulResponseHandler: createJsonResponseHandler(
        spacexaiCreateVideoResponseSchema,
      ),
  ...
  async doStatus(
    ...
    const { value: statusResponse, responseHeaders } = await getFromApi({
      ...
      validateUrl: false,
      headers: combineHeaders(this.config.headers(), options.headers),
      successfulResponseHandler: spacexaiVideoStatusResponseHandler,
      failedResponseHandler: spacexaiFailedResponseHandler,
      abortSignal: options.abortSignal,
      fetch: this.config.fetch,
    });
...
const spacexaiCreateVideoResponseSchema = z.object({
  request_id: z.string().nullish(),
});

const spacexaiVideoStatusResponseSchema = z.object({
...
const spacexaiVideoStatusJsonResponseHandler = createJsonResponseHandler(
  spacexaiVideoStatusResponseSchema,
);
...
const spacexaiVideoStatusResponseHandler: ResponseHandler<
  z.infer<typeof spacexaiVideoStatusResponseSchema>
> = async options => {
  ...
  if (options.response.status === 202) {
    ...
    const parsed = await safeParseJSON({
      text,
      schema: spacexaiVideoStatusResponseSchema,
    });
  ...
  return spacexaiVideoStatusJsonResponseHandler(options);
};
