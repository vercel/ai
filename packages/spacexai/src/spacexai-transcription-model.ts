...
import {
  spacexaiTranscriptionModelOptionsSchema,
...
interface SpaceXAITranscriptionModelConfig {
  provider: string;
...
type SpaceXAIStreamingTranscriptionEvent = {
  type?: string;
  text?: string;
...
export class SpaceXAITranscriptionModel implements TranscriptionModelV4 {
  readonly specificationVersion = 'v4';

  static [WORKFLOW_SERIALIZE](model: SpaceXAITranscriptionModel) {
  ...
  static [WORKFLOW_DESERIALIZE](options: {
    modelId: '';
    config: SpaceXAITranscriptionModelConfig;
  }) {
    return new SpaceXAITranscriptionModel(options.modelId, options.config);
  }
  ...
  constructor(
    readonly modelId: '',
    private readonly config: SpaceXAITranscriptionModelConfig,
  ) {}
  ...
  private async getArgs({
    ...
    const spacexaiOptions = await parseSpaceXAIProviderOptions({
      providerOptions,
      schema: spacexaiTranscriptionModelOptionsSchema,
    });

    const formData = new FormData();
    const transcriptionOptions = {
      audio_format: spacexaiOptions?.audioFormat,
      sample_rate: spacexaiOptions?.sampleRate,
      language: spacexaiOptions?.language,
      format: spacexaiOptions?.format,
      multichannel: spacexaiOptions?.multichannel,
      channels: spacexaiOptions?.channels,
      diarize: spacexaiOptions?.diarize,
      filler_words: spacexaiOptions?.fillerWords,
    };
    ...
    if (spacexaiOptions?.keyterm != null) {
      const keyterms = Array.isArray(spacexaiOptions.keyterm)
        ? spacexaiOptions.keyterm
        : [spacexaiOptions.keyterm];
  ...
  async doGenerate(
    ...
    const {
      ...
      successfulResponseHandler: createJsonResponseHandler(
        spacexaiTranscriptionResponseSchema,
      ),
  ...
  > {
    ...
    if (
      spacexaiOptions?.multichannel === true &&
      spacexaiOptions.channels == null
    ) {
    ...
    if (
      spacexaiOptions?.audioFormat == null &&
    ...
    const url = buildSpaceXAIStreamingTranscriptionUrl({
      baseURL: this.config.baseURL ?? 'https://api.x.ai/v1',
      inputAudioFormat: options.inputAudioFormat,
      providerOptions: spacexaiOptions,
    });
    ...
    return {
      ...
      stream: createSpaceXAIStreamingTranscriptionStream({
        webSocket: this.config.webSocket,
        url,
        headers,
        warnings,
        language: spacexaiOptions?.language ?? undefined,
        expectedDoneCount:
          spacexaiOptions?.multichannel === true
            ? spacexaiOptions.channels!
            : 1,
        audio: options.audio,
        abortSignal: options.abortSignal,
...
function createSpaceXAIStreamingTranscriptionStream({
  webSocket,
  url,
  headers,
  warnings,
    ...
    start: controller => {
      ...
      connection = connectToWebSocket({
        ...
        onMessageText: async text => {
          ...
          const raw = parsed.value as SpaceXAIStreamingTranscriptionEvent;
          ...
          switch (raw.type) {
            ...
            case 'transcript.partial': {
              ...
              if (raw.is_final && raw.speech_final) {
                const timing = timingFromSpaceXAIEvent(raw);
                if (raw.text) {
...
function buildSpaceXAIStreamingTranscriptionUrl({
  baseURL,
  inputAudioFormat,
  providerOptions,
}: {
  ...
  providerOptions: SpaceXAITranscriptionModelOptions | undefined;
}) {
...
function timingFromSpaceXAIEvent(event: SpaceXAIStreamingTranscriptionEvent) {
  return {
...
const spacexaiTranscriptionResponseSchema = z.object({
  text: z.string(),
