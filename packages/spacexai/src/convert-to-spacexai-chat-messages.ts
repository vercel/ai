...
import type {
  SpaceXAIChatPrompt,
  SpaceXAIUserMessageContent,
} from './spacexai-chat-prompt';
...
export async function convertToSpaceXAIChatMessages(
  prompt: LanguageModelV4Prompt,
): Promise<{
  messages: SpaceXAIChatPrompt;
  warnings: Array<SharedV4Warning>;
}> {
  const messages: SpaceXAIChatPrompt = [];
  ...
  for (const { role, content } of prompt) {
    switch (role) {
      ...
      case 'user': {
        ...
        const userContent: Array<SpaceXAIUserMessageContent> = [];
