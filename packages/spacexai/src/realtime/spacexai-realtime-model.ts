...
import {
  buildSpaceXAISessionConfig,
  parseSpaceXAIRealtimeServerEvent,
  serializeSpaceXAIRealtimeClientEvent,
} from './spacexai-realtime-event-mapper';

export type SpaceXAIRealtimeModelConfig = {
  provider: string;
  baseURL: string;
...
export class SpaceXAIRealtimeModel implements RealtimeModelV4 {
  readonly specificationVersion = 'v4' as const;
  readonly provider: string;
  readonly modelId: string;

  private readonly config: SpaceXAIRealtimeModelConfig;

  constructor(modelId: string, config: SpaceXAIRealtimeModelConfig) {
  ...
  parseServerEvent(raw: unknown): RealtimeModelV4ServerEvent {
    return parseSpaceXAIRealtimeServerEvent(raw);
  }

  serializeClientEvent(event: RealtimeModelV4ClientEvent): unknown {
    return serializeSpaceXAIRealtimeClientEvent(event);
  }

  buildSessionConfig(
    config: RealtimeModelV4SessionConfig,
  ): Record<string, unknown> {
    return buildSpaceXAISessionConfig(config);
  }
}
