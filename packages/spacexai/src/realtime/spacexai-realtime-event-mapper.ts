...
type SpaceXAIRealtimeWireEvent = {
  type: string;
  session?: { id?: string };
...
export function parseSpaceXAIRealtimeServerEvent(
  raw: unknown,
): RealtimeModelV4ServerEvent {
  const event = raw as SpaceXAIRealtimeWireEvent;
...
export function serializeSpaceXAIRealtimeClientEvent(
  ...
  switch (event.type) {
    case 'session-update':
      return {
        type: 'session.update',
        session: buildSpaceXAISessionConfig(event.config),
      };
...
export function buildSpaceXAISessionConfig(
  ...
  if (config.providerOptions != null) {
    const spacexaiOptions = config.providerOptions as Record<string, unknown>;

    if (Array.isArray(spacexaiOptions.tools)) {
      const existingTools = (session.tools as unknown[]) ?? [];
      session.tools = [...existingTools, ...spacexaiOptions.tools];
    }
