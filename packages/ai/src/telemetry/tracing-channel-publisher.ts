...
/**
 * Starts a tracing-channel span while setting up a lazy stream and publishes the
 * async end event when the stream's completion signal settles.
 */
export async function traceTelemetryChannelStream<T>({
  message,
  execute,
  completion,
}: {
  ...
  try {
    const result = await tracingChannel.start.runStores(context, async () => {
      const result = await tracedExecute();
    ...
    if (hasExecutionError) {
      publishAsyncEnd({ error: executionError });
      throw executionError;
    }
