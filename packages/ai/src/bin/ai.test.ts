...
describe('AI CLI', () => {
  ...
  const createTestFile = (content: string, extension = '.txt'): string => {
    const testFile = resolve(`test-${Date.now()}-${Math.random().toString(36).slice(2)}${extension}`);
  ...
  describe('parseArgs', () => {
    ...
    it('should parse model flag', async () => {
      process.argv = ['node', 'ai', '-m', 'anthropic/claude-3-5-sonnet-20241022', 'Hello'];
    ...
    it('should parse long model flag', async () => {
      process.argv = ['node', 'ai', '--model', 'groq/llama-3.1-8b-instant', 'Hello'];
    ...
    it('should parse file flags', async () => {
      ...
      process.argv = ['node', 'ai', '-f', testFile1, '--file', testFile2, 'Analyze these files'];
    ...
    it('should parse system message flag', async () => {
      process.argv = ['node', 'ai', '-s', 'You are a helpful assistant', 'Hello'];
  ...
  describe('file handling', () => {
    ...
    it('should throw error for non-existent file', async () => {
      ...
      expect(() => readFileContent('/path/to/nonexistent/file.txt'))
        .toThrow('File not found: /path/to/nonexistent/file.txt');
    });
  });
  ...
  describe('formatAttachedFiles', () => {
    it('should format text files correctly', async () => {
      const files = [
        ...
        { name: 'file2.js', content: 'console.log("test");', mediaType: 'application/javascript' },
      ];
    ...
    it('should exclude image files from formatting', async () => {
      const files = [
        ...
        { name: 'image.png', content: 'data:image/png;base64,abc123', mediaType: 'image/png' },
      ];
  ...
  describe('integration scenarios', () => {
    it('should handle model switching for images', async () => {
      ...
      const imageFiles = attachedFiles.filter(f => f.mediaType?.startsWith('image/'));
    ...
    it('should not switch model if already specified', async () => {
      ...
      process.argv = ['node', 'ai', '-m', 'anthropic/claude-3-5-sonnet-20241022', '-f', imageFile, 'Describe'];
