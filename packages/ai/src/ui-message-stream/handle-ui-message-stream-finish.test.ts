...
describe('handleUIMessageStreamFinish', () => {
  ...
  describe('abort scenarios', () => {
    ...
    it('should call onFinish when fetch is aborted (realistic streaming scenario)', async () => {
      ...
      // Create a stream that properly errors when aborted (like real fetch)
      const stream = new ReadableStream<UIMessageChunk>({
        start(controller) {
          // Wire the stream to the abort signal - this is what fetch does
          const onAbort = () => {
          ...
          // Enqueue initial chunks synchronously
          ...
          // More chunks would come async, but we'll abort before they arrive
          setTimeout(() => {
      ...
      // Now abort the fetch (like user navigating away)
      abortController.abort();

      // Next read should reject with AbortError (this is what fetch does)
      await expect(reader.read()).rejects.toThrow('Aborted');

      // The critical part: onFinish should still be called despite the error
      // This is what our cancel() method ensures happens
      expect(onFinishCallback).toHaveBeenCalledTimes(1);
