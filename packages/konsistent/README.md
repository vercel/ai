...
Add both `konsistent` and `@ai-sdk/konsistent` as dev dependencies of your provider package:

```bash
pnpm add -D konsistent @ai-sdk/konsistent
```

Then run `konsistent` against the bundled config, supplying your provider's kebab-case id as the `providerId` placeholder:

```bash
konsistent --config-package=@ai-sdk/konsistent --placeholder=providerId:my-provider
```
...
If you'd rather author your own `konsistent.json` and pull in only the conventions you want, declare `@ai-sdk/konsistent` as a convention source and reference its conventions by name:

```json
{
  "version": "v1",
  "conventionSources": {
    "aisdk": "@ai-sdk/konsistent"
  },
  "conventions": [
    {
