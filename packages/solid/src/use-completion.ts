...
import { createEffect, createMemo, createSignal, createUniqueId, Setter, type JSX, type Accessor } from 'solid-js';
...
export function useCompletion({
  ...
  const { v: completionData, mutate, revalidate } = useSwr<string, string>(createSwrKey(completionId, api));
  const completion = createMemo(() => completionData().data ?? initialCompletion);

  // eslint-disable-next-line solid/reactivity -- The key is not reactive, but requires using createMemo to access the data
  const { v: isLoadingData, mutate: mutateLoading } = useSwr<boolean, string>(createSwrKey(completionId, 'loading'));
