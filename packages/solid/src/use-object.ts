...
>): Experimental_UseObjectHelpers<RESULT, INPUT> {
  ...
  const completionId = createMemo(() => id?.() ?? hookId);

  // Store the completion state in SWR, using the completionId as the key to share states.
  const { v: objectData, mutate } = useSwr<DeepPartial<RESULT>, string>(
    // eslint-disable-next-line solid/reactivity -- The key is not reactive, but requires using createMemo to access the data
    createSwrKey(completionId, api),
  );

  const object = createMemo(() => objectData().data ?? initialValue);
