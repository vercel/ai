...
describe('BedrockChatLanguageModel doStream', () => {
  it('surfaces a modeled exception event-stream frame', async () => {
    const modeledExceptionFrame = Buffer.from(
      fs
        .readFileSync(
          'src/__fixtures__/model-stream-error-exception.eventstream.base64.txt',
          'utf8',
        )
        .trim(),
      'base64',
    ...
    const model = new BedrockChatLanguageModel(
      'anthropic.claude-3-haiku-20240307-v1:0',
      {
        ...
        headers: {},
        fetch: async () =>
          new Response(createStream([modeledExceptionFrame]), {
            status: 200,
            headers: {
