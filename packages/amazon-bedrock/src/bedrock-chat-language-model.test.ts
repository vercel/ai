...
describe('doStream', () => {
  ...
  it('should filter out streamed tool calls that violate toolChoice constraint', async () => {
    setupMockEventStreamHandler();
    server.urls[streamUrl].response = {
      type: 'stream-chunks',
      chunks: [
        ...
        JSON.stringify({
          contentBlockDelta: {
            contentBlockIndex: 0,
            delta: { toolUse: { input: '"San Francisco"}' } },
          },
        }) + '\n',
        JSON.stringify({
          contentBlockStop: { contentBlockIndex: 0 },
    ...
    const result = await convertReadableStreamToArray(stream);
    const toolCalls = result.filter(part => part.type === 'tool-call');
    expect(toolCalls).toHaveLength(0);
  });

  it('should keep streamed tool calls that match toolChoice constraint', async () => {
    setupMockEventStreamHandler();
    server.urls[streamUrl].response = {
      type: 'stream-chunks',
      chunks: [
    ...
    const { stream } = await model.doStream({
      ...
      toolChoice: { type: 'tool', toolName: 'getCurrentWeather' },
      prompt: TEST_PROMPT,
      includeRawChunks: false,
    });
...
describe('doGenerate', () => {
  ...
  it('should filter out tool calls that violate toolChoice constraint', async () => {
    server.urls[generateUrl].response = {
      type: 'json-value',
      body: {
        output: {
  ...
  it('should keep tool calls that match toolChoice constraint', async () => {
    server.urls[generateUrl].response = {
      type: 'json-value',
      body: {
        output: {
  ...
  it('should not filter tool calls when toolChoice is auto', async () => {
    server.urls[generateUrl].response = {
      type: 'json-value',
      body: {
        output: {
