...
const BedrockRerankingModelMock = BedrockRerankingModel as unknown as Mock;
...
vi.mock('./reranking/bedrock-reranking-model', () => ({
  BedrockRerankingModel: vi.fn(),
}));
...
describe('AmazonBedrockProvider', () => {
  ...
  describe('createAmazonBedrock', () => {
    ...
    it('uses a separate service-specific endpoint for Bedrock Agent Runtime', () => {
      ...
      const constructorCall = BedrockRerankingModelMock.mock.calls[0];
      expect(constructorCall[1].baseUrl()).toBe(
