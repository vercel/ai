...
describe('issue #11409 Kimi K2 Thinking tool calls', () => {
  it('keeps the required read_file action structured and reaches the final answer', async () => {
    ...
    const visibleText = readFile
      ...
      .map(part => part.delta)
      .join('');
    const structuredReadFileCall = readFile.some(
      part => part.type === 'tool-call' && part.toolName === 'read_file',
    );
    const toolSyntaxIsVisible = visibleText.includes('<function=read_file>');
    const finalAnswerReturned =
      visibleText.trim() === 'FOUND ISSUE_11409_CONTENT';

    expect(
      structuredReadFileCall && !toolSyntaxIsVisible && finalAnswerReturned,
      'ISSUE_11409_REPRODUCED: read_file was emitted as visible text, not executed, and the final answer was missing',
    ).toBe(true);
  });
});
