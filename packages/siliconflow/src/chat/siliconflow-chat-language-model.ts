...
} from './siliconflow-chat-api-types';
import { type SiliconFlowChatModelId } from './siliconflow-chat-language-model-options';
...
export class SiliconFlowChatLanguageModel implements LanguageModelV4 {
  ...
  private async getArgs({
    ...
    if (reasoning != null) {
      allWarnings.push({ type: 'unsupported', feature: 'reasoning' });
    }
      ...
      providerMetadata: {
        [this.providerOptionsName]: {
          ...
          promptCacheMissTokens:
            responseBody.usage?.prompt_cache_miss_tokens ?? undefined,
        },
      },
      request: { body: args },
      response: {
