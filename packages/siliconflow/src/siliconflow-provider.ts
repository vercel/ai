...
import type { SiliconFlowChatModelId } from './chat/siliconflow-chat-language-model-options';
...
export interface SiliconFlowProviderSettings {
  /**   * SiliconFlow API key.   */ apiKey?: string;
  /**   * Base URL for the API calls.   */ baseURL?: string;
  /**   * Custom headers to include in the requests.   */ headers?: Record<
    string,
    string
  >;
...
export interface SiliconFlowProvider extends ProviderV4 {
  ...
  /**   * Creates a SiliconFlow model for text generation.   */ languageModel(
    modelId: SiliconFlowChatModelId,
  ): LanguageModelV4;
  /**   * Creates a SiliconFlow chat model for text generation.   */ chat(
    modelId: SiliconFlowChatModelId,
  ): LanguageModelV4;
  /**   * @deprecated Use `embeddingModel` instead.   */ textEmbeddingModel(
    modelId: string,
  ): never;
}
export function createSiliconFlow(
  ...
  const getHeaders = () =>
    withUserAgentSuffix(
      {
        Authorization: `Bearer ${loadApiKey({ apiKey: options.apiKey, environmentVariableName: 'SILICONFLOW_API_KEY', description: 'SiliconFlow API key' })}`,
        ...options.headers,
      },
