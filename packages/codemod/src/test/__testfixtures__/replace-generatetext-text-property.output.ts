...
export async function basicGenerateText() {
  ...
  console.log(result.text.text); // "Hello, world!"
...
export async function generateTextWithAssignment() {
  ...
  const message = result.text.text;
  const upperText = result.text.text.toUpperCase();
...
export async function generateTextInObject() {
  ...
  return {
    ...
    length: result.text.text.length,
    isEmpty: !result.text.text
  };
}
...
export async function multipleGenerateText() {
  ...
  return result1.text.text + ' ' + result2.text.text;
}
...
export async function generateTextWithChaining() {
  ...
  return result.text.text.trim().toLowerCase();
}
...
export async function generateTextWithRandomName() {
  ...
  return random.text.text.trim().toLowerCase();
}
