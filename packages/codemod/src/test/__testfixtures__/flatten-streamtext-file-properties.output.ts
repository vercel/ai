...
export async function streamTextWithDirectUsage() {
  ...
  for await (const delta of result.fullStream) {
    if (delta.type === 'file') {
      const isImage = delta.mediaType.startsWith('image/');
      const dataLength = delta.data.length;
...
export async function streamTextWithMethodChaining() {
  ...
  for await (const delta of result.fullStream) {
    if (delta.type === 'file') {
      const extension = delta.mediaType.split('/')[1];
      const base64Data = delta.data.toString('base64');
