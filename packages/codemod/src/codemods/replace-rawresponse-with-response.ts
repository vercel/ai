  ...
  root
    ...
    .forEach(path => {
      ...
      if (init && init.type === 'AwaitExpression' && 
          init.argument && init.argument.type === 'CallExpression' &&
        ...
        if (id.type === 'Identifier') {
          ...
          const hasResponse = id.properties.some(prop => 
          ...
          if (hasResponse && rawResponseVarName) {
            ...
            const responseProp = id.properties.find(prop => 
              (prop.type === 'Property' || prop.type === 'ObjectProperty') && 
              prop.key.type === 'Identifier' && 
              prop.key.name === 'response'
            ) as any;
  ...
  root
    ...
    .forEach(path => {
      ...
      if (right.type === 'AwaitExpression' &&
          right.argument && right.argument.type === 'CallExpression' &&
  ...
  root
    ...
    .forEach(path => {
      ...
      if (varName && destructuredMapping.has(varName)) {
        ...
        if ((parent.value.type === 'Property' || parent.value.type === 'ObjectProperty') && parent.value.key === path.node) {
          return;
        }
