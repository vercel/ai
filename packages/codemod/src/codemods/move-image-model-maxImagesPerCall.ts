    ...
    .forEach((path) => {
      ...
      if (
        ...
        modelProperty.value.callee.property.name === 'image'
          ...
          if (imageCall.arguments.length === 2 && imageCall.arguments[1].type === 'ObjectExpression') {
            ...
            settingsObject.properties.forEach((settingProp: any) => {
              if (settingProp.type === 'Property' || settingProp.type === 'ObjectProperty') {
                const keyName = settingProp.key.type === 'Identifier' 
        ...
        if (maxImagesPerCallFromModel) {
          const maxImagesPerCallProp = j.property('init', j.identifier('maxImagesPerCall'), maxImagesPerCallFromModel);
        ...
        if (Object.keys(providerSettingsFromModel).length > 0 && providerName) {
          ...
          const providerSettingsProperties = Object.entries(providerSettingsFromModel).map(([key, value]) => 
            j.property('init', j.identifier(key), value)
          );

          const providerOptionsValue = j.objectExpression([
            j.property('init', j.identifier(providerName), j.objectExpression(providerSettingsProperties))
          ]);

                      providerOptionsProperty = j.property('init', j.identifier('providerOptions'), providerOptionsValue);
            ...
            if (providerOptionsProperty.value.type === 'ObjectExpression') {
              ...
              if (!providerProperty) {
                ...
                providerProperty = j.property('init', j.identifier(providerName), j.objectExpression(providerSettingsProperties));
                ...
                if (providerProperty.value.type === 'ObjectExpression') {
                  const newSettingsProperties = Object.entries(providerSettingsFromModel).map(([key, value]) => 
                    j.property('init', j.identifier(key), value)
                  );
