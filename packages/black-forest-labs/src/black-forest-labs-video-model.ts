...
type BlackForestLabsVideoDoGenerateOptions = VideoModelV3CallOptions;
...
export class BlackForestLabsVideoModel implements VideoModelV3 {
  ...
  private async start(options: VideoModelV3CallOptions) {
  ...
  private async getStatus(options: {
    ...
    const pollingUrl = url.toString();
    if (!isTrustedUrl(pollingUrl, this.config.baseURL)) {
      throw new AISDKError({
        name: 'BLACK_FOREST_LABS_VIDEO_GENERATION_ERROR',
        message: `Black Forest Labs returned an untrusted polling URL: ${pollingUrl}`,
      });
    }
    ...
    return { status: 'pending' as const, response };
  }
