...
describe('AIStream', () => {
  ...
  describe('OpenAIStream', () => {
    if (typeof Response === 'undefined') {
      ...
      it('should support { mode: "raw" }', async () => {
        const stream = OpenAIStream(
          await fetch(server.api, {
            headers: {
              'x-mock-service': 'openai',
        ...
        const rawChunks = await client.readAll()
        const chunks = rawChunks.map(chunk => JSON.parse(chunk))

        /**
         * In `raw` mode, the chunks should come back exactly as the server gets
         * them.
         */
        expect(JSON.stringify(chunks)).toEqual(JSON.stringify(server.getRecentFlushed()))
      })

      it('should support { mode: "json" }', async () => {
        const stream = OpenAIStream(
          await fetch(server.api, {
            headers: {
              'x-mock-service': 'openai',
        ...
        expect(JSON.stringify(server.getRecentFlushed())).toMatchSnapshot()
      })
